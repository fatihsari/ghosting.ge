                                ...
                                else
                                {
                                    ...
                                    foreach( $addons as $addon ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            $addon->terminationDate = Carbon\Carbon::now()->toDateString();
                                            $addon->save();
...
function ServerCreateAccount($serviceID, $addonId = 0)
{
    ...
    if( !$params["password"] ) 
    {
        $updateInfo["password"] = encrypt(createServerPassword());
    }
...
function ServerUsageUpdate()
{
    ...
    while( $serverData = mysql_fetch_array($result) ) 
    {
        ...
        {
            $updatedServerIds[] = $serverId;
            $response = $server->call("UsageUpdate", $server->getServerParams($serverId, $serverData));
            if( $response && !in_array($response, array( "success", WHMCS\Module\Server::FUNCTIONDOESNTEXIST )) ) 
            {
                logActivity("Server Usage Update Failed: " . $response . " - Server ID: " . $serverId);
            }
...
function moduleConfigFieldOutput($values)
{
    ...
    {
        case "text":
            $code = "<input type=\"text\" name=\"" . $values["Name"] . "\" class=\"form-control input-inline " . $inputClass . "\" value=\"" . WHMCS\Input\Sanitize::encode($values["Value"]) . "\"" . ((isset($values["Placeholder"]) ? " placeholder=\"" . $values["Placeholder"] . "\"" : "")) . " />";
            ...
            break;
        case "password":
            $code = "<input type=\"password\" autocomplete=\"off\" name=\"" . $values["Name"] . "\" class=\"form-control input-inline " . $inputClass . "\" value=\"" . replacePasswordWithMasks($values["Value"]) . "\" />";
            if( isset($values["Description"]) ) 
            {
                $code .= " " . $values["Description"];
            }
        ...
        case "dropdown":
            ...
            $code .= " class=\"form-control select-inline\">";
        ...
        case "textarea":
            ...
            $code = "<textarea class=\"form-control\" name=\"" . $values["Name"] . "\" cols=\"" . $cols . "\" rows=\"" . $rows . "\">" . WHMCS\Input\Sanitize::encode($values["Value"]) . "</textarea>";
            if( isset($values["Description"]) ) 
            {
