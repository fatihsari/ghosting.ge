...
{
    $clientDateFormat = WHMCS\Config\Setting::getValue("ClientDateFormat");
    ...
    return str_replace(array( "DD", "MM", "YYYY" ), array( "d", "m", "Y" ), WHMCS\Config\Setting::getValue("DateFormat"));
}

function fromMySQLDate($date, $time = "", $client = "", $zerodateval = "")
{
    global $timeoffset;
    if( $date instanceof Carbon\Carbon && (string) $date === (string) Carbon\Carbon::createFromTimestamp(0, "UTC") ) 
    {
        $date = "0000-00-00";
    }
    ...
    $clientDateFormat = getClientDateFormat();
    $year = substr($date, 0, 4);
    $month = substr($date, 5, 2);
    $day = substr($date, 8, 2);
    $hours = substr($date, 11, 2);
    ...
    if( $client && $clientDateFormat && !$isZeroDate ) 
    {
        $date = date($clientDateFormat, mktime(0, 0, 0, $month, $day, $year));
        if( $time ) 
        {
            $date .= " (" . $hours . ":" . $minutes . ")";
        }
    ...
    {
        $date = WHMCS\Config\Setting::getValue("DateFormat");
        if( !$date ) 
        {
            $date = "DD/MM/YYYY";
        }
...
{
    $title = foreignChrReplace(mb_strtolower($title));
    ...
    $title = str_replace(" ", "-", $title);
    $title = preg_replace("/[^0-9a-zA-Zа-яА-Яა-ჰ-]/iu", "", $title);
    return $title;
}
...
function _safe_unserialize($str)
{
    ...
    {
        throw new Exception(sprintf("Failed to unserialize input string. %s exceeds maximum of %s", strlen($str), getSerializeInputMaxLength()));
    }
...
function routePath($routeName, ...$routeVariables)
{
    try
    {
        ...
        $path = (string) call_user_func_array(array( $uriPathBuilder, "getPath" ), $routeVariables);
    }
    catch( Exception $e ) 
    {
