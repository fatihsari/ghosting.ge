...
{
    $result = select_query("tblinvoices", "id,invoicenum,userid,status", array( "id" => $invoiceid ));
    $data = mysql_fetch_array($result);
    ...
    {
        $euVATAddonCustomInvoiceNumbersEnabled = get_query_val("tbladdonmodules", "value", array( "module" => "eu_vat", "setting" => "enablecustominvoicenum" ));
    ...
    $result = select_query("tblinvoiceitems", "", array( "invoiceid" => $invoiceid, "type" => array( "sqltype" => "NEQ", "value" => "" ) ), "id", "ASC");
    while( $data = mysql_fetch_array($result) ) 
    {
        $userid = $data["userid"];
        $type = $data["type"];
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if( $type == "Addon" ) 
                            {
                                makeAddonPayment($relid, $invoiceid);
                            }
                            else
                            {
                                if( $type == "Upgrade" ) 
...
function makeHostingPayment($func_domainid, $invoiceID)
{
    ...
    if( $status != "Pending" ) 
    {
        $orderInvoice = WHMCS\Database\Capsule::table("tblhosting")->select("tblorders.invoiceid")->where("tblhosting.id", $func_domainid)->join("tblorders", "tblhosting.orderid", "=", "tblorders.id")->first();
...
function makeAddonPayment($func_addonid, $invoiceId)
{
    try
    {
                    ...
                    if( $addonId ) 
                    {
                        switch( $addon->productAddon->module ) 
                        {
                            case "":
                                break;
                            default:
                                if( !$addon->order || $addon->order->invoiceId != $invoiceId ) 
                                {
                                    $automation = WHMCS\Service\Automation\AddonAutomation::factory($addon);
                                    $automation->runAction("Renew");
                                    $noModule = false;
                                }
    ...
    foreach( $invoice->items as $item ) 
    {
        switch( $item->type ) 
        {
            ...
            case "Upgrade":
                ...
                break;
            case "AddFunds":
                WHMCS\Database\Capsule::table("tblcredit")->insert(array( "clientid" => $item->userId, "date" => Carbon\Carbon::now()->toDateString(), "description" => "Reversed Transaction ID: " . $originalTransactionId, "amount" => $transaction->amountIn * -1 ));
