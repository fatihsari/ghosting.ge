...
function showPaymentGatewaysList($disabledGateways = array(  ), $userId = NULL, $forceAll = false)
{
    ...
    while( $data = mysql_fetch_array($result) ) 
    {
        ...
        foreach($_LANG['ghosting']['gateways'] as $k => $v)
            $data["value"] = str_replace($k, $v, $data["value"]);
...
function logTransaction($gateway, $data, $result, array $passedParams = array(  ))
{
    global $params;
    if( !$params ) 
    {
...
function getRecurringBillingValues($invoiceId)
{
    ...
    while( $data = mysql_fetch_array($result) ) 
    {
        ...
        if( $CONFIG["TaxType"] == "Exclusive" && $taxed ) 
        {
            ...
            {
                $productAmount = $productAmount + format_as_currency($productAmount * $taxRate / 100) + format_as_currency($productAmount * $taxRate2 / 100);
            }
...
function getUpgradeRecurringValues($invoiceID)
{
    ...
    $currentServicePaidUntil = Carbon\Carbon::createFromFormat("Y-m-d", $dateUntil);
    $newServiceStartDate = Carbon\Carbon::createFromFormat("Y-m-d", $invoice->getData("duedate"));
...
{
    $serviceID = (int) $serviceID;
    $invoiceID = WHMCS\Database\Capsule::table("tblinvoiceitems")->join("tblinvoices", "tblinvoices.id", "=", "tblinvoiceitems.invoiceid")->join("tblhosting", "tblhosting.id", "=", "tblinvoiceitems.relid")->where("tblinvoiceitems.relid", "=", $serviceID)->where("tblinvoiceitems.type", "=", "Hosting")->where("tblinvoices.status", "=", "Unpaid")->where("tblinvoices.userid", "=", WHMCS\Database\Capsule::raw("`tblhosting`.`userid`"))->orderBy("tblinvoices.id")->value("tblinvoices.id");
    if( !$invoiceID ) 
    {
        $invoiceID = WHMCS\Database\Capsule::table("tblinvoiceitems")->join("tblinvoices", "tblinvoices.id", "=", "tblinvoiceitems.invoiceid")->join("tblhosting", "tblhosting.id", "=", "tblinvoiceitems.relid")->where("tblinvoiceitems.relid", "=", $serviceID)->where("tblinvoiceitems.type", "=", "Hosting")->where("tblinvoices.status", "=", "Paid")->where("tblinvoices.userid", "=", WHMCS\Database\Capsule::raw("`tblhosting`.`userid`"))->orderBy("tblinvoices.id", "desc")->value("tblinvoices.id");
    }

    if( !$invoiceID && !empty($transID) ) 
    {
        $invoiceID = WHMCS\Database\Capsule::table("tblinvoiceitems")->join("tblinvoices", "tblinvoices.id", "=", "tblinvoiceitems.invoiceid")->join("tblhosting", "tblhosting.id", "=", "tblinvoiceitems.relid")->where("tblhosting.subscriptionid", "=", $transID)->where("tblinvoiceitems.type", "=", "Hosting")->where("tblinvoices.status", "=", "Unpaid")->where("tblinvoices.userid", "=", WHMCS\Database\Capsule::raw("`tblhosting`.`userid`"))->orderBy("tblinvoices.id")->value("tblinvoices.id");
        if( !$invoiceID ) 
        {
            $invoiceID = WHMCS\Database\Capsule::table("tblinvoiceitems")->join("tblinvoices", "tblinvoices.id", "=", "tblinvoiceitems.invoiceid")->join("tblhosting", "tblhosting.id", "=", "tblinvoiceitems.relid")->where("tblhosting.subscriptionid", "=", $transID)->where("tblinvoiceitems.type", "=", "Hosting")->where("tblinvoices.status", "=", "Paid")->where("tblinvoices.userid", "=", WHMCS\Database\Capsule::raw("`tblhosting`.`userid`"))->orderBy("tblinvoices.id", "desc")->value("tblinvoices.id");
        }
