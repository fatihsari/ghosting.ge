...
function calcCartTotals($checkout = false, $ignorenoconfig = false)
{
    ...
    if( $CONFIG["TaxInclusiveDeduct"] && $CONFIG["TaxType"] == "Inclusive" && (!$taxrate && !$taxrate2 || $clientsdetails["taxexempt"]) ) 
    {
    ...
    if( is_array($cartProducts) ) 
    {
        ...
        foreach( $cartProducts as $key => $productdata ) 
        {
            ...
            if( $CONFIG["TaxEnabled"] && $tax && !$clientsdetails["taxexempt"] ) 
            {
                ...
                {
                    $taxCalculator->setTaxBase($product_total_qty_recurring);
                    $product_total_qty_recurring = $taxCalculator->getTotalAfterTaxes();
                }
                ...
                foreach( $addons as $addonid ) 
                {
                    ...
                    if( $checkout ) 
                    {
                        ...
                        for( $qtycount = 1; $qtycount <= $qty; $qtycount++ ) 
                        {
                            ...
                            $aid = insert_query("tblhostingaddons", array( "hostingid" => $serviceid, "addonid" => $addonid, "userid" => $userid, "orderid" => $orderid, "regdate" => "now()", "name" => "", "setupfee" => $addonsetupfee, "recurring" => $addon_recurring_db, "billingcycle" => $addon_billingcycle, "status" => "Pending", "nextduedate" => "now()", "nextinvoicedate" => "now()", "paymentmethod" => $paymentmethod, "tax" => $addon_tax ));
                            $orderaddonids[] = $aid;
                    ...
                    if( $addon_tax && !$clientsdetails["taxexempt"] ) 
                    {
                        ...
                        if( $CONFIG["TaxType"] == "Exclusive" ) 
                        {
                            $taxCalculator->setTaxBase($addon_recurring_db);
                            $addon_recurring_db = $taxCalculator->getTotalAfterTaxes();
                        }
    ...
    if( is_array($cartDomains) ) 
    {
        ...
        foreach( $cartDomains as $key => $domain ) 
        {
            ...
            if( !is_null($domain_renew_price_db) ) 
            {
                ...
                {
                    $taxCalculator->setTaxBase($domain_renew_price_db);
                    $recurring_cycles_total[$domain_billing_cycle] += $taxCalculator->getTotalAfterTaxes();
                }
                else
                {
            ...
            if( $checkout ) 
            {
                ...
                if( $domaintype == "register" ) 
                {
                    ...
                    $additflds->setFieldValues($domain["fields"]);
                    $additflds->saveToDatabase($domainid);
                }
    ...
    if( is_array($cartUpgrades) ) 
    {
        foreach( $cartUpgrades as $cartUpgrade ) 
        {
            ...
            try
            {
                ...
                {
                    if( $cartUpgrade["upgrade_entity_type"] == "addon" ) 
                    {
            ...
            if( $checkout ) 
            {
                ...
                $upgrade->save();
                $invoiceDescription = Lang::trans("upgradedowngradepackage") . ": ";
    ...
    if( is_array($cartRenewals) ) 
    {
        ...
        foreach( $cartRenewals as $domainid => $regperiod ) 
        {
            ...
            $sld = $domainparts[0];
            $tld = "." . $domainparts[1];
            $isPremium = (bool) (int) $data["is_premium"];
            ...
            if( $isPremium ) 
            {
                ...
                if( $extraDetails ) 
                {
                    $renewprice = (double) $extraDetails->value;
                    $regperiod = 1;
                }
            ...
            $cartdata["renewals"][$domainid] = array( "domain" => $domainname, "regperiod" => $regperiod, "price" => new WHMCS\View\Formatter\Price($renewprice, $currency), "dnsmanagement" => $dnsmanagement, "emailforwarding" => $emailforwarding, "idprotection" => $idprotection );
        }
    }
    ...
    if( $checkout ) 
    {
        ...
        update_query("tblorders", array( "amount" => $cart_total, "nameservers" => $ordernameservers, "transfersecret" => $domaineppcodes, "renewals" => substr($orderrenewals, 0, -1), "orderdata" => serialize($orderdata) ), array( "id" => $orderid ));
        $invoiceid = 0;
        if( !$_SESSION["cart"]["geninvoicedisabled"] ) 
        {
            if( !$userid ) 
        ...
        {
            foreach($_LANG['ghosting']['options'] as $k=>$v)
            {
                $adminemailitems = str_replace($k, $v, $adminemailitems);
            }
...
function acceptOrder($orderid, $vars = array(  ))
{
    ...
    foreach( $addons as $addon ) 
    {
        ...
        if( $addonId && $addon->productAddon->autoActivate == "on" ) 
        {
            $automationResult = "";
            $noModule = true;
            ...
            {
                $sendWelcomeEmail = ($addon->productAddon->welcomeEmailTemplateId ? true : false);
                if( count($vars) ) 
                {
                    $sendWelcomeEmail = $vars["addons"][$addonUniqueId]["sendwelcome"];
                }

                if( array_key_exists("api", $vars) && array_key_exists("sendemail", $vars["api"]) ) 
                {
...
function getAddons($pid, array $addons = array(  ))
{
    ...
    $orderAddons = WHMCS\Product\Addon::showOnOrderForm()->get();
...
function getAvailableOrderPaymentGateways($forceAll = false)
{
    ...
    foreach( $gatewaysList as $module => $values ) 
    {
        if( $values["type"] == "CC" || $values["type"] == "OfflineCC" ) 
        {
            ...
            if( file_exists($gatewayPath) && !function_exists($module . "_config") && !function_exists($module . "_link") && !function_exists($module . "_capture") ) 
            {
...
function canOrderBeDeleted($orderID)
{
    ...
    $orderID = (int) $orderID;
    $orderStatus = get_query_val("tblorders", "status", array( "id" => $orderID ));
    $isCancelled = get_query_val("tblorderstatuses", "showcancelled", array( "title" => $orderStatus ));
    if( $isCancelled == 1 || $orderStatus == "Fraud" ) 
    {
        return true;
    }
...
function processAddonsCancelOrFraud($addonCollection, $status)
{
    foreach( $addonCollection as $addon ) 
    {
        ...
        {
            logActivity("Running Module Terminate on Order Cancel - Addon ID: " . $addonId);
