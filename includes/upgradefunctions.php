...
function SumUpPackageUpgradeOrder($id, $newproductid, $newproductbillingcycle, $promocode, $paymentmethod = "", $checkout = "")
{
    ...
    if( $stockControlEnabled && $quantityInStock <= 0 ) 
    {
    ...
    else
    {
        ...
        if( $oldamount / $oldCycleMonths == $newamount / $newCycleMonths ) 
        {
            $newamount = $oldamount;
        }
...
function SumUpConfigOptionsOrder($id, $configoptions, $promocode, $paymentmethod = "", $checkout = "")
{
    ...
    $nextduedate = $data["nextduedate"];
    $billingcycle = $data["billingcycle"];
    $productInfo = WHMCS\Database\Capsule::table("tblproducts")->find($packageid, array( "tax", "name" ));
...
function createUpgradeOrder($serviceId, $ordernotes, $promocode, $paymentmethod)
{
    ...
    {
        $orderdescription = str_replace($k, $v, $orderdescription);
    }
...
function doUpgrade($upgradeid)
{
    ...
    else
    {
        ...
        else
        {
            ...
            migrateCustomFieldsBetweenProducts($relid, $upgrade->newValue);
            cancelUnpaidInvoiceForPreviousPriceAndRegenerateNewInvoiceByServiceId($relid);
            if( $upgrade->type == "service" ) 
            {
                run_hook("AfterProductUpgrade", array( "upgradeid" => $upgradeid ));
    ...
    if( $type == "package" || $type == "configoptions" ) 
    {
        ...
        if( $servertype ) 
        {
            ...
            else
            {
                ...
                if( $upgradeemail ) 
                {
                    $upgradeEmailTemplate = WHMCS\Mail\Template::find($upgradeemail);
        ...
        {
            $emailVars = array( "client_id" => $userid, "service_id" => $relid, "order_id" => $orderid, "upgrade_id" => $upgradeid, "upgrade_type" => $type, "upgrade_amount" => $upgradeamount, "increase_recurring_value" => $recurringchange, "promomotion" => $promocode, "package_id" => $pid, "server_type" => $servertype );
            if( $type == "package" ) 
            {
            ...
            logActivity("Automatic Product/Service Upgrade not possible - Service ID: " . $relid, $userid);
            WHMCS\Database\Capsule::table("tbltodolist")->insert(array( "date" => date("Y-m-d"), "title" => "Manual Upgrade Required", "description" => "Manual Upgrade Required for Service ID: " . $relid, "admin" => "", "status" => "Pending", "duedate" => date("Y-m-d") ));
        }
