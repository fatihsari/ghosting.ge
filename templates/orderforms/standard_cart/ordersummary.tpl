{if $producttotals}
    {if empty($producttotals.configoptions)}
    <div class="clearfix">
    ...
    {foreach $producttotals.configoptions as $configoption}
        {if $configoption}
            <div class="clearfix">
                ...
                <span class="pull-right">{if preg_match("/\d/", $configoption.recurring)}{$configoption.recurring}{else}{$configoption.recurring|replace:' ':'0.00 '}{/if}{if $configoption.setup} + {$configoption.setup} {$LANG.ordersetupfee}{/if}</span>
            </div>
        {/if}
    {/foreach}
