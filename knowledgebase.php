    ...
    else
    {
        ...
        else
        {
            if( $action == "displayarticle" ) 
            {
                ...
                $article_text = preg_replace("/!![^!!]+!!/", "", $article);
                $keywords = array();
                if(preg_match("/!!([^!!]+)!!/", $article, $m))
                {
                    $ks = explode(",", $m[1]);
                    foreach($ks as $k)
                    {
                ...
                $smartyvalues["kbarticle"] = array( "id" => $id, "categoryid" => $catid, "categoryname" => $catname, "title" => $title, "urlfriendlytitle" => getModRewriteFriendlyString($title), "text" => $article_text, 'keywords' => $keywords, "views" => $views, "useful" => $useful, "votes" => $votes, "voted" => $vote );
                $catlist = "";
