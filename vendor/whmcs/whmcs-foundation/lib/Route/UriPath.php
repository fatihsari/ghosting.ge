...
class UriPath implements Contracts\MapInterface
{
    ...
    {
        $routePath = $this->getRawPath($routeName, $routeVars);
        $mode = $this->getMode();
        $base = $this->baseUri;
        if( substr($base, -1) == "/" ) 
        {
        ...
        if( defined("ADMINAREA") ) 
        {
            ...
            if( strpos($routePath, $adminBase) === 0 && $mode !== self::MODE_REWRITE && substr($base, -1 * strlen($adminBase)) != $adminBase ) 
            {
                $base .= $adminBase;
            }
        ...
        $interpolatePlaceHolder = function() use (&$tmpRouteVars)
{
    if( count($tmpRouteVars) ) 
    {
        ...
        {
            return getModRewriteFriendlyString($value);
        }
