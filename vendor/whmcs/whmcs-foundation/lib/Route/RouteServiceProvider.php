...
{
    return new \Middlewares\Utils\Dispatcher(array( new Middleware\BackendPsr7Response(), new \WHMCS\Admin\ApplicationSupport\Route\Middleware\DirectoryValidation(), $container->make("Route\\Authentication"), $container->make("Route\\Authorization"), new Middleware\HandleProcessor() ));
}

);
        $container->singleton("Backend\\Dispatcher\\Client", function() use ($container)
{
    return new \Middlewares\Utils\Dispatcher(array( new Middleware\HandleProcessor() ));
}
 ...
 ), array( "name" => "route-not-defined", "path" => "/route-not-defined", "method" => array( "GET", "POST" ), "handle" => function(\Psr\Http\Message\ServerRequestInterface $request)
{
 ...
 ), array( "name" => "clientarea-homepage", "method" => array( "GET", "POST" ), "path" => "/", "handle" => array( "\\WHMCS\\ClientArea\\ClientAreaController", "homePage" ) ), array( "name" => "clientarea-index", "method" => array( "GET", "POST" ), "path" => "/index.php", "handle" => array( "\\WHMCS\\ClientArea\\ClientAreaController", "homePage" ) ), array( "name" => "clientarea-ka", "method" => array( "GET", "POST" ), "path" => "/ka", "handle" => array( "\\WHMCS\\ClientArea\\ClientAreaController", "homePage" ) ), array( "name" => "clientarea-en", "method" => array( "GET", "POST" ), "path" => "/en", "handle" => array( "\\WHMCS\\ClientArea\\ClientAreaController", "homePage" ) ), array( "name" => "clientarea-ru", "method" => array( "GET", "POST" ), "path" => "/ru", "handle" => array( "\\WHMCS\\ClientArea\\ClientAreaController", "homePage" ) ) ) );
    }
