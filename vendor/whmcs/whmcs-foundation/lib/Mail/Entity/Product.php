...
class Product extends \WHMCS\Mail\Emailer
{
    protected function getEntitySpecificMergeData($serviceId)
    {
        ...
        $container_data = \WHMCS\Database\Capsule::table('tblcontainers')->where('service_id', $serviceId)->first();
        if($container_data)
        {
            $email_merge_fields["container_id"] = substr($container_data->container_id, 0, 12);
            $email_merge_fields["ssh_port"] = $container_data->ssh_port;
            $email_merge_fields["sftp_port"] = $container_data->sftp_port;
            $email_merge_fields["rmysql_port"] = $container_data->rmysql_port;
        }
        else
        {
            $email_merge_fields["container_id"] = "";
            $email_merge_fields["ssh_port"] = "";
            $email_merge_fields["sftp_port"] = "";
            $email_merge_fields["rmysql_port"] = "";
        }
