...
foreach($_LANG['ghosting']['gateways'] as $k1 => $v1)
{
    $outputvars['paymentmethod'] = str_replace($k1, $v1, $outputvars['paymentmethod']);
}
...
if( $whmcs->get_config("AllowCustomerChangeInvoiceGateway") ) 
{
    ...
    $frm = new WHMCS\Form();
    $availablegateways_x = array();
    foreach($availablegateways as $k => $v)
    {
        $val = $v;
        foreach($_LANG['ghosting']['gateways'] as $k1 => $v1)
        {
            $val = str_replace($k1, $v1, $val);
        }
        $availablegateways_x[$k] = $val;
    }
    $gatewaydropdown = generate_token("form") . $frm->dropdown("gateway", $availablegateways_x, $invoice->getData("paymentmodule"), "submit()");
