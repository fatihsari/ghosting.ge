...
define("SHARED_HOSTING_IP", "35.198.99.84");
define("SHARED_HOSTING_KEY", "MTIwMTM1YzczNTY4MjM3YWE1MTJmYjEzZmRhNTk3YWYyYzQ1MGNmZmE5NTQ3MTMy");

define('TBC_CERT', __DIR__.'/ssl/tbc_crt.pem');
define('TBC_CERT_KEY', __DIR__.'/ssl/tbc_key.pem');
define('TBC_CERT_PASSWORD', 'fySRuomAcq4YBpjK');
...
try
{
    ...
    if( !defined("WHMCSLIVECHAT") ) 
    {
        if( defined("CLIENTAREA") ) 
        {
            if(isset($_GET['language']) && ($_GET['language'] === 'georgian' || $_GET['language'] === 'english' || $_GET['language'] === 'russian'))
                $_SESSION['Language'] = $_GET['language'];
            if(isset($_SESSION["uid"]) && intval($_SESSION["uid"]) > 0)
                WHMCS\Database\Capsule::table('tblclients')->where('id', $_SESSION["uid"])->where('language', '!=', $_SESSION['Language'])->update(['language' => $_SESSION['Language']]);
            DI::make("lang");
...
if( $licensing->getVersionHash() != "5cc7720f613926993107a6123a28eeaad706fc6a42324b40a3764748a28322280f5c373047bb2efad6f350a3bf070ad7ea0814abc720a6fb284fa6df4abea02e" ) 
{
    throw new WHMCS\Exception\Fatal("License Checking Error");
}
