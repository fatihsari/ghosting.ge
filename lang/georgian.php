<?php
/**
 * WHMCS Language File
 * Georgian (ka)
 *
 * Please Note: These language files are overwritten during software updates
 * and therefore editing of these files directly is not advised. Instead we
 * recommend that you use overrides to customise the text displayed in a way
 * which will be safely preserved through the upgrade process.
 *
 * For instructions on overrides, please visit:
 *   http://docs.whmcs.com/Language_Overrides
 *
 * @package    WHMCS
 * @author     WHMCS Limited <development@whmcs.com>
 * @copyright  Copyright (c) WHMCS Limited 2005-2015
 * @license    http://www.whmcs.com/license/ WHMCS Eula
 * @version    $Id$
 * @link       http://www.whmcs.com/
 */

if (!defined("WHMCS")) die("This file cannot be accessed directly");

$_LANG['locale'] = "ka_GE";

$_LANG['accountinfo'] = "ანგარიშის ინფორმაცია";
$_LANG['accountstats'] = "ანგარიშის სტატისტიკა";
$_LANG['addfunds'] = "შევსება";
$_LANG['addfundsamount'] = "თანხა";
$_LANG['addfundsmaximum'] = "მაქს. თანხა";
$_LANG['addfundsmaximumbalance'] = "მაქს. ბალანსი";
$_LANG['addfundsmaximumbalanceerror'] = "ბალანსის მაქს. რაოდენობა არის";
$_LANG['addfundsmaximumerror'] = "დეპოზიტის მაქს. რაოდენობა არის";
$_LANG['addfundsminimum'] = "მინ. დეპოზიტი";
$_LANG['addfundsminimumerror'] = "დეპოზიტის მინ. რაოდენობა";
$_LANG['addmore'] = "დამატება";
$_LANG['addtocart'] = "კალათაში დამატება";
$_LANG['affiliatesactivate'] = "პარტნიორული პროგ. აქტივაცია";
$_LANG['affiliatesamount'] = "რაოდენობა";
$_LANG['affiliatesbalance'] = "მიმდინარე ბალანსი";
$_LANG['affiliatesbullet1'] = "მიიღეთ საწყისი ბონუსი თქვენს პარტნიორის ანგარიშზე";
$_LANG['affiliatesbullet2'] = "of every payment each customer you refer to us makes for the entire duration of their hosting account";
$_LANG['affiliatescommission'] = "დარიცხული თანხა";
$_LANG['affiliatesdescription'] = "ჩაერთე პარტნიორულ პროგრამაში ან იხილე მონაგარი";
$_LANG['affiliatesdisabled'] = "ამ დროისთვის პარტნიორული პროგრამა შეჩერებულია.";
$_LANG['affiliatesearn'] = "გამომუშავება";
$_LANG['affiliatesearningstodate'] = "გამომუშავებული თანხა შემდეგი თარიღისთვის";
$_LANG['affiliatesfootertext'] = "როდესაც თქვენი რეფერალ ლინკით ვინმეს ჩვენს საიტზე გადმოამისამართებთ, ეს ინფორმაცია კომპიუტერში 'cookies' სახით შეინახება და თუკი ეს ადამიანი გარკვეული დროის გასვლის შემდეგ იყიდის მომსახურეობას, თქვენ საკომისიოს მაინც დაგერიცხებათ";
$_LANG['affiliateshostingpackage'] = "ჰოსტინგის პაკეტი";
$_LANG['affiliatesintrotext'] = "დღესვე გააქტიურეთ პარტნიორული პროგრამა:";
$_LANG['affiliateslinktous'] = "ჩვენი ბმული";
$_LANG['affiliatesnosignups'] = "ამჟამად თქვენ არ გყავთ არცერთი რეფერალი";
$_LANG['affiliatesrealtime'] = "სტატისტიკა მოცემულია რეალურ რეჟიმში, რომელიც ყოველ წამს ახლდება";
$_LANG['affiliatesreferallink'] = "უნიკალური რეფერალური ბმული";
$_LANG['affiliatesreferals'] = "რეფერალები";
$_LANG['affiliatesregdate'] = "რეგისტრაციის დრო";
$_LANG['affiliatesrequestwithdrawal'] = "განაღდების მოთხოვნა";
$_LANG['affiliatessignupdate'] = "რეგისტრაციის თარიღი";
$_LANG['affiliatesstatus'] = "სტატუსი";
$_LANG['affiliatestitle'] = "პარტნ. პროგრამა";
$_LANG['affiliatesvisitorsreferred'] = "გადმომისამართებულ ვიზიტორთა რაოდენობა";
$_LANG['affiliateswithdrawalrequestsuccessful'] = "თანხის განაღდების მოთხოვნა დადასტურდა. მოკლე ხანში დაგიკავშირდებიან.";
$_LANG['affiliateswithdrawn'] = "განაღდებული თანხის რაოდენობა";
$_LANG['all'] = "ყველა";
$_LANG['alreadyregistered'] = "ხართ რეგისტრირებული?";
$_LANG['announcementsdescription'] = "ნახეთ სიახლეები და ჩვენი განაცხედები";
$_LANG['announcementsnone'] = "განაცხადები არ გვაქვს";

$_LANG['announcementsrss'] = "იხილეთ RSS";
$_LANG['announcementstitle'] = "სიახლეები";

$_LANG['announcementsrss'] = "იხილეთ RSS";
$_LANG['announcementstitle'] = "ახალი ამბები";

$_LANG['bannedbanexpires'] = "ბანის ხანგრძლიობა";
$_LANG['bannedbanreason'] = "ბანის მიზეზი";
$_LANG['bannedhasbeenbanned'] = "ბლოკირებულია";
$_LANG['bannedtitle'] = "IP ბლოკირებულია";
$_LANG['bannedyourip'] = "თქვენი IP";
$_LANG['cartaddons'] = "დანამატები";
$_LANG['cartbrowse'] = "მოიძიეთ პროდუქტები და სერვისები";
$_LANG['cartconfigdomainextras'] = "დომენის დანამატების კონფიგურაცია";
$_LANG['cartconfigoptionsdesc'] = "ამ პროდუქტს/სერვისს გააჩნია გარკვეული პარამეტრები, რომლებიც შეგიძლიათ ქვევით სურვილისამებრ აირჩიოთ.";
$_LANG['cartconfigserver'] = "სერვერის კონფიგურაცია";
$_LANG['cartcustomfieldsdesc'] = "მოცემული პროდუქტით/სერვისით სარგებლობისთვის თქვენგან დამატებითი ინფორმაცია მოითხოვება.";
$_LANG['cartdomainsconfig'] = "დომეინების კონფიგურაცია";
$_LANG['cartdomainsconfigdesc'] = "Below you can configure the domain names in your shopping cart selecting the addon services you would like, providing required information for them and defining the nameservers that they will use.";

$_LANG['cartdomainshashosting'] = "აქვს ჰოსტინგი";
$_LANG['cartdomainsnohosting'] = "არააქვს ჰოსტინგი! დააჭირეთ დასამატებლად";
$_LANG['carteditproductconfig'] = "კონფიგურაციის რედაქტირება";
$_LANG['cartempty'] = "თქვენი კალათა ცარიელია.";
$_LANG['cartemptyconfirm'] = "ნამდვილად გსურთ კალათის გასუფთავება?";
$_LANG['cartexistingclientlogin'] = "არსებული მომხმარებლის ";

$_LANG['cartdomainshashosting'] = "აქვს ჰოსტინიგი";
$_LANG['cartdomainsnohosting'] = "ჰოსტინგის გარეშე! დამატება";
$_LANG['carteditproductconfig'] = "კონფიგურაციის შეცვლა";
$_LANG['cartempty'] = "თქვენი კალათა ცარიელია.";
$_LANG['cartemptyconfirm'] = "ნამდვილად გსურთ კალათის გასუფთავება?";
$_LANG['cartexistingclientlogin'] = "არსებული მომხმარებელი";

$_LANG['cartexistingclientlogindesc'] = "ამ შეკვეთის არსებულ ანგარიშზე მისაბმელად, საჭიროა გაიაროთ ავტორიზაცია.";
$_LANG['cartnameserversdesc'] = "თუკი გსურთ გამოიყენოთ საკუთარი NameServer - მისამართი ქვემოთ შეიყვანეთ. ჩვეულებრივ, ახალ დომეინებს ავტომატურად ჩვენი NameServer ენიჭებათ.";
$_LANG['cartproductaddons'] = "დამატებითი პროდუქტი";
$_LANG['cartproductaddonschoosepackage'] = "პაკეტის არჩევა";
$_LANG['cartproductaddonsnone'] = "თქვენი სერვისისთვის დანამატები არ არის";
$_LANG['cartproductaddonsnone'] = "თქვენ მიერ არჩეული პრუდქტისთვის/სერვისისთვის დამატებით პროდუქტები ხელმიუწვდომელია";
$_LANG['cartproductconfig'] = "პროდუქტების კონფიგურაცია";
$_LANG['cartproductdesc'] = " არჩეულ პროდუქტს/სერვისს გააჩნია შემდეგი პარამეტრები, რომლებიც სურვილისამებრ შეგიძლიათ მოაწყოთ";
$_LANG['cartproductdomain'] = "დომეინები";
$_LANG['cartproductdomainchoose'] = "დომეინის არჩევა";
$_LANG['cartproductdomaindesc'] = "არჩეული პროდუქტი/სერვისი საჭიროებს დომეინს, ამიტომაც მიუთითეთ თქვენი დომეინი.";
$_LANG['cartproductdomainuseincart'] = "კალათაში არსებული დომეინის გამოყენება.";
$_LANG['cartremove'] = "მოხსნა";
$_LANG['cartremoveitemconfirm'] = "ნამდვილად გსურთ ამ სერვისის კალათიდან ამოგდება?";
$_LANG['carttaxupdateselections'] = "გადასახადი შესაძლოა შეიცვალოს ქვეყნების მიხედვით. არჩევის შემდეგ დააჭირეთ, რათა ხელახლა გამოთვალოთ გადასახადები. ";
$_LANG['carttaxupdateselectionsupdate'] = "განახლება";
$_LANG['carttitle'] = "კალათა";
$_LANG['changessavedsuccessfully'] = "ცვლილები წარმატებით შეინახა!";
$_LANG['checkavailability'] = "ხელმისაწვდომობის შემოწმება";
$_LANG['checkout'] = "შეკვეთა";
$_LANG['choosecurrency'] = "ვალუტის არჩევა";
$_LANG['choosedomains'] = "დომეინის არჩევა";
$_LANG['clickheretologin'] = "შესვლისთვის აქ დააჭირეთ";
$_LANG['clientareaaccountaddons'] = "ანგარიშის დანამატები";
$_LANG['clientareaactive'] = "აქტიური";
$_LANG['clientareaaddfundsdisabled'] = "ამჟამად ჩვენთან დეპოზიტის შევსება არ არის ნებადართული.";
$_LANG['clientareaaddfundsnotallowed'] = "თქვენ უნდა გქონდეთ სულ მცირე ერთი აქტიური სერვისი, რათა შეძლოთ ანგარიშის შევსება!";
$_LANG['clientareaaddon'] = "დანამატი";
$_LANG['clientareaaddonorderconfirmation'] = "გმადლობთ. თქვენი შეკვეთა განთავსებულია. ახლა კი გადახდის მეთოდი აირჩიეთ.";
$_LANG['clientareaaddonpricing'] = "ღირებულება";
$_LANG['clientareaaddonsfor'] = "დანამატები";
$_LANG['clientareaaddress1'] = "მისამართი 1";
$_LANG['clientareaaddress2'] = "მისამართი 2";
$_LANG['clientareabwlimit'] = "ტრაფიკის ლიმიტი";

$_LANG['clientareabwusage'] = "გამოყენებული ტრაფიკი";
$_LANG['clientareabwusage'] = "მოხმარებული ტრაფიკი";
$_LANG['clientareacancel'] = "ცვლილებების გაუქმება";

$_LANG['clientareabwusage'] = "გამოყენებული ტრაფიკი";
$_LANG['clientareacancel'] = "ცვლილებების გაუქმება";

$_LANG['clientareacancelconfirmation'] = "გმადლობთ. გაუქმების მოთხოვნა დადასტურდა. თუ თქვენ შემთხვევით გააუქმეთ მომსახურეობა, გთხოვთ დაგვიკავშირდით, რათა თქვენი მონაცემები არ წაიშალოს. .";
$_LANG['clientareacancelinvalid'] = "ამ ანგარიშის გაუქმების მოთხოვნა უკვე მიღებულია, ამიტომაც ხელმეორედ ვერ მოითხოვთ.";
$_LANG['clientareacancellationimmediate'] = "მყისიერი";
$_LANG['clientareacancellationtype'] = "გაუქმების ტიპი";
$_LANG['clientareacancelled'] = "გაუქმებული";
$_LANG['clientareacancelproduct'] = "გაუქმების მოთხოვნა";
$_LANG['clientareacancelreason'] = "მოთხოვნის მიზეზის მოკლე აღწერა";
$_LANG['clientareacancelrequest'] = "ანგარიშის გაუქმების მოთხოვნა";
$_LANG['clientareacancelrequestbutton'] = "გაუქმების მოთხოვნა";
$_LANG['clientareachangepassword'] = "შეცვალეთ თქვენი პაროლი";

$_LANG['clientareachangesuccessful'] = "თქვენი მონაცემები შეიცვალა";

$_LANG['clientareachangesuccessful'] = "თქვენი მონაცემები წარმატებით შეიცვალა";

$_LANG['clientareachoosecontact'] = "აირჩიეთ კონტაქტი";
$_LANG['clientareachoosecontact'] = "კონტაქტის არჩევა";

$_LANG['clientareachoosecontact'] = "აირჩიეთ კონტაქტი";

$_LANG['clientareacity'] = "ქალაქი";
$_LANG['clientareacompanyname'] = "კომპანიის სახელწოდება";
$_LANG['clientareaconfirmpassword'] = "პაროლის დადასტურება";
$_LANG['clientareacontactsemails'] = "ელ.ფოსტის მოწყობა";
$_LANG['clientareacontactsemailsdomain'] = "დომეინები - გახანგრძლივების შეტყობინებები, რეგისტრაციის დასტური და ა.შ...";
$_LANG['clientareacontactsemailsgeneral'] = "საერთო - ძირითადი შეტყობინებები და პაროლების შემხსენებელი.";
$_LANG['clientareacontactsemailsinvoice'] = "გადახდები - ინვოისები და საგადახდო შეხსენებები";
$_LANG['clientareacontactsemailsproduct'] = "პროდუქტები - შეკვეთის დეტალები, მისასალმებელი წერილები და ა.შ...";
$_LANG['clientareacontactsemailssupport'] = "მხარდაჭერა - მოცემული მომხარებლისთვის, თქვენს ანგარიშზე ბილეთის გახსნის უფლების მინიჭება.";
$_LANG['clientareacountry'] = "ქვეყანა";
$_LANG['clientareacurrentsecurityanswer'] = "შეიყვანეთ არსებული პასუხი";
$_LANG['clientareacurrentsecurityquestion'] = "აირჩიეთ კითხვა უსაფრთხოებისათვის";
$_LANG['clientareadeletecontact'] = "კონტაქტის წაშლა";
$_LANG['clientareadeletecontactareyousure'] = "ნამდვილად გსურთ ამ კონტაქტის წაშლა?";
$_LANG['clientareadescription'] = "იხილეთ და განახლეთ თქვენი ანგარიშის მონაცემები";
$_LANG['clientareadisklimit'] = "დისკის ლიმიტი";
$_LANG['clientareadiskusage'] = "გამოყენებული დისკური სივრცე";
$_LANG['clientareadomainexpirydate'] = "გაუქმების თარიღი";
$_LANG['clientareadomainnone'] = "დომენები არაა რეგისტრირებული";
$_LANG['clientareaemail'] = "ელ. ფოსტა";
$_LANG['clientareaemails'] = "ჩემი წერილები";
$_LANG['clientareaemailsdate'] = "გაგზავნის დრო";
$_LANG['clientareaemailsintrotext'] = "ქვემოთ მოყვანილია ჩვენ მიერ გაგზავნილი შეტყობინებების ისტორია. ეს გეხმარებათ თავიდანვე იხილოთ ყველა თქვენს ანგარიშზე გამოგზავნილი ინფორმაცია მათი დაკარგვისას.";
$_LANG['clientareaemailssubject'] = "შეტყობინების თემა";
$_LANG['clientareaerroraddress1'] = "თქვენ არ შეგიყვანიათ მისამართი (ველი 1)";
$_LANG['clientareaerroraddress12'] = "თქვენი მისამართე შეიძლება შეიცავდეს მხოლოდ ციფრებსა და ასოებს";
$_LANG['clientareaerrorbannedemail'] = "მოცემულ ელ.ფოსტის პროვაიდერის მომხმარებელს არ ვრთავთ ნებას ისარგებლონ ჩვენი სერვისით. გთხოვთ, შეცვალოთ ელ.ფოსტა.";
$_LANG['clientareaerrorcity'] = "არ შეგიყვანიათ ქალაქის დასახელება";
$_LANG['clientareaerrorcity2'] = "ქალაქის დასახელება უნდა შეიცავდეს მხოლოდ ასოებს.";
$_LANG['clientareaerrorcountry'] = "აირჩიეთ თქვენი ქვეყანა ჩამოსაშლელი მენიუდან";
$_LANG['clientareaerroremail'] = "არ შეგიყვანიათ ელ.ფოსტის მისამართი";
$_LANG['clientareaerroremailinvalid'] = "თქვენი ელ.ფოსტა არასწორია";
$_LANG['clientareaerrorfirstname'] = "არ შეგიყვანიათ თქვენი სახელი";
$_LANG['clientareaerrorfirstname2'] = "თქვენი სახელი უნდა შეიცავდეს მხოლოდ ასოებს";
$_LANG['clientareaerrorisrequired'] = "მოითხოვება";
$_LANG['clientareaerrorlastname'] = "არ შეგიყვანიათ მხოლოდ თქვენი გვარი";
$_LANG['clientareaerrorlastname2'] = "თქვენი გვარი უნდა შეიცავდეს მხოლოდ ასოებს";
$_LANG['clientareaerroroccured'] = "დაფიქსირდა შეცდომა, სცადეთ მოგვიანებით.";
$_LANG['clientareaerrorpasswordconfirm'] = "პაროლი არ დაგიდასტურებიათ";
$_LANG['clientareaerrorpasswordnotmatch'] = "შეყვანილი პაროლები ერთმანეთს არ ემთხვევა";
$_LANG['clientareaerrorphonenumber'] = "არ შეგიყვანიათ ტელეფონის ნომერი";
$_LANG['clientareaerrorphonenumber2'] = "ტელეფონის ნომერი არასწორია";
$_LANG['clientareaerrorpostcode'] = "არ შეგიყვანიათ საფოსტო ინდექსი";
$_LANG['clientareaerrorpostcode2'] = "საფოსტო მისამართ უნდა შეიცავდეს მხოლოდ ციფრებს ან ასოებს.";
$_LANG['clientareaerrors'] = "დაფიქსირდა შემდეგი შეცდომები:";
$_LANG['clientareaerrorstate'] = "არ მიგითითებიათ თქვენი რეგიონი";
$_LANG['clientareaexpired'] = "დროგასული";
$_LANG['clientareafirstname'] = "სახელი";
$_LANG['clientareafraud'] = "თაღლითობა";
$_LANG['clientareafullname'] = "მომხმარებლის სახელი";
$_LANG['clientareaheader'] = "მოგესალმებით, თქვენ მოხვდით მომხმარებლის არეში, საიდანაც შეძლებთ ანგარიშის მართვას. აქ, ინახება თქვენი საკონტაქტო ინფორმაცია, ინფორმაცია თქვენს ჰოსტინგ-პაკეტებზე და დომეინებზე. ასევე, შეგიძლიათ მიმართოთ  ტექ.დახმარების გუნდს ან შეუკვეთოთ დამატებით მომსახურეობები. ყურადღება მიაქციეთ საკონტაქტო დეტალების სწორად შევსებას.";
$_LANG['clientareahostingaddons'] = "დანამატები";
$_LANG['clientareahostingaddonsintro'] = "თქვენ გაქვთ შემდეგი დამანამტები ამ პროდუქტისთვის.";
$_LANG['clientareahostingaddonsview'] = "ნახვა";
$_LANG['clientareahostingamount'] = "რაოდენობა";
$_LANG['clientareahostingdomain'] = "დომეინი";
$_LANG['clientareahostingnextduedate'] = "გადახდის თარიღი";
$_LANG['clientareahostingpackage'] = "პაკეტი";
$_LANG['clientareahostingregdate'] = "რეგისტრაციის თარიღი";
$_LANG['clientarealastname'] = "გვარი";
$_LANG['clientarealastupdated'] = "ბოლოს განახლდა";
$_LANG['clientarealeaveblank'] = "თუკი არ გსურთ პაროლის შეცვლა - დატოვეთ ცარიელი.";
$_LANG['clientareamodifydomaincontactinfo'] = "დომეინის საკონტაქტო ინფორმაციის შეცვლა";
$_LANG['clientareamodifynameservers'] = "NameServer-ების ცვლილება";
$_LANG['clientareamodifynameservers'] = "Nameserver-ების ცვლილება";
$_LANG['clientareamodifywhoisinfo'] = "WHOIS საკონტაქტო ინფორმაციის შეცვლა";
$_LANG['clientareanameserver'] = "NameServer";

$_LANG['clientareanavaddcontact'] = "დაამატეთ ახალი კონტაქტი";

$_LANG['clientareanavaddcontact'] = "დაამატეთ ახალი კონტაქტი";

$_LANG['clientareanavchangecc'] = "შეცვალეთ საკრედიტო ბარათის დეტალები";
$_LANG['clientareanavchangepw'] = "პაროლის შეცვლა";
$_LANG['clientareanavdetails'] = "ჩემი მონაცემები";
$_LANG['clientareanavdomains'] = "ჩემი დომეინები";
$_LANG['clientareanavhome'] = "მთავარი";
$_LANG['clientareanavlogout'] = "გამოსვლა";
$_LANG['clientareanavorder'] = "შესყიდვების გაგრძელება";
$_LANG['clientareanavsecurityquestions'] = "საიდუმლო კითხვის შეცვლა";
$_LANG['clientareanavservices'] = "ჩემი სერვისები";
$_LANG['clientareanavsupporttickets'] = "დახმარების ბილეთები";

$_LANG['clientareanocontacts'] = "კონტაქტები არ მოიძებნა";
$_LANG['clientareanocontacts'] = "კონტაქტები არაა ნაპოვნი";

$_LANG['clientareanocontacts'] = "კონტაქტები არ მოიძებნა";

$_LANG['clientareapassword'] = "პაროლი";
$_LANG['clientareapending'] = "დაუდასტურებელი";
$_LANG['clientareapendingtransfer'] = "მიმდინარე ტრანსფერი";
$_LANG['clientareaphonenumber'] = "ტელ. ნომერი";
$_LANG['clientareapostcode'] = "საფოსტო ინდექსი";
$_LANG['clientareaproductdetails'] = "პროდუქტის მონაცემები";
$_LANG['clientareaproducts'] = "ჩემი პროდუქტები &amp; სერვისები";
$_LANG['clientareaproductsnone'] = "არცერთი პროდუქტი/სერვისი არ დაგიკვეთიათ";
$_LANG['clientarearegistrationperiod'] = "რეგისტრაციის დრო";
$_LANG['clientareasavechanges'] = "ცვლილებების შენახვა";
$_LANG['clientareasecurityanswer'] = "მიუთითეთ პასუხი";
$_LANG['clientareasecurityconfanswer'] = "დაადასტურეთ პასუხი";
$_LANG['clientareasecurityquestion'] = "აირჩიეთ საიდუმლო კითხვა";
$_LANG['clientareaselectcountry'] = "აირჩიეთ ქვეყანა";
$_LANG['clientareasetlocking'] = "დაბლოკვა";
$_LANG['clientareastate'] = "შტატი/რეგიონი";
$_LANG['clientareastatus'] = "სტატუსი";
$_LANG['clientareasuspended'] = "შეჩერებული";
$_LANG['clientareaterminated'] = "წაშლილი";
$_LANG['clientareaticktoenable'] = "ჩასართავად მონიშნე";
$_LANG['clientareatitle'] = "მომხმარებლის გვერდი";
$_LANG['clientareaunlimited'] = "ულიმიტო";
$_LANG['clientareaupdatebutton'] = "განახლება";
$_LANG['clientareaupdateyourdetails'] = "განახლეთ თქვენი მონაცემები";
$_LANG['clientareaused'] = "გამოყენებული";
$_LANG['clientareaviewaddons'] = "დამატებითი პროდუქტი";
$_LANG['clientareaviewdetails'] = "მონაცემების ნახვა";
$_LANG['clientlogin'] = "მომხმარებლსი ავტორიზაცია";
$_LANG['clientregisterheadertext'] = "რეგისტრაციისთვის შეავსეთ ქვემოთ მოცემული ველები.";
$_LANG['clientregistertitle'] = "რეგისტრაცია";
$_LANG['clientregisterverify'] = "რეგისტრაციის შემოწმება";
$_LANG['clientregisterverifydescription'] = "Please enter the text you see in the image below into the text box provided. This is required to prevent automated registrations.";
$_LANG['clientregisterverifyinvalid'] = "სავერიფიკაციო კოდი არასწორადაა შეყვანილი";
$_LANG['closewindow'] = "ფანჯრის დახურვა";
$_LANG['completeorder'] = "შეკვეთის დასრულება";
$_LANG['confirmnewpassword'] = "ახალი პაროლის დადასტურება";
$_LANG['contactemail'] = "ელ. ფოსტა";
$_LANG['contacterrormessage'] = "წერილისთვის განკუთვნილი სივრცე ცარიელია";
$_LANG['contacterrorname'] = "არ მიგითითებიან თქვენი სახელი";
$_LANG['contacterrorsubject'] = "არ მიგითითებიათ წერილის თემა";
$_LANG['contactheader'] = "თუკი გაქვთ შეკითხვები ან გარკვეული მიზეზით გსურთ ჩვენთან დაკავშირება - გამოიყენეთ ქვემოთ მოცემული ფორმა. ";
$_LANG['contactmessage'] = "შეტყობინება";
$_LANG['contactname'] = "სახელი";
$_LANG['contactsend'] = "შეტყობინების გაგზავნა";
$_LANG['contactsent'] = "თქვენი შეტყობინება გაიგზავნა";
$_LANG['contactsubject'] = "თემა";
$_LANG['contacttitle'] = "კონსულტაციის მიღების ფორმა";
$_LANG['continueshopping'] = "შესყიდვების გაგრძელება";
$_LANG['creditcard'] = "საკრედიტო ბარათით გადახდა";
$_LANG['creditcard3dsecure'] = "As part of our fraud prevention measures, you will now be asked to perform the Verified by Visa or Mastercard SecureCode verification to complete your payment.<br /><br />Do not click the refresh or back button or this transaction may be interrupted or cancelled.";
$_LANG['creditcardcardexpires'] = "ვადის გასვლის თარიღი";
$_LANG['creditcardcardissuenum'] = "გამოშვების ნომერი";
$_LANG['creditcardcardnumber'] = "ბარათის ნომერი";
$_LANG['creditcardcardstart'] = "გაცემის თარიღი";
$_LANG['creditcardcardtype'] = "ბარათის ტიპი";
$_LANG['creditcardccvinvalid'] = "საჭიროა ბარათის CVV კოდის მითთება";
$_LANG['creditcardconfirmation'] = "გმადლობთ! თქვენი ბარათის მონაცემები დადასტურდა და პირველი ტრანზაქციაც გახორციელდა. ელ.ფოსტაზე გამომგეზავნათ სავერიფიკაციო წერილი.";
$_LANG['creditcardcvvnumber'] = "CVV/CVC2";
$_LANG['creditcardcvvwhere'] = "სად შემიძლია ვიპოვე ეს?";
$_LANG['creditcarddeclined'] = "თქვენი ბარათის მონაცემები უარყოფილია. სცადეთ სხვა ბარათი ან დაუკავშირდით ტექ.დახმა";
$_LANG['creditcarddetails'] = "საკრედიტო ბარათის მონაცემები";
$_LANG['creditcardenterexpirydate'] = "არ მიგითითებიათ ბარათის ვარგისიანობის თარიღი";
$_LANG['creditcardenternewcard'] = "შეიყვანეთ ახალი ბარათის მონაცემები";
$_LANG['creditcardenternumber'] = "არ მიგითითებიათ საკრედიტო ბარათის ნომერი";
$_LANG['creditcardinvalid'] = "საკრედიტო ბარათის მონაცემები არასწორია. სცადეთ სხვა ბარათი ან დაუკავშირდით ჰოსტინგის დახმარების ცენტრს.";
$_LANG['creditcardnumberinvalid'] = "საკრედიტო ბარათის ნომერი არასწორია.";
$_LANG['creditcardsecuritynotice'] = "თქვენ მიერ შეყვანილი ინფორმაცია დაშიფრულია და დაცულია თაღლითების მავნე ხელისგან.";
$_LANG['creditcarduseexisting'] = "არსებული ბარათის გამოყენება";
$_LANG['customfieldvalidationerror'] = "მნიშვნელობა არასწორია";
$_LANG['days'] = "დღე"; #არ შეიცვალოს რადგან ქართულში დღეების რაოდენობის შემდეგ "დღე" მხოლობითშია
$_LANG['defaultbillingcontact'] = "ნაგულისხმევი ბილინგის კონტაქტი";
$_LANG['domainalternatives'] = "სცადეთ მოცემული ალტერნატივები:";
$_LANG['domainavailable'] = "ხელმისაწვდომია! ახლავე შეუკვეთეთ";
$_LANG['domainavailable1'] = "გილოცავთ!";
$_LANG['domainavailable2'] = "ხელმისაწვდომია!";
$_LANG['domainavailableexplanation'] = "დომეინს დასარეგისტირებლად დააჭირეთ ლინკს";
$_LANG['domainbulksearch'] = "დომეინების მასობრივი ძებნა";
$_LANG['domainbulksearchintro'] = "დომეინების მასობრივი ძიება საშუალებას გაძლევთ 20 დომეინზე ერთდროულად მოიძიოთ ინფორმაცია. ქვემოთ მოცემულ ველში შეიყვანეთ დომეინები, ერთ ხაზზე მიუთითე ერთი დომეინი - არ არის საჭირო დომეინების წინ  www. ან http:// პრეფიქსების დართვა.";
$_LANG['domainbulktransferdescription'] = "თქვენ შეგიძლიათ დომეინების ჩვენთან გადმოტანა/ტრანსფერი. ამისათვის, მიუთითეთ თქვენი დომეინი, თითო ერთ ხაზზე - არ გამოიყენოთ www. ან http://";
$_LANG['domainbulktransfersearch'] = "დომეინების მასობრივი ტრანსფერი";
$_LANG['domaincheckerdescription'] = "შეამოწმეთ დომეინის ხელმისაწვდომობა";
$_LANG['domaincontactinfo'] = "საკონტაქტო ინფორმაცია";
$_LANG['domaincurrentrenewaldate'] = "ამჟამინდელი გახანგრძლივების თარიღი";
$_LANG['domaindnsaddress'] = "მისამართი";
$_LANG['domaindnshostname'] = "Host Name";
$_LANG['domaindnsmanagement'] = "DNS-ის მართვა";
$_LANG['domaindnsmanagementdesc'] = "Point your domain to a web site by pointing to an IP Address, or forward to another site, or point to a temporary page (known as Parking), and more. These records are also known as sub-domains.";
$_LANG['domaindnsrecordtype'] = "ჩანაწერის ტიპი";
$_LANG['domainemailforwarding'] = "ელ.ფოსტის გადამისამართება";
$_LANG['domainemailforwardingdesc'] = "If the Email Forwarding Server determines the Forward To address is invalid, we will disable the forwarding record automatically. Please check the Forward To address before you enable it again. The changes on any existing forwarding record may not take effect for up to 1 hour.";
$_LANG['domainemailforwardingforwardto'] = "გადამისამართება სად";
$_LANG['domainemailforwardingprefix'] = "პრეფიქსი";
$_LANG['domaineppcode'] = "EPP კოდი";
$_LANG['domaineppcodedesc'] = "მოცემული კოდი უნდა გამოითხოვოთ ამჟამინდელი რეგისტრატორიდან, რათა გაიაროთ ავტორიზაცია.";
$_LANG['domaineppcoderequired'] = "თქვენ უნდა შეიყვანოთ EPP კოდი ";
$_LANG['domainerror'] = "რეგისტრატორთან დაკავშირება ვერ მოხერხდა. სცადეთ მოგვიანებით.";
$_LANG['domainerrornodomain'] = "შეიყვანეთ ვალიდური დომეინი";
$_LANG['domainerrortoolong'] = "მითითებული დომეინი ძალიან გრძელია. დომეინებისთვის დასაშვებია მაქსიმუმ 67 სიმბოლო.";
$_LANG['domaingeteppcode'] = "EPP კოდის მიღება";
$_LANG['domaingeteppcodeemailconfirmation'] = "EPP კოდი მოთხოვნილია! იგი გაიგზავნა ახალი რეგისტრანტი ელ.ფოსტაზე, რათა დასრულდეს დომეინის ტრანსფერი."; 
$_LANG['domaingeteppcodeexplanation'] = "EPP კოდი არის დომეინის ერთგვარი პაროლი. იგი ემსახურება უსაფრთხოებას, რათა დომეინი გამოიყენოს მან ვის კუთვნილებაშიცაა. EPP კოდი დაგჭირდებათ მაშინ, როდესაც ერთი რეგისტრატორიდან მეორეზე გადადიხართ. ";
$_LANG['domaingeteppcodefailure'] = "EPP კოდის მოთხოვნისას დაფიქსირდა შეცდომა:";
$_LANG['domaingeteppcodeis'] = "თქვენი დომეინის EPP კოდი არის:";
$_LANG['domainidprotection'] = "ID დაცვა";
$_LANG['domainintrotext'] = "შეიყვანეთ დომეინი და აირჩიეთ სასურველი დაბოლოოება, შემდგომ კი დააჭირეთ ღილაკს 'მოძებნა' და შეამოწმეთ დომეინის ხელმისაწვდომობა. ";
$_LANG['domainlookupbutton'] = "მოძებნა";
$_LANG['domainmanagementtools'] = "სამართავი ხელსაწყოები";
$_LANG['domainminyears'] = "მინ. წელი";
$_LANG['domainmoreinfo'] = "მეტი ინფორმაცია";
$_LANG['domainname'] = "დომეინის სახელწოდება";
$_LANG['domainnameserver1'] = "NS 1";
$_LANG['domainnameserver2'] = "NS 2";
$_LANG['domainnameserver3'] = "NS 3";
$_LANG['domainnameserver4'] = "NS 4";
$_LANG['domainnameserver5'] = "NS 5";
$_LANG['domainnameservers'] = "NameServers";
$_LANG['domainordernow'] = "ახლავე შეუკვეთე!";
$_LANG['domainorderrenew'] = "გახანგრძლივება";
$_LANG['domainprice'] = "ფასი";
$_LANG['domainregisterns'] = "NameServer-ების რეგისტრაცია";
$_LANG['domainregisternscurrentip'] = "მიმდინარე IP მისამართი";
$_LANG['domainregisternsdel'] = "NameServer-ის წაშლა";
$_LANG['domainregisternsdelsuccess'] = "NameServer-ები წაიშალა.";
$_LANG['domainregisternsexplanation'] = "აქედან შეგიძლიათ დაამათოთ და მართოთ NameServer-ები თქვენი From here you can create and manage custom NameServers for your domain (მაგ. NS1.hostnodes.ge, NS2.hostnodes.ge ...).";
$_LANG['domainregisternsip'] = "IP მისამართი";
$_LANG['domainregisternsmod'] = "NameServer-ის IP შეცვლა";
$_LANG['domainregisternsmodsuccess'] = "NameServer - ები წარმატებით შეიცვალა";
$_LANG['domainregisternsnewip'] = "ახალი IP მისამართი";
$_LANG['domainregisternsns'] = "NameServer-ები";
$_LANG['domainregisternsreg'] = "NameServer-ის სახელის რეგისტრაცია";
$_LANG['domainregisternsregsuccess'] = "NameServer  წარმატებით დარეგისტრირდა";
$_LANG['domainregistrantchoose'] = "აირჩიეთ საკონტაქტო ინფორმაცია, რომლის გამოყენებაც გსურთ";
$_LANG['domainregistrantinfo'] = "ინფორმაცია დომეინის რეგისტრანტზე";
$_LANG['domainregistrarlock'] = "რეგისტრანტის ბლოკი";
$_LANG['domainregistrarlockdesc'] = "გააქტიურეთ რეგისტრანტის ბლოკი (რეკომენდირებულია). არა ავტორიზუბული ტრანსფერებისგან თავის დასაცავად.";
$_LANG['domainregistration'] = "დომეინის რეგისტრაცია";
$_LANG['domainregistryinfo'] = "ინფორმაცია დარეგისტრირებულ დომეინზე";
$_LANG['domainregnotavailable'] = "არ არის ხელმისაწვდომი";
$_LANG['domainrenew'] = "დომეინის გახანგრძლივება";
$_LANG['domainrenewal'] = "დომეინის განახლება";
$_LANG['domainrenewalprice'] = "განახლება";
$_LANG['domainrenewdesc'] = "დომეინის დაკარგვის თავიდან ასაცილებლად გაახახნგრძლივეთ რამდენიმე წლით. აირჩიეთ რამდენი წლით გინდათ გაახანგრძლივოთ, შემდგომ კი დაადასტურეთ მოთხოვნა.";
$_LANG['domainsautorenew'] = "ავტომატური განახლება";
$_LANG['domainsautorenewdisable'] = "ავტომატური განხლების გამორთვა";
$_LANG['domainsautorenewdisabled'] = "გამორთულია";
$_LANG['domainsautorenewdisabledwarning'] = "გაფრთხილება! ამ დომეინისთვის ავტომატური განახლება გამორთულია.<br />It will therefore expire and become inactive at the end of the current term unless manually renewed.";
$_LANG['domainsautorenewenable'] = "ავტო-გახანგრძლივების ჩართვა";
$_LANG['domainsautorenewenabled'] = "ჩართულია";
$_LANG['domainsautorenewstatus'] = "ამჟამინდელი სტატუსი";
$_LANG['domainsimplesearch'] = "დომეინის გამარტივებული ძებნა";
$_LANG['domainspricing'] = "დომეინის ფასები";
$_LANG['domainsregister'] = "რეგისტრაცია";
$_LANG['domainsrenew'] = "გახანგრძლივება";
$_LANG['domainsrenewnow'] = "გაახანგრძლივე ახლავე";
$_LANG['domainstatus'] = "სტატუსი";
$_LANG['domainstransfer'] = "ტრანსფერი";
$_LANG['domaintitle'] = "დომეინის შემმოწმებელი";
$_LANG['domaintld'] = "TLD";
$_LANG['domaintransfer'] = "დომეინის ტრანსფერი";
$_LANG['domainunavailable'] = "ხელმიუწვდომელია";
$_LANG['domainunavailable1'] = "ვწუხვართ!";
$_LANG['domainunavailable2'] = "უკვე დაკავებულია!";
$_LANG['domainreserved'] = "დაჯავშნულია";
$_LANG['domainreserved1'] = "დომეინი";
$_LANG['domainreserved2'] = "თავისუფალია, მაგრამ დაჯავშნული.";
$_LANG['domainviewwhois'] = "იხილეთ whois ჩანაწერები";
$_LANG['downloaddescription'] = "აღწერა";
$_LANG['downloadloginrequired'] = "წვდომა შეზღუდულია - ფაილის გადმოსაწერად საჭიროა იყოთ რეგისტრებული";
$_LANG['downloadname'] = "გადმოწერა";
$_LANG['downloadpurchaserequired'] = "წვდომა შეზღუდულია - გადმოსაწერად საჭიროა შესაბამისი პროდუქტის/სერვისის ყიდვა.";
$_LANG['downloadscategories'] = "კატეგორიები";
$_LANG['downloadsdescription'] = "იხილეთ ჩვენი გადმოსაწერი ფაილების ბიბლიოთეკა";
$_LANG['downloadsfiles'] = "ფაილები";
$_LANG['downloadsfilesize'] = "ფაილის ზომა";
$_LANG['downloadsintrotext'] = "ფაილების ბიბლიოთეკა შეიცავს დოკუმენტაციებს, ინსტრუქციებს, პროგრამებს და ა.შ - ყველაფერ საჭიროს თქვენი საიტის მუშაობისთვის";
$_LANG['downloadspopular'] = "ხშირად გადმოწერილი ფაილები";
$_LANG['downloadsnone'] = "ჩამოსატვირთი ფაილები არ არის";
$_LANG['downloadstitle'] = "ფაილები";
$_LANG['email'] = "ელ.ფოსტა";
$_LANG['emptycart'] = "კალათა ცარიელია";
$_LANG['existingpassword'] = "ამჟამინდელი პაროლი";
$_LANG['existingpasswordincorrect'] = "თქვენ არასწორად შეიყვანეთ ამჟამინდელი პაროლი.";
$_LANG['firstpaymentamount'] = "პირველადი გადახდის ოდენობა";
$_LANG['flashtutorials'] = "Flash-გაკვეთილები";
$_LANG['flashtutorialsdescription'] = "Click here to view tutorials showing you how to use your hosting control panel";
$_LANG['flashtutorialsheadertext'] = "Our Flash Tutorials are here to help you fully utilise your web hosting control panel. Choose a task from below to see a step by step tutorial on how to complete it.";
$_LANG['forwardingtogateway'] = "დაელოდეთ, სანამ გადამისამართდებით თქვენს მიერ არჩეულ გადახდის მეთოდზე...";
$_LANG['globalsystemname'] = "მთავარი გვერდი";
$_LANG['globalyouarehere'] = "თქვენ ხართ";
$_LANG['go'] = "Go";
$_LANG['headertext'] = "მოგესალმებით დახმარების გვერდზე.";
$_LANG['hometitle'] = "მთავარი";
$_LANG['imagecheck'] = "გთხოვთ, შეიყვანოთ მოცემული კოდი.";
$_LANG['invoiceaddcreditamount'] = "მიუთითეთ ოდენობა";
$_LANG['invoiceaddcreditapply'] = "კრედიტის გამოყენება";
$_LANG['invoiceaddcreditdesc1'] = "თქვენი საკრედიტო შედგება";
$_LANG['invoiceaddcreditdesc2'] = "ეს შეგიძლიათ გამოიყენოთ ინვოისების გადასახდელად..";
$_LANG['invoiceaddcreditoverbalance'] = "თქვენ ვერ გამოიყენებთ საკრედიტო ბალანს, თუკი გადასახდელი თანხა მის ოდენობას აღემატება.";
$_LANG['invoiceaddcreditovercredit'] = "თქვენ ვერ გამოიყენებთ იმაზე მატ კრედიტს, ვიდრე რეალურად ანგარიშზე გაქვთ.";
$_LANG['invoicenumber'] = "ინვოისი #";
$_LANG['invoiceofflinepaid'] = "საკრედიტო ბარათით Offline გადახდა სრულდება მექანიკურად, კომპანიის თანამშრომლების მიერ.<br />წარმატებული გადახდის შემდეგ, თქვენი ელ.ფოსტაზე  დამადასტურებელ წერილს მიიღებთ.";
$_LANG['invoicerefnum'] = "დანიშნულება";
$_LANG['invoices'] = "ჩემი ინვოისები";
$_LANG['invoicesamount'] = "ოდენობა";
$_LANG['invoicesattn'] = "ATTN";
$_LANG['invoicesbacktoclientarea'] = "&laquo; მომხმარებლის არეში დაბრუნება";
$_LANG['invoicesbalance'] = "ბალანსი";
$_LANG['invoicesbefore'] = "before";
$_LANG['invoicescancelled'] = "გაუქმებული";
$_LANG['invoicescollections'] = "ინკასო";
$_LANG['invoicescredit'] = "კრედიტი";
$_LANG['invoicesdatecreated'] = "შექმნის დრო";
$_LANG['invoicesdatedue'] = "გადახდის თარიღი";
$_LANG['invoicesdescription'] = "აღწერა";
$_LANG['invoicesdownload'] = "ჩამოტვირთვა";
$_LANG['invoiceserror'] = "დაფიქსირდა შეცდომა. სცადეთ კიდევ ერთხელ.";
$_LANG['invoicesinvoicedto'] = "გადამხდელი";
$_LANG['invoicesinvoicenotes'] = "დამატებითი ინფო";
$_LANG['invoicesnoinvoices'] = "ინვოისები არ არის";
$_LANG['invoicesnotes'] = "ჩანაწერები";
$_LANG['invoicesoutstandinginvoices'] = "გადაუხდელი ინვოისები";
$_LANG['invoicespaid'] = "გადახდილია";
$_LANG['invoicespaynow'] = "ახლავე გადაიხადეთ";
$_LANG['invoicespayto'] = "მიმღები";
$_LANG['invoicesrefunded'] = "ანაზღაურებულია";
$_LANG['invoicesstatus'] = "სტატუსი";
$_LANG['invoicessubtotal'] = "ჯამი";
$_LANG['invoicestax'] = "დამ.გადასახადებითურთ";
$_LANG['invoicestaxindicator'] = "დაბეგვრის ინდიკატორი.";
$_LANG['invoicestitle'] = "ინვოისი #";
$_LANG['invoicestotal'] = "სულ";
$_LANG['invoicestransactions'] = "ტრანზაქციები";
$_LANG['invoicestransamount'] = "ოდენობა";
$_LANG['invoicestransdate'] = "ტრანზაქციის თარიღი";
$_LANG['invoicestransgateway'] = "გადახდის ტიპი";
$_LANG['invoicestransid'] = "ტრანაქციის ID";
$_LANG['invoicestransnonefound'] = "ინვოისისთვის ტრანზაქციები ვერ მოიძებნა";
$_LANG['invoicesunpaid'] = "გადაუხდელია";
$_LANG['invoicesdraft'] = "Draft";
$_LANG['invoicesview'] = "ინვოისის ნახვა";
$_LANG['jobtitle'] = "სათაური";
$_LANG['kbsuggestions'] = "რჩევები 'დაზმარების გვერდიდან'";
$_LANG['knowledgebaserelatedarticles'] = "მსგავსი თემები";
$_LANG['knowledgebaseserachplaceholder'] = "რით შეგვიძლია დაგეხმაროთ? ";
$_LANG['kbsuggestionsexplanation'] = "ჩვენს 'დახმარების გვერდზე' მოიძებნა სტატია, რომელიც თქვენს კითხვაე პასუხს მოიცავს. შეგიძლიათ, კითხვის დასმამდე სტატიას გადახედოთ..";
$_LANG['knowledgebasearticles'] = "სტატიები";
$_LANG['knowledgebasecategories'] = "კატეგორიები";
$_LANG['knowledgebasedescription'] = "გადახედე დახმარების გვერდს FAQ-ის სანახავად";
$_LANG['knowledgebasefavorites'] = "ფავორიტებში დამატება";
$_LANG['knowledgebasehelpful'] = "მიიღეთ სრული ინფორმაცია კითხვაზე?";
$_LANG['knowledgebaseintrotext'] = "დახმარების გვერდი რამდენიმე კატეგორიადაა დაყოფილი. ან კატეგორია აირჩიეთ ან მოძებნეთ შესატყვისი სიტყვებით თქვენს სასურველს კითხვაზე პასუხი";
$_LANG['knowledgebasemore'] = "იხილეთ მეტი";
$_LANG['knowledgebaseno'] = "არა";
$_LANG['knowledgebasenoarticles'] = "სტატიები არ მოიძებნა";
$_LANG['knowledgebasenorelated'] = "მსგავსი სტატიები არ მოიძებნა";
$_LANG['knowledgebasepopular'] = "ყველაზე პოპულარული სტატიები";
$_LANG['knowledgebaseprint'] = "სტატიის ამობეჭვდა";
$_LANG['knowledgebaserating'] = "რეიტინგი:";
$_LANG['knowledgebaseratingtext'] = "მოხმარებელს სტატია გამოადგა";
$_LANG['knowledgebaserelated'] = "მსგავსი სტატიები";
$_LANG['knowledgebasesearch'] = "ძებნა";
$_LANG['knowledgebasetitle'] = "დახმარების გვერდი";
$_LANG['knowledgebaseviews'] = "ნახვები";
$_LANG['knowledgebasevote'] = "ხმის მიცემა";
$_LANG['knowledgebasevotes'] = "ხმები";
$_LANG['knowledgebaseyes'] = "დიახ";
$_LANG['knowledgebaseArticleRatingThanks'] = "მადლობა სტატიის შეფასებისთვის";
$_LANG['language'] = "ენა";
$_LANG['latefee'] = "ვადაგადაცილების ჯარიმა";
$_LANG['latefeeadded'] = "დამატებულია";
$_LANG['latestannouncements'] = "განცხადებები";
$_LANG['loginbutton'] = "ავტორიზაცია";
$_LANG['loginemail'] = "ელ.ფოსტის მისამართი";
$_LANG['loginforgotten'] = "დაგავიწყდათ პაროლი?";
$_LANG['loginforgotteninstructions'] = "მოითხოვეთ აღდგენა";
$_LANG['loginincorrect'] = "შეყვანილი მონაცემები არასწორია. სცადეთ კიდევ ერთხელ.";
$_LANG['loginintrotext'] = "გვერდის სანახავად საჭიროა ანგარიშზე შესვლა. ";
$_LANG['loginpassword'] = "პაროლი";
$_LANG['loginrememberme'] = "დამიმახსოვრე";
$_LANG['logoutcontinuetext'] = "გასაგრძელებლად აქ დააჭირეთ...";
$_LANG['logoutsuccessful'] = "თქვენ წარმატებით გამოხვედით სისტემიდან..";
$_LANG['logouttitle'] = "გამოსვლა";
$_LANG['maxmind_anonproxy'] = "პროქსი სერვერით მოსარგებლეთა შეკვეთა ავტომატურად უქმდება";
$_LANG['maxmind_callingnow'] = "თაღლითობისგან თავდაცვის მიზნით, თქვენს ტელეფონზე განხორციელდება ზარი. მოგაწვდით ოთხ ციფრა კოდს, რომელიც შეკვეთის დასასრულებლად უნდა მიუთითოთ.";
$_LANG['maxmind_countrymismatch'] = "თქვენი IP მისამართის ქვეყანა, არ ემთხვევა საბილინგო მისამართში მითითებულ ქვეყანას, რის გამოც შეკვეთა ვერ განხორციელდება.";
$_LANG['maxmind_error'] = "შეცდომა";
$_LANG['maxmind_faileddescription'] = "მითითებული კოდი არასწორია. თუ თვლით, რომ კოდი სწორია, დაუკავშირდით ჩვენს ტექნიკური დახმარების დეპარტამენტს.";
$_LANG['maxmind_highfraudriskscore'] = "MaxMind-ის ავტომატური შემოწმების პროგრამამ განსაზღვრა, რომ შესაძლოა ამ შეკვეთისას თაღლითობას ჰქონდეს ადგილი. ამიტომაც, შეკვეთა დაკავდება მექანიკური შემოწმებისთვის.<br /><br />თუკი თვლით, რომ მსგავსი რამ არ ხდება, მაშინ ბოდიშს გიხდით და გთხოვთ, <a href=\"submitticket.php\">გახსნათ დახმარების ბილეთი</a> პრობლემის მალევე აღმოსაფხვრელად.  გმადლობთ.";
$_LANG['maxmind_highriskcountry'] = "სამწუხაროდ, მოთხოვნას ვერ მივიღებთ, ვინაინად თქვენი ქვეყნიდან ფიქსირდება დიდი ოდენობით თაღლითური შემთხვევები. თუკი გსურთ სიტუაციის გარკვევა ან ალტერნატიული გადახდის მეთოდით სარგებლობა - დაგვიკავშირდით.";
$_LANG['maxmind_incorrectcode'] = "კოდი არასწორია";
$_LANG['maxmind_pincode'] = "Pin კოდი";
$_LANG['maxmind_rejectemail'] = "ჩვენ არ გრთავთ ნებას შეკვეთა განახორციელოთ უფასო საფოსტო სისტემებით, გთხოვთ, გამოიყენოთ სხვა ელ.ფოსტა.";
$_LANG['maxmind_title'] = "MaxMind";
$_LANG['more'] = "მეტი";
$_LANG['morechoices'] = "მეტი არჩევანი";
$_LANG['networkissuesaffecting'] = "Affecting";
$_LANG['networkissuesaffectingyourservers'] = "Please Note: Issues affecting servers you have accounts on will be highlighted with a gold background";
$_LANG['networkissuesdate'] = "თარიღი";
$_LANG['networkissuesdescription'] = "გაეცანით ამჟამინდელ და მოსალოდნელ ქსელურ ხარვეზებს.";
$_LANG['networkissueslastupdated'] = "ბოლოს განახლდა";
$_LANG['networkissuesnonefound'] = "ქსელური ხარვეზები არაა";
$_LANG['networkissuespriority'] = "პრიორიტეტი";
$_LANG['networkissuesprioritycritical'] = "კრიტიკული";
$_LANG['networkissuespriorityhigh'] = "მაღალი";
$_LANG['networkissuesprioritylow'] = "დაბალი";
$_LANG['networkissuesprioritymedium'] = "საშუალო";
$_LANG['networkissuesstatusinprogress'] = "მიმდინარე";
$_LANG['networkissuesstatusinvestigating'] = "აღმოფხვრა";
$_LANG['networkissuesstatusopen'] = "მიმდინარე";
$_LANG['networkissuesstatusoutage'] = "ხარვეზი";
$_LANG['networkissuesstatusreported'] = "შეტყობინებული";
$_LANG['networkissuesstatusresolved'] = "დასრულებული";
$_LANG['networkissuesstatusscheduled'] = "დაგეგმილი";
$_LANG['networkissuestitle'] = "ქსელის ხარვეზი";
$_LANG['networkissuestypeother'] = "სხვა";
$_LANG['networkissuestypeserver'] = "სერვერი";
$_LANG['networkissuestypesystem'] = "სისტემა";
$_LANG['newpassword'] = "ახალი პაროლი";
$_LANG['nextpage'] = "შემდეგ გვერდი";
$_LANG['no'] = "არა";
$_LANG['nocarddetails'] = "საკრედიტო ბარათის ჩანაწერები არ მოიძებნა";
$_LANG['none'] = "არცერთი";
$_LANG['norecordsfound'] = "ჩანაწერები ვერ მოიძებნა.";
$_LANG['or'] = "ან";
$_LANG['orderadditionalrequiredinfo'] = "დამატებითი ინფორმაცია";
$_LANG['orderaddon'] = "დამატება";
$_LANG['orderaddondescription'] = "ამ პროდუქტისთვის ხელმისაწვდომია მოცემულ დამატებები. ამოირჩიეთ სასურველი დამატებები მათ შესაკვეთად.";
$_LANG['orderavailable'] = "ხელმისაწვდომი";
$_LANG['orderavailableaddons'] = "დააჭირეთ დანამატების სანახავად";
$_LANG['orderbillingcycle'] = "გადახდის ციკლი";
$_LANG['ordercategories'] = "კატეგორიები";
$_LANG['orderchangeaddons'] = "დანამატების შეცვლა";
$_LANG['orderchangeconfig'] = "შეჩვალეთ მოწყობადი პარამეტრები";
$_LANG['orderchangedomain'] = "დომეინის შეცვლა";
$_LANG['orderchangenameservers'] = "NameServer-ების შეცვლა";
$_LANG['orderchangeproduct'] = "პროდუქტის შეცვლა";
$_LANG['ordercheckout'] = "შეკვეთა";
$_LANG['orderchooseaddons'] = "სერვისის დანამატის არჩევა";
$_LANG['orderchooseapackage'] = "პაკეტის არჩევა";
$_LANG['ordercodenotfound'] = "მითითებული ფასდაკლების კოდი არ არსებობს.";
$_LANG['ordercompletebutnotpaid'] = "გაფრთხილება! თქვენი შეკვეთა მიღებულია, თუმცა თანხა ჯერ არ გადაგიხდიათ, შესაბამისად კი სერვისი/პროდუქტი არა აქტიურია.<br />დააჭირეთ ქვემოთ მოცემულ ლინკს, რათა ინვოისის სანახავად და მის გადასახდელად.";
$_LANG['orderconfigpackage'] = "მოწყობადი პარამეტრები";
$_LANG['orderconfigure'] = "კონფიგურაცია";
$_LANG['orderconfirmation'] = "შეკვეთის დადასტურება";
$_LANG['orderconfirmorder'] = "შეკვეთის დადასტურება";
$_LANG['ordercontinuebutton'] = "გასაგრძელებლად დააკლიკეთ >>";
$_LANG['orderdesc'] = "აღწერა";
$_LANG['orderdescription'] = "Place a new order with us";
$_LANG['orderdiscount'] = "ფასდაკლება";
$_LANG['orderdomain'] = "დომეინი";
$_LANG['orderdomainoption1part1'] = "მე მსურს";
$_LANG['orderdomainoption1part2'] = "ახალი დომეინის დარეგისტრირება";
$_LANG['orderdomainoption2'] = "მე განვაახლებ NameServer-ებს არსებულ ანგარიშზე ან დავარეგისტრირებ ახალ დომეინს.";
$_LANG['orderdomainoption3'] = "მინდა დომეინის ტრანსფერი შემდეგ მისამართზე ";
$_LANG['orderdomainoption4'] = "მსურს უფასო ქვედომეინის გამოყენება.";
$_LANG['orderdomainoptions'] = "დომეინის პარამეტრები";
$_LANG['orderdomainregistration'] = "დომეინის რეგისტრაცია";
$_LANG['orderdomainregonly'] = "დომეინის რეგისტრაცია";
$_LANG['orderdomaintransfer'] = "დომეინის ტრანსფერი";
$_LANG['orderdontusepromo'] = "ფასდაკლების არგამოყენება";
$_LANG['ordererroraccepttos'] = "თქვენ უნდა დაეთანხმოთ ჩვენს ხელშეკრულებას";
$_LANG['ordererrordomainalreadyexists'] = "დომეინი რომელიც თქვენ შეიყვანეთ უკვე რეგისტრირებულია ჩვენთან. ახალის შეკვეთის განსათავსებლად საჭიროა ძველის წაშლა.";
$_LANG['ordererrordomaininvalid'] = "მითითებული დომეინი არასწორია";
$_LANG['ordererrordomainnotld'] = "საჭიროა უმაღლესი დონის დომეინის მითითება(TLD)";
$_LANG['ordererrordomainnotregistered'] = "შეუძლებელია იმ დომეინის ტრანსფერი, რომელიც არ არის რეგისტრირებული.";
$_LANG['ordererrordomainregistered'] = "მითითებული დომეინით უკვე სხვა არის რეგისტრირებული";
$_LANG['ordererrornameserver1'] = "მიუთითეთ NameServer 1";
$_LANG['ordererrornameserver2'] = "მიუთითეთ NameServer 2";
$_LANG['ordererrornodomain'] = "თქვენ არ შეგიყვანიათ დომეინის სახელწოდება";
$_LANG['ordererrorpassword'] = "არ მიგითითებიათ პაროლი";
$_LANG['ordererrorserverhostnameinuse'] = "თქვენს მიერ შეყვანილი დომენის სახელი უკვე გამოიყენება. გთხოვთ აირჩიოთ სხვა.";
$_LANG['ordererrorservernohostname'] = "თქვენ უნდა შეიყვანოთ დომენის სახელი თქვენი სერვერისთვის";
$_LANG['ordererrorservernonameservers'] = "თქვენ უნდა შეიყვანოთ პრეფიქსი ორივე Nameserver - ისთვის";
$_LANG['ordererrorservernorootpw'] = "მიუთითეთ სასურველი პაროლი ადმინისტრატორისთვის";
$_LANG['ordererrorsubdomaintaken'] = "მითითებული ქვედომეინი უკვე დაკავებულია - სცადეთ კიდევ ერთხელ";
$_LANG['ordererrortransfersecret'] = "ტრანსფერისთვის საჭიროა მიუთითოთ საიდუმლო კოდი";
$_LANG['ordererroruserexists'] = "მომხმარებელ მსგავსი ელ.ფოსტით უკვე არსებობს";
$_LANG['orderexistinguser'] = "მე თქვენი მომხმარებელი ვარ და მსურს ამ შეკვეთის ჩემს ანგარიშზე მიბმა.";
$_LANG['orderfailed'] = "შეკვეთა ჩაიშალა";
$_LANG['orderfinalinstructions'] = "თუკი გაქვთ დამატებითი კითხვები თქვენი შეკვეთის შესახებ, please open a support ticket from your client area and quote your order number.";
$_LANG['orderfree'] = "უფასო!";
$_LANG['orderfreedomainappliesto'] = "applies to the following extensions only";
$_LANG['orderfreedomaindescription'] = "on selected payment terms";
$_LANG['orderfreedomainonly'] = "უფასო დომეინი";
$_LANG['orderfreedomainregistration'] = "დომეინის უფასო რეგისტრაცია";
$_LANG['ordergotoclientarea'] = "მომხმარებლის არეში გადასასვლელად დააჭირეთ აქ";
$_LANG['orderinvalidcodeforbillingcycle'] = "This code does not apply to the selected billing cycle";
$_LANG['orderlogininfo'] = "საავტორიზაციო ინფორმაცია";
$_LANG['orderlogininfopart1'] = "შეიყვანეთ სასურველი პაროლი, რომლითაც გსურთ გამოიყენოთ მომხმარებლის არეში შესასვლელად";
$_LANG['orderlogininfopart2'] = "ის განსხავვდება იმ ლოგინისა და პაროლისგან, რომელსაც თქვენს საიტზე იყენებთ..";
$_LANG['ordernewuser'] = "მე ახალი მოხმარებელი ვარ და მსურსა ანგარიშის შექმნა";
$_LANG['ordernoproducts'] = "პროდუქტები არ მოიძებნა";
$_LANG['ordernotes'] = "ჩანაწერები / დამატებითი ინფორმაცია";
$_LANG['ordernotesdescription'] = "შეგიძლიათ თქვენს შეკვეთას დამატებითი ინფორმაციაც სურვილისამებრ დაურთოთ..";
$_LANG['ordernowbutton'] = "შეუკვეთეთ";
$_LANG['ordernumberis'] = "თქვენი შეკვეთის ნომერია:";
$_LANG['orderpaymentmethod'] = "გადახდის მეთოდი";
$_LANG['orderpaymentterm12month'] = "12 თვის ღირებულება";
$_LANG['orderpaymentterm1month'] = "1 თვის ღირებულება";
$_LANG['orderpaymentterm24month'] = "24 თვის ღირებულება";
$_LANG['orderpaymentterm3month'] = "3 თვის ღირებულება";
$_LANG['orderpaymentterm6month'] = "6 თვის ღირებულება";
$_LANG['orderpaymenttermannually'] = "ყოველწლიური";
$_LANG['orderpaymenttermbiennially'] = "ყოველ ორ წელში";
$_LANG['orderpaymenttermfreeaccount'] = "უფასო ანგარიში";
$_LANG['orderpaymenttermmonthly'] = "ყოველთვიური";
$_LANG['orderpaymenttermonetime'] = "ერთჯერადი";
$_LANG['orderpaymenttermquarterly'] = "კვარტალური";
$_LANG['orderpaymenttermsemiannually'] = "ნახევარ-წლიური";
$_LANG['orderprice'] = "ღირებულება";
$_LANG['orderproduct'] = "სერვისები";
$_LANG['orderprogress'] = "მდგომარეობა";
$_LANG['orderpromoexpired'] = "ფასდაკლების კოდი ვადა გასულია";
$_LANG['orderpromoinvalid'] = "ფასდაკლების კოდი კალათაში მოცემულ სერვისებისთვის არ არის განკუთვნილი";
$_LANG['orderpromomaxusesreached'] = "ფასდაკლება დამთავრებულია";
$_LANG['orderpromotioncode'] = "ფასდაკლების კოდი";
$_LANG['orderpromovalidatebutton'] = "კოდის დადასტურება";
$_LANG['orderPromoCodePlaceholder'] = "შეიყვანეთ ფასდაკლების კოდი";
$_LANG['orderprorata'] = "Pro Rata";
$_LANG['orderreceived'] = "გმადლობთ შეკვეთისთვის. მალევე მიიღებთ დასადასტურებელ ელ.ფოსტას.";
$_LANG['orderregisterdomain'] = "დომეინის დარეგისტრირება";
$_LANG['orderregperiod'] = "რეგისტრაციის პერიოდი";
$_LANG['ordersecure'] = "მიმდინარე შეკვეთის ფორმა შემოთავაზებულია დაცულ გარემოში, რათა თქვენი მიმდინარე იპ დაცული იყოს ინფორმაციის არასანქცირებული დაშვებისგან.";
$_LANG['ordersecure2'] = "is being logged.";
$_LANG['orderserverhostname'] = "Server Hostname";
$_LANG['orderservernameservers'] = "Nameserver-ები";
$_LANG['orderservernameserversdescription'] = "The prefixes you enter here will determine the default nameservers for the server eg. ns1.yourdomain.com and ns2.yourdomain.com";
$_LANG['orderservernameserversprefix1'] = "პრეფიქსი 1";
$_LANG['orderservernameserversprefix2'] = "პრეფიქსი 2";
$_LANG['orderserverrootpassword'] = "Root Password";
$_LANG['ordersetupfee'] = "დაყენების ფასი";
$_LANG['orderstartover'] = "Start Over";
$_LANG['ordersubdomaininuse'] = "The subdomain you entered is already in use";
$_LANG['ordersubtotal'] = "გადასახდელი თანხა";
$_LANG['ordersummary'] = "შეკვეთის შეჯამება";
$_LANG['ordertaxcalculations'] = "გადასახადების ანგარიში";
$_LANG['ordertaxstaterequired'] = "მიუთითეთ  შტატი გადასახადების დასაანგარიშებლად";
$_LANG['ordertitle'] = "შეკვეთა";
$_LANG['ordertos'] = "ხელშეკრულება(ს)";
$_LANG['ordertosagreement'] = "მე წავიკითხე და ვეთანხმები";
$_LANG['ordertotalduetoday'] = "სულ გადასახდელია";
$_LANG['ordertotalrecurring'] = "ჯამში გადასახდელი";
$_LANG['ordertransferdomain'] = "არსებული დომეინის ტრანსფერი";
$_LANG['ordertransfersecret'] = "ტრანსფერის საიდუმლო კოდი";
$_LANG['ordertransfersecretexplanation'] = "შეიყვანეთ ტრანსფერის საიდუმლო კოდი(Transfer Secret), რომელიც თქვენს ამჟამინდელ რეგისტრანტს უნდა გამოართვათ. ";
$_LANG['orderusesubdomain'] = "ქვედომეინის გამოყენება";
$_LANG['orderyears'] = "წელი";
$_LANG['orderyourinformation'] = "თქვენი ინფო";
$_LANG['orderyourorder'] = "თქვენი შეკვეთა";
$_LANG['organizationname'] = "ორგანიზაციის სახელწოდება";
$_LANG['outofstock'] = "ამოწურულია";
$_LANG['outofstockdescription'] = "We are currently out of stock on this item so orders for it have been suspended until more stock is available. For further information, please contact us.";
$_LANG['page'] = "გვერდი";
$_LANG['pageof'] = "of";
$_LANG['please'] = "გთხოვთ,";
$_LANG['pleasewait'] = "გთხოვთ მოიცადოთ...";
$_LANG['presalescontactdescription'] = "Place any pre-sales enquiries here";
$_LANG['previouspage'] = "წინა გვერდი";
$_LANG['proformainvoicenumber'] = "Proforma Invoice #";
$_LANG['promoexistingclient'] = "ამ კოდის გამოსაყენებლად საჭიროა აქტიური მომხსახურეობა/სერვისი";
$_LANG['promoonceperclient'] = "მოცემული კოდის გამოყენება მხოლოდ 1 მომხმარებელს შეუძლია";
$_LANG['pwstrengthfail'] = "მითითებული პაროლი არ არის საკმარისად ძლიერი - მოიფიქრეთ უფრო რთული პაროლი";
$_LANG['pwdoesnotmatch'] = "მითითებული პაროლები ერთმანეთს არ ემთხვევა";
$_LANG['quicknav'] = "სწრაფი ნავიგაცია";
$_LANG['recordsfound'] = "ჩანაწერები მოიძებნა";
$_LANG['recurring'] = "მოითხოვება";
$_LANG['recurringamount'] = "მოთხოვნილი ოდენობა";
$_LANG['every'] = "ყოველი";
$_LANG['registerdomain'] = "დომეინის რეგისტრაცია";
$_LANG['registerdomaindesc'] = "Type in the domain you wish to register below to check for availability.";
$_LANG['registerdomainname'] = "დომეინის რეგისტრაცია";
$_LANG['relatedservice'] = "მსგავსი სერვისი";
$_LANG['rssfeed'] = "Feed";
$_LANG['securityanswerrequired'] = "მოითხოვება საიდუმლო კითხვის პასუხის მითითება";
$_LANG['securitybothnotmatch'] = "Your answer and confirm answer do not match";
$_LANG['securitycurrentincorrect'] = "Your current question and answer is incorrect";
$_LANG['serverchangepassword'] = "პაროლის შეცვლა";
$_LANG['serverchangepasswordintro'] = "From here you can change the password of the product/service (note: this does not affect your password for our client area)";
$_LANG['serverchangepasswordconfirm'] = "პაროლის დადასტურება";
$_LANG['serverchangepasswordenter'] = "ახალი პაროლის მითითება";
$_LANG['serverchangepasswordfailed'] = "პაროლი ვერ შეიცვალა!";
$_LANG['serverchangepasswordsuccessful'] = "პაროლი წარმატებით შეიცვალა!";
$_LANG['serverchangepasswordupdate'] = "განახლება";
$_LANG['serverhostname'] = "Hostname";
$_LANG['serverlogindetails'] = "ავტორიზაციის დეტალები";
$_LANG['servername'] = "Server";
$_LANG['serverns1prefix'] = "NS1 Prefix";
$_LANG['serverns2prefix'] = "NS2 Prefix";
$_LANG['serverpassword'] = "პაროლი";
$_LANG['serverrootpw'] = "Root Password";
$_LANG['serverstatusdescription'] = "View live status info for our servers";
$_LANG['serverstatusnoservers'] = "No Servers are currently being monitored";
$_LANG['serverstatusnotavailable'] = "არ არის ხელმისაწვდომი";
$_LANG['serverstatusoffline'] = "Offline";
$_LANG['serverstatusonline'] = "Online";
$_LANG['serverstatusphpinfo'] = "PHP ინფო";
$_LANG['serverstatusserverload'] = "სერვერის დატვირთვა";
$_LANG['serverstatustitle'] = "სერვერის სტატუსი";
$_LANG['serverstatusuptime'] = "Uptime";
$_LANG['serverusername'] = "Username";
$_LANG['show'] = "ჩვენება";
$_LANG['ssladmininfo'] = "Administrative Contact Information";
$_LANG['ssladmininfodetails'] = "The contact information below will not be displayed on the Certificate - it is used only for contacting you regarding this order. The SSL Certificate and future renewal reminders will be sent to the email address specified below.";
$_LANG['sslcertapproveremail'] = "Certificate Approver Email";
$_LANG['sslcertapproveremaildetails'] = "You must now choose from the options below where you would like the approval email request for this certificate to be sent.";
$_LANG['sslcertinfo'] = "SSL სერტიფიკაცის ინფორმაცია";
$_LANG['pleasechooseone'] = "აირჩიეთ ერთი...";
$_LANG['sslcerttype'] = "სერტიფიკატის ტიპი";
$_LANG['sslconfigcomplete'] = "Configuration Complete";
$_LANG['sslconfigcompletedetails'] = "Your SSL certificate configuration has now been completed and sent to the Certificate Authority for validation. You should receive an email from them shortly to approve it.";
$_LANG['sslconfsslcertificate'] = "Configure SSL Certificate";
$_LANG['sslcsr'] = "CSR";
$_LANG['sslerrorapproveremail'] = "You must choose an approver email address";
$_LANG['sslerrorentercsr'] = "You must enter your certificate signing request (CSR)";
$_LANG['sslerrorselectserver'] = "You must select your server type";
$_LANG['sslinvalidlink'] = "Invalid Link Followed. Please go back and try again.";
$_LANG['sslorderdate'] = "შეკვეთის თარიღი";
$_LANG['sslserverinfo'] = "Server Information";
$_LANG['sslserverinfodetails'] = "You must have a valid \"CSR\" (Certificate Signing Request) to configure your SSL Certificate. The CSR is an encrypted piece of text that is generated by the web server where the SSL Certificate will be installed. If you do not already have a CSR, you must generate one or ask your web hosting provider to generate one for you. Also please ensure you enter the correct information as it cannot be changed after the SSL Certificate has been issued.";
$_LANG['sslservertype'] = "Web Server Type";
$_LANG['sslstatus'] = "Configuration Status";
$_LANG['statscreditbalance'] = "Account Credit Balance";
$_LANG['statsdueinvoicesbalance'] = "Due Invoices Balance";
$_LANG['statsnumdomains'] = "დომეინების რაოდენობა";
$_LANG['statsnumproducts'] = "პროდუქტების/სერვისების რაოდენობა";
$_LANG['statsnumreferredsignups'] = "Number of Referred Signups";
$_LANG['statsnumtickets'] = "Number of Support Tickets";
$_LANG['submitticketdescription'] = "გახსენით დახმარების ბილეთი";
$_LANG['supportclickheretocontact'] = "დასაკავშირებლად დააჭირეთ აქ";
$_LANG['supportpresalesquestions'] = "If you have pre-sales questions";
$_LANG['supportticketinvalid'] = "დაფიქსირდა შეცდომა. მოთხოვნილი ბილეთი ვერ მოიძებნა.";
$_LANG['supportticketsallowedextensions'] = "დაშვებული ფაილების ფორმატი";
$_LANG['supportticketschoosedepartment'] = "განყოფილების არჩევა";
$_LANG['supportticketsclient'] = "მომხმარებელი";
$_LANG['supportticketsclientemail'] = "ელ.ფოსტა";
$_LANG['supportticketsclientname'] = "სახელი";
$_LANG['supportticketsdate'] = "თარიღი";
$_LANG['supportticketsdepartment'] = "განყოფილება";
$_LANG['supportticketsdescription'] = "View and respond to existing tickets";
$_LANG['supportticketserror'] = "შეცდომა";
$_LANG['supportticketserrornoemail'] = "ელ.ფოსტა არ მიგითითებიათ";
$_LANG['supportticketserrornomessage'] = "წერილი ცარიელია";
$_LANG['supportticketserrornoname'] = "სახელი არ მიგითებიათ";
$_LANG['supportticketserrornosubject'] = "წერილის თემა არ მიგითითებიათ";
$_LANG['supportticketsfilenotallowed'] = "ის ფაილი, რომლის ატვირთვასაც ცდილობთ, დაუშვებელია.";
$_LANG['supportticketsheader'] = "გთხოვთ, გახსნათ ბილეთი შესაბამისი განყოფილებისთვის.";
$_LANG['supportticketsnotfound'] = "ბილეთები არ მოიძებნა";
$_LANG['supportticketsopentickets'] = "გახსნილი დახმარების ბილეთები";
$_LANG['supportticketspagetitle'] = "დახმარების ბილეთები";
$_LANG['supportticketsposted'] = "Posted";
$_LANG['supportticketsreply'] = "პასუხი";
$_LANG['supportticketsstaff'] = "პერსონალი";
$_LANG['supportticketsstatus'] = "სტატუსი";
$_LANG['supportticketsstatusanswered'] = "პასუხგაცემული";
$_LANG['supportticketsstatusclosed'] = "დახურული";
$_LANG['supportticketsstatuscloseticket'] = "თუ პრობლემა გადაჭრილია, ბილეთს დასახურად დააჭირეთ აქ";
$_LANG['supportticketsstatuscustomerreply'] = "პასუხგაუცემელი";
$_LANG['supportticketsstatusinprogress'] = "მიმდინარე";
$_LANG['supportticketsstatusonhold'] = "შეჩერებული";
$_LANG['supportticketsstatusopen'] = "გახსნილი";
$_LANG['supportticketssubject'] = "წერილის თემა";
$_LANG['supportticketssubmitticket'] = "ბილეთის გახსნა";
$_LANG['supportticketssystemdescription'] = "დახმარების ბილეთი იძლევა საშუალებას დაგეხმაროთ და აღმოვფხვრათ თქვენი პრობლემები, რაც შეიძლება მალე. როდესაც ბილეთზე გიპასუხებთ, თქვენ ელ.ფოსტაზეც მიიღებთ შეტყობინებას.";
$_LANG['supportticketsticketattachments'] = "დანართი";
$_LANG['supportticketsticketcreated'] = "ბილეთი გაიხსნა";
$_LANG['supportticketsticketcreateddesc'] = "ბილეთი გახსნილია. თქვენს ელ. ფოსტაზე გაიგზავნა წერილი ახალი ბილეთის გახსნის შესახებ. ამ ბილეთის ნახვა შეგიძლიათ \"მთავარი გვერდი / მომხმარებლის გვერდი /  დახმარების ბილეთები\"";
$_LANG['supportticketsticketid'] = "ბილეთის ID";
$_LANG['supportticketsticketsubject'] = "თემა";
$_LANG['supportticketsticketsubmit'] = "დადასტურება";
$_LANG['supportticketsticketurgency'] = "Urgency";
$_LANG['supportticketsticketurgencyhigh'] = "მაღალი";
$_LANG['supportticketsticketurgencylow'] = "დაბალი";
$_LANG['supportticketsticketurgencymedium'] = "საშუალო";
$_LANG['supportticketsuploadfailed'] = "დანართი ვერ აიტვირთა";
$_LANG['supportticketsviewticket'] = "ბილეთის ნახვა";
$_LANG['supportticketclosedmsg'] = "ეს ბილეთი დახურულია.  შეგიძლიათ დაურთოთ პასუხი მის გასანახლებლადა და გასახსნელად.";
$_LANG['telesignincorrectpin'] = "არასწორი PIN!";
$_LANG['telesigninitiatephone'] = "We can't initiate phone verification for your number. Please contact us.";
$_LANG['telesigninvalidnumber'] = "არასწორად შედგენილი მობილურის ნომერი";
$_LANG['telesigninvalidpin'] = "The PIN you entered was invalid!";
$_LANG['telesigninvalidpin2'] = "The pin you entered was not valid.";
$_LANG['telesigninvalidpinmessage'] = "Pin Code Verification Failed";
$_LANG['telesignmessage'] = "Phone verification initiated for number %s . Please wait...";
$_LANG['telesignphonecall'] = "Phone call";
$_LANG['telesignpin'] = "მიუთითეთ PIN: ";
$_LANG['telesignsms'] = "Sms";
$_LANG['telesignsmstextmessage'] = "Thank you for using our SMS verification system. Your code is: %s Please enter this code on your computer now.!";
$_LANG['telesigntitle'] = "TeleSign phone verification.";
$_LANG['telesigntype'] = "Choose verification type for number %s:";
$_LANG['telesignverificationcanceled'] = "There is a temporary problem with the phone verification service and phone verification has been canceled.";
$_LANG['telesignverificationproblem'] = "There was a problem with the phone verification service and your order could not be validated. Please try again later.";
$_LANG['telesignverify'] = "Your phone number %s needs to be verified to complete the order.";
$_LANG['ticketratingexcellent'] = "Excellent";
$_LANG['ticketratingpoor'] = "Poor";
$_LANG['ticketratingquestion'] = "How would you rate this reply?";
$_LANG['ticketreatinggiven'] = "You rated this response";
$_LANG['transferdomain'] = "დომეინის ტრანსფერი";
$_LANG['transferdomaindesc'] = "Want to move your domain to us? If so, enter your domain below to begin.";
$_LANG['transferdomainname'] = "Transfer a Domain Name";
$_LANG['updatecart'] = "Update Cart";
$_LANG['upgradechooseconfigoptions'] = "გაზარდეთ ან შეამცირეთ აღნიშნული პროდუქტის მონაცემები.";
$_LANG['upgradechoosepackage'] = "Choose the package you want to upgrade/downgrade your current package to from the options below.";
$_LANG['upgradecurrentconfig'] = "მიმდინარე კონფიგურაცია";
$_LANG['upgradedowngradeconfigoptions'] = "პარამეტრების გაზრდა/შემცირება";
$_LANG['upgradenewconfig'] = "ახალი კონფიგურაცია";
$_LANG['upgradenochange'] = "დარჩეს იგივე";
$_LANG['upgradeproductlogic'] = "Upgrade price is calculated from a credit of the unused portion of the current plan and billing of the new plan for the same period";
$_LANG['upgradesummary'] = "Below is a summary of your upgrade order.";
$_LANG['usedefaultcontact'] = "გამოიყენეთ ნაგულისხმევი კონტაქტი(დეტალები ზემოთ)";
$_LANG['varilogixfraudcall_callnow'] = "Call Now!";
$_LANG['varilogixfraudcall_description'] = "As part of our fraud prevention measures, we will now call the phone number registered for your account and ask you to enter the above pin code. Please make a note of the pin code and when you are ready for us to place the phone call, please click on the button below.";
$_LANG['varilogixfraudcall_error'] = "An error occurred and we could not call your phone number to verify your order. Please contact our support department as soon as possible to complete your order.";
$_LANG['varilogixfraudcall_fail'] = "The call to verify your order failed. This could be because your phone number was incorrectly entered or is blacklisted on our system. Please contact our support department as soon as possible to complete your order.";
$_LANG['varilogixfraudcall_failed'] = "Failed";
$_LANG['varilogixfraudcall_pincode'] = "Pin კოდი";
$_LANG['varilogixfraudcall_title'] = "VariLogix FraudCall";
$_LANG['viewcart'] = "კალათის ნახვა";
$_LANG['welcomeback'] = "მოგესალმებით";
$_LANG['whoisresults'] = "WHOIS Results for";
$_LANG['yes'] = "დიახ";
$_LANG['yourdetails'] = "თქვენი მონაცემები";

# Version 4.1

$_LANG['clientareafiles'] = "მიბმული ფაილები";
$_LANG['clientareafilesdate'] = "მიბმის თარიღი";
$_LANG['clientareafilesfilename'] = "ფაილის სახელწოდება";

$_LANG['pwreset'] = "პაროლის აღდგენა";
$_LANG['pwresetdesc'] = "თუკი პაროლი დაგავიწყდათ - შეგიძლია აქვე აღადგინოთ. როდესაც მიუთითებთ ელ.ფოსტას, რომლითაც დარეგისტრირდით (და უპასუხებთ საიდუმლო კითხვას), გამოგეგზავნებათ ინსტრუქციები თუ როგორ უნდა აღადგინოთ პაროლი.";
$_LANG['pwresetemailrequired'] = "გთხოვთ შეიყვანეთ თქვენი ელ.ფოსტა";
$_LANG['pwresetemailnotfound'] = "მითითებულ მოსამართზე მომხმარებელი არ მოიძებნა";
$_LANG['pwresetsecurityquestionrequired'] = "ქვემოთ, შეგიძლიათ საიდუმლო კითხვას გასცეთ პასუხი.";
$_LANG['pwresetsecurityquestionincorrect'] = "თქვენი პასუხი არ ემთხვევა ამ მომხმარებლისავე მითითებულს";
$_LANG['pwresetsubmit'] = "დადასტურება";
$_LANG['pwresetvalidationsent'] = "სავერიფიკაციო წერილი ელ.ფოსტაზე გამოიგზავნა";
$_LANG['pwresetvalidationcheckemail'] = "პაროლის აღდგენის პროცესი დაწყებულია. შეამოწმეთ ელ.ფოსტა და იხილეთ რისი გაკეთებაღა დაგრჩათ.";
$_LANG['pwresetkeyinvalid'] = "პაროლის აღსადგენი ლინკი არასწორია. სცადეთ კიდევე ერთხელ.";
$_LANG['pwresetkeyexpired'] = "პაროლის აღსადგენი ლინკს ვადა გაუვიდა. სცადეთ კიდევ ერთხელ.";
$_LANG['pwresetvalidationsuccess'] = "პაროლი წარმატებით აღდგა";

$_LANG['overagescharges'] = "Overage Charge";
$_LANG['overagestotaldiskusage'] = "გამოყენებული სივრცის ჯამი";
$_LANG['overagestotalbwusage'] = "გამოყენებული ტრაფიკის ჯამი";

$_LANG['affiliatescommissionspending'] = "დასარიცხად მოსალოდნელი თანხა";
$_LANG['affiliatescommissionsavailable'] = "ანგარიშზე დარიცხული თანხა";
$_LANG['affiliatessignups'] = "რეგისტრაციების რაოდენობა";
$_LANG['affiliatesconversionrate'] = "კონვერსია";

$_LANG['configoptionqtyminmax'] = "%s has a minimum requirement of %s and maximum of %s";

$_LANG['creditcardnostore'] = "Tick this box if you do NOT want us to store your credit card details for recurring billing";
$_LANG['creditcarddelete'] = "Delete Saved Card Details";
$_LANG['creditcarddeleteconfirmation'] = "The stored credit card details have now been removed from your account";
$_LANG['creditcardupdatenotpossible'] = "Credit Card Details cannot be updated at the current time. Please try again later.";

$_LANG['invoicepaymentsuccessconfirmation'] = "Thank You! Your payment was successful.";
$_LANG['invoicepaymentfailedconfirmation'] = "Unfortunately your payment attempt was not successful.<br />Please try again or contact support.";

# Version 4.2

$_LANG['promoappliedbutnodiscount'] = "The promotion code you entered has been applied to your cart but no items qualify for the discount yet - please check the promotion terms";

$_LANG['upgradeerroroverdueinvoice'] = "You cannot currently upgrade or downgrade this product because an invoice has already been generated for the next renewal.<br /><br />To proceed, please first pay the outstanding invoice and then you will be able to upgrade or downgrade immediately following that and be charged the difference or credited as appropriate.";
$_LANG['upgradeexistingupgradeinvoice'] = "ამ პროდუქტის განახლებას ვერ შეცვლით, რადგან ის უკვე განახლების პროცესშია. <br /> <br /> გთხოვთ, ჯერ გადაიხადოთ ინვოისი ცვლილებისთვის, შემდეგ შეგიძლიათ შეცვალოთ პროდუქტი. <br/> <br/> გთხოვთ, დაუკავშირდეთ დახმარებას, თუ თვლით, რომ შეცდომა მოხდა.";

$_LANG['subaccountactivate'] = "ქვე-ანგარიშის გააქტიურება";
$_LANG['subaccountactivatedesc'] = "მონიშნეთ ქვე-ანგარიშის მოსაწყობად";
$_LANG['subaccountpermissions'] = "ქვე-ანგარიშის უფლებები";
$_LANG['subaccountpermsprofile'] = "ანგარიშის მფლობელის მონაცემების შეცვლა.";
$_LANG['subaccountpermscontacts'] = "კონტაქტების ნახვა და მართვა";
$_LANG['subaccountpermsproducts'] = "პროდუქტებისა და სერვისების ნახვა";
$_LANG['subaccountpermsmanageproducts'] = "პროდუქტების პაროლის ნახვა და რედაქტირება.";
$_LANG['subaccountpermsdomains'] = "დომეინების ნახვა";
$_LANG['subaccountpermsmanagedomains'] = "დომეინის პარამეტრების მართვა";
$_LANG['subaccountpermsinvoices'] = "ინვოისების ნახვა და გადახდა.";
$_LANG['subaccountpermstickets'] = "დახმარების ბილეთების ნახვა და გახსნა.";
$_LANG['subaccountpermsaffiliates'] = "აფილეიტ ანგარიშის ნახვა და მართვა.";
$_LANG['subaccountpermsemails'] = "ელ.ფოსტის ნახვა";
$_LANG['subaccountpermsorders'] = "მორიგი შეკვეთების/გაუმჯობესების/გაუქმების მოთხოვნის უფლება.";
$_LANG['subaccountpermissiondenied'] = "თქვენ არ გაქვთ ამ გვერდის ნახვის უფლება.";
$_LANG['subaccountallowedperms'] = "თქვენ გაქვთ შემდეგი გვერდების ნახვის უფლება:";
$_LANG['subaccountcontactmaster'] = "დაუკავშირდით ანგარიშის მფლობელს, თუკი თვლით, რომ ეს შეცდომაა. ";
$_LANG['subaccountSsoDenied'] = "თქვენ არ გაქვთ უფლება გაიაროთ ავტორიზაცია Single Sign-On-ით.";

$_LANG['knowledgebasealsoread'] = "აგრეთვე ნახეთ";

$_LANG['orderpaymenttermtriennially'] = "ყოველ სამ წელში";
$_LANG['orderpaymentterm36month'] = "36 თვის ღირებულება";

$_LANG['domainrenewals'] = "დომეინის გახანგრძლივება";
$_LANG['domaindaysuntilexpiry'] = "ვადის გასვლამდე დარჩა";
$_LANG['domainrenewalsnoneavailable'] = "ამ ანგარიშზე დომეინები გასახანგრძლივებლად არ არის";
$_LANG['domainrenewalspastgraceperiod'] = "Past Renewable Period";
$_LANG['domainrenewalsingraceperiod'] = "Last Chance to Renew!";
$_LANG['domainrenewalsdays'] = "დღე";
$_LANG['domainrenewalsdaysago'] = "დღის წინ";

$_LANG['invoicespartialpayments'] = "Partial Payments";
$_LANG['invoicestotaldue'] = "Total Due";

$_LANG['masspaytitle'] = "სრულად გადახდა";
$_LANG['masspaydescription'] = "ქვემოთ მოცემულია მონიშნული ინვოისების გადასახადის ჯამი, რომლის გადახდაც გსურთ. გთხოვთ არიჩიოთ გადახდის სასურველი გზა და დააჭირეთ დასტურის ღილაკს.";
$_LANG['masspayselected'] = "არჩეულის გადახდა";
$_LANG['masspayall'] = "გადახდა";
$_LANG['masspaymakepayment'] = "გადახდა";

# Version 4.3

$_LANG['searchenterdomain'] = "მოსაძებნად შეიყვანეთ დომეინი";
$_LANG['searchfilter'] = "ფილტრი";

$_LANG['suspendreason'] = "Suspension Reason";
$_LANG['suspendreasonoverdue'] = "Overdue on Payment";

$_LANG['vpsnetmanagement'] = "VPS-ის მართვა";
$_LANG['vpsnetpowermanagement'] = "Power Management";
$_LANG['poweron'] = "ჩართვა";
$_LANG['poweroffforced'] = "გამორთვა (იძულებით)";
$_LANG['powerreboot'] = "გადატვირთვა";
$_LANG['powershutdown'] = "გამორთვა";
$_LANG['vpsnetcpugraphs'] = "CPU სტატისტიკა";
$_LANG['vpsnetnetworkgraphs'] = "ქსელური სტატისტიკა";
$_LANG['vpsnethourly'] = "საათობრივი";
$_LANG['vpsnetdaily'] = "დღიური";
$_LANG['vpsnetweekly'] = "კვირეული";
$_LANG['vpsnetmonthly'] = "თვიური";
$_LANG['view'] = "ნახვა";
$_LANG['vpsnetbackups'] = "Backup Options";
$_LANG['vpsnetgenbackup'] = "Generate Backup";
$_LANG['vpsnetrestorebackup'] = "Restore Backup";
$_LANG['vpsnetrestorebackupwarning'] = "Restoring the backup will over write your VPS server";
$_LANG['vpsnetnobackups'] = "There are no backups";
$_LANG['vpsnetrunning'] = "Running";
$_LANG['vpsnetnotrunning'] = "Not Running";
$_LANG['vpsnetpowercycling'] = "Power is cycling";
$_LANG['vpsnetcloud'] = "Cloud";
$_LANG['vpsnettemplate'] = "Template";
$_LANG['vpsnetstatus'] = "System Status";
$_LANG['vpsnetbwusage'] = "Bandwidth Usage";

$_LANG['twitterlatesttweets'] = "Our Latest Tweets";
$_LANG['twitterfollow'] = "Follow Us on Twitter";
$_LANG['twitterfollowus'] = "Follow us";
$_LANG['twitterfollowuswhy'] = "to stay up to date with our latest news &amp; offers";

$_LANG['chatlivehelp'] = "Live ჩატი";

$_LANG['domainrelease'] = "Release Domain";
$_LANG['domainreleasedescription'] = "Enter a new TAG here to move your domain name to another registrar";
$_LANG['domainreleasetag'] = "New Registrar Tag";

# Ajax Order Form

$_LANG['orderformtitle'] = "შეკვეთის ფორმა";

$_LANG['signup'] = "რეგისტრაცია";
$_LANG['loading'] = "დაელოდეთ...";

$_LANG['ordersummarybegin'] = "დასაწყებად არიჩიეთ რომელიმე სერვისი";

$_LANG['cartchooseproduct'] = "სერვისის არჩევა";
$_LANG['cartconfigurationoptions'] = "Configuration Options";

$_LANG['ordererrorsoccurred'] = "The following errors occurred and must be corrected before checkout:";
$_LANG['ordererrortermsofservice'] = "The Terms of Service must be agreed to";
$_LANG['ordertostickconfirm'] = "Please tick to confirm you agree to the";

$_LANG['cartnewcustomer'] = "ახალი მომხმარებელი";
$_LANG['cartexistingcustomer'] = "არსებული მომხმარებელი";

$_LANG['cartpromo'] = "ფასდაკლება";
$_LANG['cartenterpromo'] = "შეიყვანეთ ფასდაკლების კოდი";
$_LANG['cartremovepromo'] = "ფასდაკლების მოხსნა";

$_LANG['cartrecurringcharges'] = "Recurring Charges";

$_LANG['cartenterdomain'] = "Please enter the domain you would like to use below.";

$_LANG['cartdomainavailableoptions'] = "გილოცავთ, ეს დომეინი ხელმისაწვდომია!";
$_LANG['cartdomainavailableregister'] = "Please register this domain for";
$_LANG['cartdomainavailablemanual'] = "მოგვიანებით, დამოუკიდებლად დავარეგისტრირებ.";

$_LANG['cartdomainunavailableoptions'] = "ვწუხვართ, ეს დომეინი დაკავებულია. თუ თქვენ თავად ხართ ამ დომეინის მფლობელი, აირჩიეთ ქვემოთ მოცემული მითითებებიდან ერთ-ერთი...";
$_LANG['cartdomainunavailabletransfer'] = "Please transfer my domain for";
$_LANG['cartdomainunavailablemanual'] = "მე უკვე ვფლობ ამ დომეინს და განვაახლებ nameserver-ებს.";

$_LANG['cartdomaininvalid'] = "თქვენ არასწორად შეიყვანეთ დომეინი. შეიყვანეთ მხოლოდ www.-ს შემდგომი ნაწილი, დაბოლოების გარეშე(.com, .net, .org)";

# Version 4.4

$_LANG['dlinvalidlink'] = "Invalid Link Followed. Please Contact Support";

$_LANG['domaindnsmanagementlaunch'] = "Launch DNS Manager";
$_LANG['domainemailforwardinglaunch'] = "Launch Mail Forwarding Manager";

# Version 4.5

$_LANG['domaindnspriority'] = "პრიორიტეტი";
$_LANG['domaindnsmxonly'] = "Priority Record for MX Only";

$_LANG['orderpromoprestart'] = "This promotion has not yet started. Please try again later.";

$_LANG['ticketmerge'] = "MERGED";

$_LANG['quote'] = "შეთავაზება";
$_LANG['quotestitle'] = "ჩემი შეთავაზებები";
$_LANG['quoteview'] = "ნახვა";
$_LANG['quotedownload'] = "ჩამოტვირთვა";
$_LANG['quoteacceptbtn'] = "შეთავაზების მიღება";
$_LANG['quotedlpdfbtn'] = "PDF ჩამოტვირთვა";
$_LANG['quotediscountheading'] = "ფასდაკლება (%)";
$_LANG['noquotes'] = "ამჟამად თქვენს ანგარიშზე არაა არცერთი შეთავაზება.<br />შეთავაზების მისაღებად გთხოვთ გახსნათ დახმარების ბილეთი.";
$_LANG['quotenumber'] = "შეთავაზება #";
$_LANG['quotesubject'] = "თემა";
$_LANG['quotedatecreated'] = "შექმნის დრო";
$_LANG['quotevaliduntil'] = "მოქმედების სასრული";
$_LANG['quotestage'] = "ეტაპი";
$_LANG['quoterecipient'] = "Recipient";
$_LANG['quoteqty'] = "რდნ";
$_LANG['quotedesc'] = "აღწერა";
$_LANG['quoteunitprice'] = "ერთეულის ღირებულება";
$_LANG['quotediscount'] = "ფასდაკლება %";
$_LANG['quotelinetotal'] = "სულ";
$_LANG['quotestagedraft'] = "Draft";
$_LANG['quotestagedelivered'] = "მოწოდებული";
$_LANG['quotestageonhold'] = "შეჩერებული";
$_LANG['quotestageaccepted'] = "დათანხმებული";
$_LANG['quotestagelost'] = "ვადაგასული";
$_LANG['quotestagedead'] = "ვადაგასული";
$_LANG['quoteref'] = "Re Quote #";
$_LANG['quotedeposit'] = "Deposit";
$_LANG['quotefinalpayment'] = "Balance from Deposit";

$_LANG['invoiceoneoffpayment'] = "ერთჯერადი გადახდა";
$_LANG['invoicesubscriptionpayment'] = "ავტომატური გადახდა";

$_LANG['invoicepaymentpendingreview'] = "მადლობა! Your payment was successful and will be applied to your invoice as soon as 2CheckOut's Review Process has completed.<br /><br />This can take up to a few hours so your patience is appreciated.";

$_LANG['step'] = "Step %s";
$_LANG['cartdomainexists'] = "ჩვენს ბაზაში ეს დომეინი უკვე არსებობს, ამიტომაც მას ვეღარ შეუკვეთავთ.";
$_LANG['cartcongratsdomainavailable'] = "გილოცავთ, %s თავისუფალია!";
$_LANG['cartregisterhowlong'] = "How long do you want to register this for?";
$_LANG['cartdomaintaken'] = "სამწუხაროდ, %s უკვე დაკავებულია";
$_LANG['carttransfernotregistered'] = "%s ჯერაც არ არის რეგისტრირებული";
$_LANG['carttransferpossible'] = "გილოცავთ, %s დომეინის ტრანსფერი დასრულდა, რისთვისაც გადასახდელია %s";
$_LANG['cartotherdomainsuggestions'] = "სხვა დომეინები, რომლებლმაც შეიძლება დაგაინტერესოთ...";
$_LANG['cartdomainsconfiginfo'] = "The following options and settings are available for the domains you have chosen. Required fields are indicated with a *.";
$_LANG['cartnameserverchoice'] = "Nameserver-ების არჩევა";
$_LANG['cartnameserverchoicedefault'] = "გამოიყენეთ ნაგულისხმები nameserver-ები თქვენი ჰოსტინგისთვის";
$_LANG['cartnameserverchoicecustom'] = "გამოიყენეთ საკუთარი nameserver-ები";
$_LANG['cartfollowingaddonsavailable'] = "The following addons are available for your active products & services.";
$_LANG['cartregisterdomainchoice'] = "დომეინის რეგისტრირება";
$_LANG['carttransferdomainchoice'] = "გააკეთეთ დომეინის ტრანსფერი სხვა რეგისტრატორიდან ჩვენთან";
$_LANG['cartexistingdomainchoice'] = "ვაპირებ ჩემივე დომეინის გამოყენებასა და შემდგომ nameserver-ების განახლებას";
$_LANG['cartsubdomainchoice'] = "Use a subdomain from %s";
$_LANG['carterrordomainconfigskipped'] = "დაბრუნდეთ უკან და შეავსეთ დომეინის საკონფიგურაციო ველები.";
$_LANG['cartproductchooseoptions'] = "არჩევის პარამეტრები";
$_LANG['cartproductselection'] = "Product Selection";
$_LANG['cartreviewcheckout'] = "განხილვა და გადახდა";
$_LANG['cartchoosecycle'] = "აირჩიეთ გადახდის ციკლი";
$_LANG['cartavailableaddons'] = "ხელმისაწვდომი დამ. პროდუქტები";
$_LANG['cartsetupfees'] = "გამართვის ღირებულება";
$_LANG['cartchooseanotherproduct'] = "აირჩიეთ სხვა პროდუქტი";
$_LANG['cartaddandcheckout'] = "ბარათში დაამატე და გადაიხადე";
$_LANG['cartchooseanothercategory'] = "აირჩიეთ სერვისების სხვა კატეგორია";
$_LANG['carttryanotherdomain'] = "სცადეთ სხვა დომეინი";
$_LANG['cartmakedomainselection'] = "Please provide us with the domain you want to use with your hosting service by selecting an option from the selections below.";
$_LANG['cartfraudcheck'] = "Fraud Check";

$_LANG['newcustomer'] = "ახალი მომხმარებელი";
$_LANG['existingcustomer'] = "არსებული მომხარებელი";
$_LANG['newcustomersignup'] = "<strong>არ ხართ რეგისტირრებული?</strong> %sდააჭირეთ დასარეგისტრირებლად...%s";

$_LANG['upgradeonselectedoptions'] = "(On Selected Options)";
$_LANG['recurringpromodesc'] = "This promotion code also includes a %s Recurring Discount<br />(This discount will apply to future renewals of the product's total price)";

# Version 4.5.2

$_LANG['ajaxcartcheckout'] = "გადადით პირდაპირ გადახდაზე &raquo;";
$_LANG['ordersummarybegin'] = "კალათა ცარიელია<br/>Please choose a product and domain name option to begin...";
$_LANG['ajaxcartconfigreqnotice'] = "You're on the way to signing up with us, but you must choose a domain before you can add the selected product to your cart...";

# Version 5.0.0

$_LANG['cancelrequestdomain'] = "გავაუქმოთ დომეინის გახანგრძლივების პროცესი?";
$_LANG['cancelrequestdomaindesc'] = "You also have an active domain registration for the domain associated with this product<br />This domain is due to renew on %s at a cost of %s for %s Year/s<br /><br />If you would like to cancel the domain as well, and let it expire at the end of the current registration, then simply tick the box below.";
$_LANG['cancelrequestdomainconfirm'] = "ვადასტურებ, რომ აღარ მსურს ამ დომეინის გახანგრძლივება.";

$_LANG['startingfrom'] = "Starting from";

$_LANG['orderpromopriceoverride'] = "Price Override";
$_LANG['orderpromofreesetup'] = "უფასო დაყენება";

$_LANG['thereisaproblem'] = "უუპს, ხარვეზთან გვაქვს საქმე...";
$_LANG['problemgoback'] = "დაბრუნდით უკან და სცადეთ კიდევ ერთხელ";

$_LANG['quantity'] = "რაოდენობა";
$_LANG['cartqtyenterquantity'] = "გსრუთ ერთზე მეტი?? შეიყვანეთ რაოდენობა აქ:";
$_LANG['cartqtyupdate'] = "განახლება";
$_LANG['invoiceqtyeach'] = "/თითო";

$_LANG['nschoicedefault'] = "გამოიყენეთ ნაგულისხმები nameserver-ები";
$_LANG['nschoicecustom'] = "გამოიყენეთ საკუთარი nameserver-ები (მიუთითეთ ქვემოთ)";

$_LANG['jumpto'] = "გადასვლა";
$_LANG['top'] = "მაღლა";

$_LANG['domaincontactusexisting'] = "გამოიყენოთ არსებული ანგარიშის კონტაქტი";
$_LANG['domaincontactusecustom'] = "მიუთეთ სასურველი ინფორმაცია";
$_LANG['domaincontactchoose'] = "აირჩეთ კონტაქტი";
$_LANG['domaincontactprimary'] = "Primary Profile Data";

$_LANG['invoicepdfgenerated'] = "PDF გენერაცია ჩართულია";

$_LANG['domainrenewalsbeforerenewlimit'] = "Minimum Advance Renewal is %s Days";

$_LANG['promonewsignupsonly'] = "მოცემული ფასდაკლების კოდი მოქმედია მხოლოდ ახალი მომხმარებლებისთვის.";

# Bulk Domain Management

$_LANG['domainbulkmanagement'] = "დომეინების მასობრივი მართვა";
$_LANG['domainbulkmanagementchangesaffect'] = "გაკეთებული ცვილილებები განხორციელდება ქვემოთ მოცემულ დომეინებზე:";
$_LANG['domainbulkmanagementchangeaffect'] = "გაკეთებული ცვლილება განხორციელდება ქვემოთ მოცემულ დომეინებზე:";
$_LANG['domaincannotbemanaged'] = "ავტომატური მართვა შეუძლებელია - ნებისმიერ ცვლილებასთან დაკავშირებით დაუკავშირდით დახმარების ცენტრს";
$_LANG['domainbulkmanagementnotpossible'] = "სამწუხაროდ, მოცემული პარამეტრების შეცვლა მომხმარებლის არედან შეუძლებელია.";

$_LANG['domainmanagens'] = "Nameserver-ების მართვა";

$_LANG['domainautorenewstatus'] = "ავტომატური გახანგრძლივების სტატუსი";
$_LANG['domainautorenewinfo'] = "ავტომატური გახანგრძლივება იცავს  დომეინს. როდესაც ის აქტიურია, დომეინის გაუქმებამდე კვირებით ადრე გამოგიგზავნით ინვოისს, რომლის გადახდაც საშუალებას მოგცემთ დომეინის გაახანგრძლივოთ. ინვოისის გადახდა კი მოხდება ავტომატურად.";
$_LANG['domainautorenewrecommend'] = "გირჩევთ, რომ ჩართოთ ეს ფუნქცია, რათა თავიდან აირიდოთ დომეინის დაკარგვა.";

$_LANG['domainreglockstatus'] = "რეგისტრანტის ბლოკის სტატუსი";
$_LANG['domainreglockinfo'] = "რეგისტრანტის ბლოკი (ცნობილი როგორც \"Theft Protection\") იცავს თქვენს დომეინს არა ავტორიზებული ტრანსფერებისგან.";
$_LANG['domainreglockrecommend'] = "ჩვენი გირჩევთ ჩართოთ რეგისტრანტის ბლოკი, except when transferring your Domain Name away.";
$_LANG['domainreglockenable'] = "რეგისტრანტის ბლოკის ჩართვა";
$_LANG['domainreglockdisable'] = "რეგისტრანტის ბლოკის გამორთვა";

$_LANG['domaincontactinfoedit'] = "საკონტაქტო ინფორმაციის შეცვლა";

$_LANG['domainmassrenew'] = "დომეინის გახანგრძლივება";

# reCAPTCHA

$_LANG['captchatitle'] = "ვერიფიკაცია";
$_LANG['captchaverify'] = "შეიყვანეთ ქვემოთ მოცემულ სურათზე გამოსახული სიმბოლოები..";
$_LANG['captchaverifyincorrect'] = "თქვენ მიერ შეყვანილი სიმბოლოები სურათს არ ემთხვევა. სცადეთ კიდევ ერთხელ.";
$_LANG['googleRecaptchaIncorrect'] = "გაიარეთ captcha და სცადეთ კიდევ ერთხელ.";
$_LANG['recaptcha-invalid-site-private-key'] = "დაფიქსირდა შეცდომა, გამოიყენეთ დახმარების განყოფილება (შეცდომის კოდი: cap1)";
$_LANG['recaptcha-invalid-request-cookie'] = "დაფიქსირდა შეცდომა, სცადეთ კიდევ ერთხელ (შეცდომის კოდი: cap2)";
$_LANG['recaptcha-incorrect-captcha-sol'] = "თქვენ მიერ შეყვანილი სიმბოლოები არ ემთხვევა სავერიფიკაციო სიტყვას. სცადეთ კიდევ ერთხელ.";

# Product Bundles

$_LANG['bundledeal'] = "Bundle Deal!";
$_LANG['bundlevaliddateserror'] = "Bundle Unavailable";
$_LANG['bundlevaliddateserrordesc'] = "This bundle is either not yet active or has expired. If you feel this message to be an error, please contact support.";
$_LANG['bundlemaxusesreached'] = "Bundle Unavailable";
$_LANG['bundlemaxusesreacheddesc'] = "This bundle offer has reached the maximum number of uses allowed and so unfortunately is no longer available. Please contact us if you you're interested in our services to discuss.";
$_LANG['bundlereqsnotmet'] = "Bundle Requirements Not Met";
$_LANG['bundlewarningpromo'] = "The selected bundle cannot be used in conjunction with any other promotions or offers";
$_LANG['bundlewarningproductcycle'] = "The selected bundle requires you choose the billing cycle '%s' for product %s to qualify";
$_LANG['bundlewarningproductconfopreq'] = "The selected bundle requires you select '%s' for '%s' in order to qualify";
$_LANG['bundlewarningproductconfopyesnoenable'] = "The selected bundle requires you enable the option '%s' in order to qualify";
$_LANG['bundlewarningproductconfopyesnodisable'] = "The selected bundle requires you deselect the option '%s' in order to qualify";
$_LANG['bundlewarningproductconfopqtyreq'] = "The selected bundle requires you choose a quantity of '%s' for '%s' in order to qualify";
$_LANG['bundlewarningproductaddonreq'] = "The selected bundle requires you select the addon '%s' for product %s to qualify";
$_LANG['bundlewarningdomainreq'] = "The selected bundle requires you register or transfer a domain with the product %s to qualify";
$_LANG['bundlewarningdomaintld'] = "The selected bundle requires you choose a domain with the extension(s) '%s' for domain %s to qualify";
$_LANG['bundlewarningdomainregperiod'] = "The selected bundle requires you select the registration period '%s' for domain %s to qualify";
$_LANG['bundlewarningdomainaddon'] = "The selected bundle requires you select the addon '%s' for domain %s to qualify";

# New Client Area Template  Lines

$_LANG['navservices'] = "სერვისები";
$_LANG['navservicesorder'] = "ახალი შეკვეთა";
$_LANG['navservicesplaceorder'] = "ახალი შეკვეთა";
$_LANG['navdomains'] = "დომეინები";
$_LANG['navrenewdomains'] = "დომეინის გახანგრძლივება";
$_LANG['navregisterdomain'] = "დომეინის დარეგისტრირება";
$_LANG['navtransferdomain'] = "დომეინის ჩვენთან გადმოტანა";
$_LANG['navdomainsearch'] = "დომეინის ძებნა";
$_LANG['navbilling'] = "გადახდები";
$_LANG['navinvoices'] = "ინვოისები";
$_LANG['navsupport'] = "დახმარება";
$_LANG['navtickets'] = "ბილეთები";
$_LANG['navopenticket'] = "ახალი ბილეთის გახსნა";
$_LANG['navmanagecc'] = "საკრედიტო ბარათის მართვა";
$_LANG['navemailssent'] = "საფოსტო ყუთის ისტორია";

$_LANG['hello'] = "სალამი";
$_LANG['helloname'] = "სალამი, %s!";
$_LANG['account'] = "ანგარიში";
$_LANG['login'] = "შესვლა";
$_LANG['register'] = "რეგისტრაცია";
$_LANG['forgotpw'] = "დაგავიწყდათ პაროლი?";
$_LANG['editaccountdetails'] = "ანგარიშის მონაცემების ჩასწორება";

$_LANG['clientareanavccdetails'] = "საკრედიტო ბარაფ";
$_LANG['clientareanavcontacts'] = "კონტაქტები/ქვე-ანგარიშები";

$_LANG['manageyouraccount'] = "ანგარიშის მართვა";
$_LANG['accountoverview'] = "ანგარიშის შეჯამება";
$_LANG['paymentmethod'] = "გადახდის მეთოდი";
$_LANG['paymentmethoddefault'] = "გამოიყენეთ ნაგულისხმევი (თითოეული შეკვეთისას)";
$_LANG['productmanagementactions'] = "Management Actions";
$_LANG['clientareanoaddons'] = "No Addons Purchased Yet";
$_LANG['downloadssearch'] = "Search Downloads";
$_LANG['emailviewmessage'] = "წერილის ნახვა";
$_LANG['resultsperpage'] = "შედეგების რაოდ. გვერდზე";
$_LANG['accessdenied'] = "წვდომა შეზღუდულია";
$_LANG['search'] = "ძებნა";
$_LANG['cancel'] = "გაუქმება";
$_LANG['clientareabacklink'] = "&laquo; უკან დაბრუნება";
$_LANG['backtoserviceslist'] = "&laquo; დაბრუნდით სერვისების სიის გვერდზე";
$_LANG['backtodomainslist'] = "&laquo; დაბრუნდით დომეინების სიის გვერდზე";

$_LANG['clientareahomeorder'] = "ეწვიეთ შეკვეთის ფორმას, რათა იხილოთ ის პროდუქტები და სერვისები, რომლებსაც გთავაზობთ. არსებულ მომხმარებლებს შეუძლიათ დამატებით პროდუქტების/მომსახურეობის შეძენაც.";
$_LANG['clientareahomelogin'] = "უკვე რეგისტრირებული ხართ? თუ ასეა, დააჭირეთ ქვემოთ მოცემულ ღილაკს, გაიარეთ ავტორიზაცია და ისარგებლეთ მომხმარებლის უპირატესობებით.";
$_LANG['clientareahomeorderbtn'] = "შეკვეთის ფორმაზე გადასვლა";
$_LANG['clientareahomeloginbtn'] = "მომხმარებლის დაცული ავტორიზაცია";

$_LANG['clientareaproductsintro'] = "These are all the services you have registered in this account.";
$_LANG['clientareaproductdetailsintro'] = "Here is an overview of your product/service with us.";
$_LANG['clientareadomainsintro'] = "View &amp; manage all the domains you have registered with us from here...";
$_LANG['invoicesintro'] = "Below you can review your entire invoice history with us.";
$_LANG['quotesintro'] = "Here are all the quotes we've generated for you.";
$_LANG['emailstagline'] = "Here's a copy of the recent emails we've sent you...";
$_LANG['supportticketsintro'] = "Submit and track any enquiries with us here...";
$_LANG['addfundsintro'] = "Deposit money in advance";
$_LANG['registerintro'] = "გახდით ჩვენი წევრი . . .";
$_LANG['masspayintro'] = "Pay all the invoices listed below in a single easy transaction by choosing a payment method";
$_LANG['domaincheckerintro'] = "Start your web hosting search here by checking if your domain is available...";
$_LANG['networkstatusintro'] = "Service Status Information and Network Announcements";

$_LANG['creditcardyourinfo'] = "ინფომრაცია გადახდებზე";
$_LANG['ourlatestnews'] = "ჩვენი უახლესი სიახლეები";
$_LANG['ccexpiringsoon'] = "საკრედიტო ბარათს ვადა ეწურება";
$_LANG['ccexpiringsoondesc'] = "თქვენს საკრედიტო ბარათს ვადა ეწურება, ამიტომაც  %sგანაახლეთ თქვენი ბარათის მონაცემები%s, როცა მას განაახლებთ";
$_LANG['availcreditbal'] = "Available Credit Balance";
$_LANG['availcreditbaldesc'] = "You have a credit balance of %s and this will be automatically applied to any new invoices";
$_LANG['youhaveoverdueinvoices'] = "თქვენ გაქვთ %s გადაუხდელი ინვოისი";
$_LANG['overdueinvoicesdesc'] = "To avoid service interruption, please pay your outstanding invoices as soon as possible. %sPay Now &raquo;%s";
$_LANG['supportticketsnoneopen'] = "There are currently no open support tickets";
$_LANG['invoicesnoneunpaid'] = "There are currently no unpaid invoices";

$_LANG['registerdisablednotice'] = "To register please place an <strong><a href=\"cart.php\">order</a></strong>";
$_LANG['registerCreateAccount'] = "To create an account, please";
$_LANG['registerCreateAccountOrder'] = "place an order with us";

$_LANG['pwstrength'] = "პაროლის სირთულე";
$_LANG['pwstrengthenter'] = "მიუთითეთ პაროლი";
$_LANG['pwstrengthweak'] = "მარტივი";
$_LANG['pwstrengthmoderate'] = "საშუაალო";
$_LANG['pwstrengthstrong'] = "რთული";

$_LANG['managing'] = "მართვა";
$_LANG['information'] = "ინფორმაცია";
$_LANG['withselected'] = "With Selected";
$_LANG['managedomain'] = "დომეინის მართვა";
$_LANG['changenameservers'] = "NameServer შეცვლა";
$_LANG['clientareadomainmanagedns'] = "DNS მართვა";
$_LANG['clientareadomainmanageemailfwds'] = "Manage Email Forwards";
$_LANG['moduleactionsuccess'] = "ოპერაცია წარმატებით შესრულდა!";
$_LANG['moduleactionfailed'] = "ოპერაციის შესრულების დროს დაფიქსირდა შეცდომა: ";

$_LANG['domaininfoexp'] = "To the right you can find the details of your domain. You can manage your domain using the tabs above.";
$_LANG['domainrenewexp'] = "Enable auto renew to have us automatically send you a renewal invoice before your domain expires.";
$_LANG['domainnsexp'] = "You can change where your domain points to here. Please be aware changes can take up to 24 hours to propagate.";
$_LANG['domainlockingexp'] = "Lock your domain to prevent it from being transferred away without your authorization.";
$_LANG['domaincurrentlyunlocked'] = "Domain Currently Unlocked!";
$_LANG['domaincurrentlyunlockedexp'] = "You should enable the registrar lock unless you are transferring the domain.";
$_LANG['searchmultipletlds'] = "Search Multiple TLDs";

$_LANG['networkstatustitle'] = "ქსელის სტატუსი";
$_LANG['networkstatusnone'] = "ამჟამად არ არის %s ქსელური შეფერხებები";
$_LANG['serverstatusheadingtext'] = "Below is a real-time overview of our servers where you can check if there's any known issues.";

$_LANG['clientareacancelreasonrequired'] = "თქვენ უნდა შეიყვანოთ გაუქმების მიზეზები";

$_LANG['addfundsdescription'] = "დაამატეთ თანხა თქვენს ანგარიშზე, რომ თავიდან აირიდოთ მცირე ტრანზაქციები და ავტომატურად მოხდეს ახალი დაგენერირებული ინვოისების გადახდა.";
$_LANG['addfundsnonrefundable'] = "* ჩარიცხული თანხა არ უბრუნდება მფლობელს.";

$_LANG['creditcardexpirydateinvalid'] = "ვადის გასვლის თარიღი უნდა იყოს შემდეგ ფორმატში \"თვე/წელი\" და იგი ვერ იქნება წარსულ დროში.";

$_LANG['domaincheckerchoosedomain'] ="შეარჩიეთ დომეინი...";
$_LANG['domaincheckerchecknewdomain'] = "შეამოწმეთ დომეინის ხელმისაწვდომობა";
$_LANG['domaincheckerdomainexample'] = " მაგ. hostnodes.net";
$_LANG['domaincheckerinvalidtld'] = "დომეინი არ არის TLD. გთხოვთ, სცადოთ კიდევ ერთხელ.";
$_LANG['domaincheckerinvalidtld2'] = "TLD ვერიფიკაცია ვერ გაიარა.";
$_LANG['domaincheckerinvaliddomain'] = "მოთხოვნილი დომეინის სახელწოდება არასწორად არის შედგენილი.";
$_LANG['domaincheckernopricing'] = "ფასები ამ ტიპის TLD-თვის არ არის მოცემული.";
$_LANG['domaincheckerhostingonly'] = "შეუკვეთეთ მხოლოდ ჰოსტინგი";
$_LANG['domaincheckeravailtransfer'] = "შესაძლებელია ტრანსფერი";
$_LANG['domaincheckerenterdomain'] = "შეიძინეთ ვებ-ჰოსტინგის მართვის გამოცდილება იმ დომეინის მითითებით, რომელის რეგისტრაცია ან ტრანსფერიც გსურთ, ან უბრალოდ შეუკვეთე მხოლოდ ჰოსტინგი.";
$_LANG['domaincheckerbulkinvaliddomain'] = "ზემოთ მითითებული ერთი ან მეტი დომეინი, რომელიც არასწორად იყო შედგენილი ამოვარდა შედეგებიდან.";

$_LANG['kbquestionsearchere'] = "გაქვთ კითხვა? მოძებნეთ აქვე";
$_LANG['contactus'] = "დაგვიკავშირდით";

$_LANG['opennewticket'] = "ბილეთის გახსნა";
$_LANG['searchtickets'] = "შეიყვანეთ ბილეთის # ან თემა";
$_LANG['supportticketspriority'] = "მნიშვნელობა";
$_LANG['supportticketsubmitted'] = "Submitted";
$_LANG['supportticketscontact'] = "კონტაქტი";
$_LANG['supportticketsticketlastupdated'] = "ბოლოს განახლდა";

$_LANG['upgradedowngradepackage'] = " პაკეტის გაზრდა/შემცირება";
$_LANG['upgradedowngradechooseproduct'] = "მომსახურების არჩევა";

$_LANG['jobtitlereqforcompany'] = "(Required if Organization Name is set)";

$_LANG['downloadproductrequired'] = "Downloading this item requires you to have an active instance of the following product/service:";

$_LANG['affiliatesignuptitle'] = "გაგვიწიეთ რეკომენდაცია და მიიღეთ სარგებელი";
$_LANG['affiliatesignupintro'] = "გააქტიურეთ თქვენი აფილეიტ ანგარიში და დღესვე დაიწყეთ თანხის გამომუშავება";
$_LANG['affiliatesignupinfo1'] = "თქვე დაგერიცხებათ პროცენტი თითოეული კლიენტის გადახდილი თანხიდან.";
$_LANG['affiliatesignupinfo2'] = "იმ შემთხვევაშიც კი, თუკი მომხმარებელი ლინკზე გადასვლისთანავე არ იყიდის პროდუქტს, მაგრამ ლინკზე გადასვლიდან 30 დღის განმავლობაში შეიძენს მომსახურეობას - თქვენ მიიღებთ პროცენტს.";
$_LANG['affiliatesignupinfo3'] = "თუ მეტის გაგება გსურს - დაგვიკავშირდი.";

# Version 5.1

$_LANG['copyright'] = "უფლებები";
$_LANG['allrightsreserved'] = "ყველა უფლება დაცულია";
$_LANG['supportticketsclose'] = "დახურვა";
$_LANG['affiliatesinitialthen'] = "Initially then";
$_LANG['invoicesoutstandingbalance'] = "Outstanding Balance";

$_LANG['cpanellogin'] = "cPanel-ში შესვლა";
$_LANG['cpanelwhmlogin'] = "WHM-ში შესვლა";
$_LANG['cpanelwebmaillogin'] = "Webmail-ში შესვლა";
$_LANG['enkompasslogin'] = "Enkompass-ში შესვლა";
$_LANG['plesklogin'] = "Plesk-ის საკონტროლო პანელში შესვლა";
$_LANG['helmlogin'] = "Helm-ის საკონტროლო პანელში შესვლა";
$_LANG['hypervmrestart'] = "VPS სერვერის დარესტარტება";
$_LANG['siteworxlogin'] = "Login to SiteWorx Control Panel";
$_LANG['nodeworxlogin'] = "Login to NodeWorx Control Panel";
$_LANG['veportallogin'] = "vePortal-ში შესვლა";
$_LANG['virtualminlogin'] = "საკონტროლო პანელში შესვლა";
$_LANG['websitepanellogin'] = "საკონტროლო პანელში შესვლა";
$_LANG['whmsoniclogin'] = "საკონტროლო პანელში შესვლა";
$_LANG['xpanelmaillogin'] = "Webmail-ში შესვლა";
$_LANG['xpanellogin'] = "XPanel-ში შესვლა";
$_LANG['heartinternetlogin'] = "საკონტროლო პანელში შესვლა";
$_LANG['gamecplogin'] = "GameCP-ში შესვლა";
$_LANG['fluidvmrestart'] = "VPS სერვერის დარესტარტება";
$_LANG['enomtrustedesc'] = "The TRUSTe Control Panel contains the set up wizard to get your Privacy Policy up and running.";
$_LANG['enomtrustelogin'] = "TrustE-ს საკონტროლო პანელში შესვლა";
$_LANG['directadminlogin'] = "DirectAdmin-ში შესვლა";
$_LANG['centovacastlogin'] = "Centova Cast-ში შესვლა";
$_LANG['castcontrollogin'] = "საკონტროლო პანელში შესვლა ";

$_LANG['sslconfigurenow'] = "Configure Now";
$_LANG['sslprovisioningdate'] = "SSL Provisioning Date";
$_LANG['globalsignvoucherscode'] = "Your OneClickSSL Voucher Code";
$_LANG['globalsignvouchersnotissued'] = "Not Yet Issued";

$_LANG['domaintrffailreasonunavailable'] = "Failure Reason Unavailable";

$_LANG['clientareaprojects'] = "ჩემი პროექტები";

$_LANG['clientgroupdiscount'] = "Client Discount";
$_LANG['billableitemshours'] = "საათი";
$_LANG['billableitemshour'] = "საათი";

$_LANG['invoicefilename'] = "ინვოისი-";
$_LANG['quotefilename'] = "Quote-";

# Domain Addons

$_LANG['domainaddons'] = "Addons";
$_LANG['domainaddonsinfo'] = "The following addons are available for your domain(s)...";
$_LANG['domainaddonsdnsmanagement'] = "DNS Host Record Management";
$_LANG['domainaddonsidprotectioninfo'] = "დაიცავით პირადი ინფორმაცია და თავი აირიდეთ სპამური შეტყობინებისგან ID დაცვისის გააქტიურებით.";
$_LANG['domainaddonsdnsmanagementinfo'] = "გარე DNS ჰოსტინგს შეუძლია უფრო სწრაფი გახადოს თქვენი საიტი და ხელი შეუწყოს მის ხელმისაწვდომობას
.";
$_LANG['domainaddonsemailforwardinginfo'] = "გაადამისამართეთ წერილები სასურველ ელ.ფოსტაზე და აკონტროლეთ რამდენიმე ფოსტა ერთი ანგარიშით.";
$_LANG['domainaddonsbuynow'] = "Buy Now for";
$_LANG['domainaddonsperyear'] = "/Year";
$_LANG['domainaddonscancelareyousure'] = "Are you sure you want to disable & cancel this domain addon?";
$_LANG['domainaddonsconfirm'] = "Confirm Cancellation";
$_LANG['domainaddonscancelsuccess'] = "Addon Deactivated Successfully!";
$_LANG['domainaddonscancelfailed'] = "Failed to deactivate addon. Please contact support.";

# Version 5.2

$_LANG['yourclientareahostingaddons'] = "You have the following addons for this product.";
$_LANG['loginrequired'] = "მოითხოვება ავტორიზაცია";
$_LANG['unsubscribe'] = "გამოწერის გაუქმება";
$_LANG['emailoptout'] = "გამოწერისგან განთავისუფლება";
$_LANG['newsletterunsubscribe'] = "სიახლეების ლენტის გამოწერის გაუქმება";
$_LANG['emailoptoutdesc'] = "მონიშნეთ გამოწერის გასაუქმებლად";
$_LANG['alreadyunsubscribed'] = "თქვენ უკვე გაუქმებული გაქვთ გამოწერა.";
$_LANG['newsletterresubscribe'] = "If you wish to re-subscribe you can do so from the %sMy Details%s section of our client area at any time.";
$_LANG['unsubscribehashinvalid'] = "Unsubscribe failed, please contact support.";
$_LANG['unsubscribesuccess'] = "Unsubscribe Successful";
$_LANG['newsletterremoved'] = "Thank you, Your email has now been removed from our mailing list.";
$_LANG['erroroccured'] = "An Error Occurred";
$_LANG['pwresetsuccessdesc'] = "თქვენი პაროლი შეცვლილია. სამართავ პანელში გადასასვლელად %sდააჭირეთ აქ...%s";
$_LANG['pwresetenternewpw'] = "გთხოვთ, შეიყვანოთ თქვენი ახალი პაროლი.";
$_LANG['ordererrorsbudomainbanned'] = "The subdomain prefix you entered is not allowed - please try another";

$_LANG['ticketfeedbacktitle'] = "Feedback Request for Ticket";

$_LANG['nosupportdepartments'] = "დამხმარე განყოფილებები ვერ მოიძებნა. გთხოვთ სცადოთ მოგვიანებით.";

$_LANG['feedbackclosed'] = "Feedback cannot be provided until the ticket is closed";
$_LANG['feedbackprovided'] = "You have already provided feedback for this ticket";
$_LANG['feedbackthankyou'] = "We thank you for taking the time to provide your feedback.";
$_LANG['feedbackreceived'] = "Submission Received";
$_LANG['feedbackdesc'] = "Please can we ask you to take a moment of your time to fill out the below form about the quality of your experience with our support team.";
$_LANG['feedbackclickreview'] = "Click here to Review The Ticket";
$_LANG['feedbackopenedat'] = "გაიხსნა";
$_LANG['feedbacklastreplied'] = "Last Replied To";
$_LANG['feedbackstaffinvolved'] = "Staff Involved";
$_LANG['feedbacktotalduration'] = "Total Duration";
$_LANG['feedbackpleaserate1'] = "Please rate (on a scale of 1 to 10) how well";
$_LANG['feedbackpleasecomment1'] = "Please comment on how well";
$_LANG['feedbackhandled'] = "handled this support request";
$_LANG['feedbackworst'] = "საშინელი";
$_LANG['feedbackbest'] = "საუკეთესო";
$_LANG['feedbackimprove'] = "How may we make your experience better in the future?";
$_LANG['pleaserate2'] = "handled this support request";
$_LANG['returnclient'] = "მოხმარებლის არეში დაბრუნება";

$_LANG['clientareanavsecurity'] = "უსაფრთხოების პარამეტრები";
$_LANG['twofactorauth'] = "ორმაგი ავტორიზაცია";
$_LANG['twofaenable'] = "ორმაგი ავტორიზაციის ჩართვა";
$_LANG['twofadisable'] = "ორმაგი ავტორიზაციის გამორთვა";
$_LANG['twofaenableclickhere'] = "დააჭირეთ ჩასართავად";
$_LANG['twofadisableclickhere'] = "დააჭირეთ გამოსართავად";
$_LANG['twofaenforced'] = "For your security, we require that you must enable Two-Factor Authentication before you can continue. This page will guide you through the process of setting it up.";
$_LANG['twofasetup'] = "Two-Factor Authentication Setup Process";
$_LANG['twofasetupgetstarted'] = "Get Started";
$_LANG['twofaactivationintro'] = "Two-Factor Authentication adds an extra layer of protection to logins. Once enabled &amp; configured, each time you sign in you will be asked to enter both your username & password as well as a second factor such as a security code.";
$_LANG['twofaactivationmultichoice'] = "To continue, please choose your desired Two-Factor Authentication method from below.";
$_LANG['twofadisableintro'] = "To disable Two-Factor Authentication please confirm your password in the field below.";
$_LANG['twofaactivationerror'] = "An error occurred while attempting to activate Two-Factor Authentication for your account. Please try again.";
$_LANG['twofamoduleerror'] = "An error occurred loading the module. Please try again.";
$_LANG['twofaactivationcomplete'] = "Two-Factor Authentication Setup is Complete!";
$_LANG['twofadisableconfirmation'] = "Two-Factor Authentication has now been disabled for your account.";
$_LANG['twofabackupcodeis'] = "Your Backup Code is";
$_LANG['twofanewbackupcodeis'] = "Your New Backup Code is";
$_LANG['twofabackupcodelogin'] = "Enter Your Backup Code to Login";
$_LANG['twofabackupcodeexpl'] = "Write this down on paper and keep it safe.<br />It will be needed if you ever lose your 2nd factor device or it is unavailable to you.";
$_LANG['twofaconfirmpw'] = "Enter Your Password";
$_LANG['twofa2ndfactorreq'] = "Your second factor is required to complete login.";
$_LANG['twofa2ndfactorincorrect'] = "The second factor you supplied was incorrect. Please try again.";
$_LANG['twofabackupcodereset'] = "Login via Backup Code Successful<br />Backup Codes are valid once only. It will now be reset.";
$_LANG['twofacantaccess2ndfactor'] = "Can't Access Your 2nd Factor Device?";
$_LANG['twofaloginusingbackupcode'] = "Login using Backup Code";
$_LANG['twofageneralerror'] = "An error occurred loading the module. Please try again.";

$_LANG['continue'] = "გაგრძელება";
$_LANG['disable'] = "გამორთვა";
$_LANG['manage'] = "მართვა";

# Version 5.3
$_LANG['quoteacceptancetitle'] = "Quote Acceptance";
$_LANG['quoteacceptancehowto'] = "To accept the quote, please confirm your acceptance of our terms of service which can be viewed @";
$_LANG['quoteacceptancewarning'] = "Please be aware that accepting a quote is considered entering into a contract and you will not be able to cancel once accepted.";

$_LANG['contactform'] = "საკონტაქტო ფორმა";

$_LANG['twoipverificationstep'] = "Verification Step";
$_LANG['twoipverificationstepmsg'] = "Enter the security code generated by your mobile authenticator app and we'll make sure it's configured correctly before enabling it.";
$_LANG['twoipverificationerror'] = "It seem's there's a problem...";
$_LANG['twoipcodemissmatch'] = "The code you entered did not match what was expected. Please try again.";
$_LANG['twoiptimebasedpassword'] = "Time-based One-Time Password";
$_LANG['twoiptimebasedexplain'] = "This authentication option get's it's second factor using a time based algorithm.  Your mobile phone can be used to generate the codes.  If you don't already have an app that can do this, we recommend Google Authenticator which is available for iOS, Android and Windows mobile devices.";
$_LANG['twoipconfigureapp'] = "To configure your authenticator app:";
$_LANG['twoipconfigurestep1'] = "Begin by selecting to add a new time based token";
$_LANG['twoipconfigurestep2'] = "Then use your app to scan the barcode below, or alternatively enter this secret key manually: ";
$_LANG['twoipgdmissing'] = "GD is missing from the PHP build on your server so unable to generate image";

$_LANG['domaincontactdetails']['First Name'] = "სახელი";
$_LANG['domaincontactdetails']['Last Name'] = "გვარი";
$_LANG['domaincontactdetails']['Full Name'] = "სახელი და გვარი";
$_LANG['domaincontactdetails']['Contact Name'] = "საკონტაქტო სახელი";
$_LANG['domaincontactdetails']['Email'] = "ელ.ფოსტა";
$_LANG['domaincontactdetails']['Email Address'] = "ელ.ფოსტის მისამართი";
$_LANG['domaincontactdetails']['Job Title'] = "თანამდებობა";
$_LANG['domaincontactdetails']['Company Name'] = "კომპანიის სახელწოდება";
$_LANG['domaincontactdetails']['Organisation Name'] = "ორგანიზაციის დასახელება";
$_LANG['domaincontactdetails']['Address'] = "მისამართი";
$_LANG['domaincontactdetails']['Street'] = "ქუჩა";
$_LANG['domaincontactdetails']['Address 1'] = "მისამართი 1";
$_LANG['domaincontactdetails']['Address 2'] = "მისამართი 2";
$_LANG['domaincontactdetails']['Address 3'] = "მისამართი 3";
$_LANG['domaincontactdetails']['City'] = "ქალაქი";
$_LANG['domaincontactdetails']['State'] = "შტატი/რეგიონი";
$_LANG['domaincontactdetails']['County'] = "ქვეყანა";
$_LANG['domaincontactdetails']['Region'] = "რეგიონი";
$_LANG['domaincontactdetails']['Postcode'] = "საფოსტო ინდექსი";
$_LANG['domaincontactdetails']['ZIP Code'] = "ZIP კოდი";
$_LANG['domaincontactdetails']['ZIP'] = "ZIP";
$_LANG['domaincontactdetails']['Country'] = "ქვეყანა";
$_LANG['domaincontactdetails']['Phone'] = "ტელეფონი";
$_LANG['domaincontactdetails']['Phone Number'] = "ტელეფონის ნომერი";
$_LANG['domaincontactdetails']['Fax'] = "ფაქსი";

$_LANG['serverhostnameexample'] = "მაგ. server1(.yourdomain.ge)";
$_LANG['serverns1prefixexample'] = "მაგ. ns1(.yourdomain.ge)";
$_LANG['serverns2prefixexample'] = "მაგ. ns2(.yourdomain.ge)";

$_LANG['hosting'] = "ჰოსტინგი";

$_LANG['enomfrregistration']['Heading'] = ".fr domains have different required values depending on your nationality and type of registration:";
$_LANG['enomfrregistration']['French Individuals']['Name'] = "French Individuals";
$_LANG['enomfrregistration']['French Individuals']['Requirements'] = "Please provide your \"Birthdate\", \"Birthplace City\", and \"Birthplace Postcode\".";
$_LANG['enomfrregistration']['EU Non-French Individuals']['Name'] = "EU Non-French Individuals";
$_LANG['enomfrregistration']['EU Non-French Individuals']['Requirements'] = "Please provide your \"Birthdate\".";
$_LANG['enomfrregistration']['French Companies']['Name'] = "French Companies";
$_LANG['enomfrregistration']['French Companies']['Requirements'] = "Please provide the \"Birthdate\", \"Birthplace City\", and \"Birthplace Postcode\" for the owner contact, along with your SIRET number.";
$_LANG['enomfrregistration']['EU Non-French Companies']['Name'] = "EU Non-French Companies";
$_LANG['enomfrregistration']['EU Non-French Companies']['Requirements'] = "Please provide the company \"DUNS Number\", and the \"Birthdate\" of the Owner Contact.";
$_LANG['enomfrregistration']['Non-EU Warning'] = "Client contact information must be within the EU or else registration will fail.";

$_LANG['confirm'] = "დადასტურება";

$_LANG['maxmind_checkconfiguration'] = "An error occurred with the Fraud Check. Please contact support.";
$_LANG['maxmind_addressinvalid'] = "Your address is not recognised. Please check and re-enter.";
$_LANG['maxmind_invalidip'] = "IP Address invalid or a local address. Please contact support.";

$_LANG['ssounabletologin'] = "Unable to auto-login. Please contact support.";
$_LANG['ssofatalerror'] = "A fatal error occurred. Please contact support.";

# Version 6.0

$_LANG['announcementschoosemonth'] = "აირჩიეთ თვე";
$_LANG['announcementsbymonth'] = "თვეებით";
$_LANG['announcementsolder'] = "ძველი განცხადებები";
$_LANG['createnewcontact'] = "კონტაქტის დამატება...";
$_LANG['due'] = "Due";
$_LANG['affiliatessignups'] = "რეგისტრაცია";
$_LANG['affiliatesconversionrate'] = "კონვერსია";
$_LANG['affiliatesclicks'] = "კლიკები";
$_LANG['contacts'] = "კონტაქტები";
$_LANG['backtoservicedetails'] = "უკან დაბრუნება";
$_LANG['invoicesintro'] = "თქვენი ინვოისების ისტორია";

$_LANG['sidebars']['viewAccount']['yourAccount'] = "თქვენი ანგარიში";
$_LANG['sidebars']['viewAccount']['myDetails'] = "ჩემი მონაცემები";
$_LANG['sidebars']['viewAccount']['billingInformation'] = "საბილინგო ინფორმაცია";
$_LANG['sidebars']['viewAccount']['contacts/subAccounts'] = "კონტაქტები/ქვე-ანგარიშები";
$_LANG['sidebars']['viewAccount']['changePassword'] = "პაროლის შეცვლა";
$_LANG['sidebars']['viewAccount']['securitySettings'] = "უსაფრთხოების მონაცემები";
$_LANG['sidebars']['viewAccount']['emailHistory'] = "ელ.ფოსტის ისტორია";

$_LANG['aboutsecurityquestions'] = "რისთვისაა საიდუმლო კითხვა?";
$_LANG['registersecurityquestionblurb'] = "ეს პარამეტრი უზრუნველყოფს დამატებით უსაფრთხოება, თქვენს ანგარიშსზე განსახორციებელი ნებისმიერი ქმედებისთვის საჭიროა პასუხის ცოდნა.";

$_LANG['update'] = "განახლება";
$_LANG['yourinfo'] = "თქვენი ინფო";
$_LANG['shortcuts'] = "მალსახმობები";

$_LANG['yourservices'] = "თქვენი სერვისები";
$_LANG['yourdomains'] = "თქვენი დომეინები";
$_LANG['yourtickets'] = "თქვენი ბილეთები";
$_LANG['managecontacts'] = "კონტაქტების მართვა";
$_LANG['billingdetails'] = "საბილინგო დეტალები";
$_LANG['homechooseproductservice'] = "აირჩიეთ პროდუქტი/სერვისი სამართავად:";

$_LANG['invoicesdue'] = "გადასახდელი ინვოისი";
$_LANG['invoicesduemsg'] = "თქვენ გაქვთ %s გადაუხდელი ინვოისი, რომლის(რომელთა) საერთო გადასახადია %s";
$_LANG['noinvoicesduemsg'] = "ამჟამად არ არის გადაუხდელი ინვოისები.";

$_LANG['expiringsoon'] = "ვადის გასვლამდე ცოტაღა დარჩა";

$_LANG['notice'] = "შენიშნვნა";
$_LANG['networkstatussubtitle'] = "სიახლეები";

$_LANG['myaccount'] = "ჩემი ანგარიში";

$_LANG['manageproduct'] = "სერვისის მართვა";
$_LANG['overview'] = "ზოგადი მიმოხილვა";
$_LANG['servername'] = "სერვერის დასახელება";
$_LANG['visitwebsite'] = "ეწვიეთ საიტს";
$_LANG['whoisinfo'] = "WHOIS ინფო";

$_LANG['tableshowing'] = "ნაჩვენებია _START_ დან _END_ - სულ _TOTAL_ ჩანაწერი";
$_LANG['tableempty'] = "ნაჩვენებია 0 დან 0 - სულ 0 ჩანაწერი";
$_LANG['tablefiltered'] = "(ფილტრირებული სულ _MAX_ ჩანაწერიდან)";
$_LANG['tablelength'] = "მაქს. _MENU_ ჩვენება";
$_LANG['tableloading'] = "დაელოდეთ...";
$_LANG['tableprocessing'] = "იტვირთება...";
$_LANG['tablepagesfirst'] = "პირველი";
$_LANG['tablepageslast'] = "ბოლო";
$_LANG['tablepagesnext'] = "შემდეგი";
$_LANG['tablepagesprevious'] = "წინა";
$_LANG['tableviewall'] = "ყველა";
$_LANG['tableentersearchterm'] = "ძიება...";

$_LANG['actions'] = "მოქმედებები";

$_LANG['upgradedowngradeshort'] = "აწევა/დაწევა";

$_LANG['masspayintro'] = "გადაიხადე ყველა ინვოისი ერთდროულად";
$_LANG['masspaymentselectgateway'] = "აირჩიეთ მასობრივი გადახდის საშუალება";

$_LANG['ticketfeedbackrequest'] = "Feedback Request";
$_LANG['ticketfeedbackforticket'] = "for Ticket #";

$_LANG['notifications'] = "შეტყობინებები";
$_LANG['notificationsnone'] = "ამ დროისთვის, შეტყობინებები არ არის.";

$_LANG['creditcardnonestored'] = "No card on file";

$_LANG['kbviewingarticlestagged'] = "იხილეთ მონიშნული სტატიები";

$_LANG['domainprivatenameservers'] = "პირადი Nameserver-ები";

$_LANG['transferinadomain'] = "დომეინების ტრანსფერი";

$_LANG['nodomainextensions'] = "There are no Domain Extensions currently configured for purchase";

$_LANG['homebegin'] = "მოიძიეთ თქვენთვის სასურველი დომეინი...";
$_LANG['howcanwehelp'] = "რით შეგვიძლია დაგეხმაროთ?";
$_LANG['exampledomain'] = "მაგ. domain.com";
$_LANG['buyadomain'] = "დომეინის ყიდვა";
$_LANG['orderhosting'] = "სერვისის შეკვეთა";
$_LANG['makepayment'] = "გადახდა";
$_LANG['getsupport'] = "მიიღე დახმარება";

$_LANG['news'] = "სიახლეები";
$_LANG['allthelatest'] = "სიახლეები შემდეგი წყაროდან";
$_LANG['readmore'] = "იხილეთ მეტი";
$_LANG['noannouncements'] = "განცხადებები არ არის";

$_LANG['kbsearchexplain'] = "გაქვთ შეკითხვა? დაიწყეთ პასუხის ძებნა აქედან.";
$_LANG['readyforquestions'] = "ჩვენ მზად ვართ ვუპასუხოთ თქვენს შეკითხვებს";

$_LANG['restrictedpage'] = "გვერდი ხელმიუწვდომელია";
$_LANG['enteremail'] = "შეიყვანეთ ელ.ფოსტა";

$_LANG['passwordtips'] = "<strong>რჩევები კარგი პაროლისთვის</strong><br />გამოიყენეთ დიდი და პატარა ასოები<br />გამოიყენეთ მინ. ერთი სიმბოლო (# $ ! % &amp; etc...)<br />არ გამოიყენოთ გამოყენებადი სიტყვები";

$_LANG['regdate'] = "რეგ. თარიღი";
$_LANG['nextdue'] = "შემდგომი გადახდა";

$_LANG['domaincheckertagline'] = "მოძებნეთ თქვენთვის სასურველი დომეინი...";
$_LANG['findyourdomain'] = "მოძებნეთ სასურველი დომეინი";
$_LANG['searchtermrequired'] = "საძიებლად შეიყვანეთ დომეინი დასახელება ან საძიებო სიტყვა";
$_LANG['unabletolookup'] = "ვწუხვართ, მითითებული სიტყვებით რაიმეს მოძებნა ვერ მოხერხდა";
$_LANG['invalidchars'] = "ნუ გამოიყენებთ 'space' ღილაკს და სპეციალური სიმბოლოებს";
$_LANG['bulkoptions'] = "მასობრივი პარამეტრები";
$_LANG['checkingdomain'] = "მიმდინარეობს მოთხოვნილი დომეინის ხელმისაწვდომობის შემოწმება...";
$_LANG['domainsgotocheckout'] = "გადახდაზე გადასვლა";
$_LANG['domainssearchresults'] = "ძიების შედეგები";
$_LANG['domainssuggestions'] = "რჩევები";
$_LANG['domainsothersuggestions'] = "შესაძლოა დაინტერესებული იყოთ ალტერნატიული დასახელებებით";
$_LANG['domainsmoresuggestions'] = "მომეცი მეტი რჩევა";
$_LANG['domainssuggestionswarnings'] = "შესაძლოა დომეინის რჩევის ფუნქცია ყოველეთვის არ იყოს ხელმისაწვდომი. ხოლო დომეინის ხელმისაწვდომობა მოწმდება რეალური დროის რეჟიმში.";
$_LANG['disclaimers'] = "შენიშვნა";
$_LANG['tldpricing'] = "TLD ფასები";
$_LANG['alltldpricing'] = "ყველა TLD-ს ფასი";

$_LANG['quotesdesc'] = "Quotes we have generated for you";
$_LANG['quotesrejected'] = "Rejected";

$_LANG['ticketsyourhistory'] = "ბილეთების ისტორია";

$_LANG['clientareaemaildesc'] = "თქვენი ელ.ფოსტის ისტორია";

$_LANG['sslconfssl'] = "SSL მოწყობა";
$_LANG['sslnoconfigurationpossible'] = "წარმატებით დაკონფიგურირდა!  ხარვეზის აღმოჩენის შემთხვევაში დაგვიკავშირდით.";

$_LANG['adminloggedin'] = "თქვენ შემოსული ხართ, როგორც ადმინისტრატორი.";
$_LANG['returntoadminarea'] = "მართვის პანელში დაბრუნება";
$_LANG['adminmasqueradingasclient'] = "პანელს ათვალიერებთ როგორც მომხმარებელი.";
$_LANG['logoutandreturntoadminarea'] = "მართვის პანელში დაბრუნება";

$_LANG['supportAndUpdatesExpired'] = "Support & Updates are expired.";
$_LANG['supportAndUpdatesExpiredLicense'] = "Your Support & Updates period for this license has expired";
$_LANG['supportAndUpdatesRenewalRequired'] = "Support & Updates access needs to be renewed before you can access this download.";
$_LANG['supportAndUpdatesClickHereToRenew'] = "Click Here to Renew";

$_LANG['pwresetemailneeded'] = "დაგავიწყდათ პაროლი? შეიყვანეთ თქვენი ელ. ფოსტა ქვემოთ და დააჭირეთ დასტურის ღილკს.";

$_LANG['quotestageexpired'] = "ვადაგასულია";

$_LANG['ticketinfo'] = "ინფორმაცია ბილეთზე";
$_LANG['customfield'] = "დამატებითი ველები";

$_LANG['domainsActive'] = "აქტიური";
$_LANG['domainsExpired'] = "ვადაგასული";
$_LANG['domainsCancelled'] = "გაუქმებული";
$_LANG['domainsFraud'] = "Fraud";
$_LANG['domainsPending'] = "დაუდასტურებელი";
$_LANG['domainsPendingTransfer'] = "Pending Transfer";
$_LANG['domainsExpiringInTheNext30Days'] = "ვადა გადის 30 დღეში";
$_LANG['domainsExpiringInTheNext90Days'] = "ვადა გადის 90 დღეში";
$_LANG['domainsExpiringInTheNext180Days'] = "ვადა გადის 180 დღეში";
$_LANG['domainsExpiringInMoreThan180Days'] = "ვადა გადის 180+ დღეში";

$_LANG['kbtagcloud'] = "თეგების ღრუბელი";

$_LANG['cancellationrequestedexplanation'] = "There is an outstanding cancellation request for this product/service";
$_LANG['cancellationrequested'] = "გაუქმება მოთხოვნილია";

$_LANG['yourrecenttickets'] = "ბოლო ბილეთები";

$_LANG['domains']['deTermsDescription1'] = "To register a new domain, transfer or change registrant information the registrant must explicitly accept the .DE terms and conditions.";
$_LANG['domains']['deTermsDescription2'] = "(See full text of .de terms and conditions: http://www.denic.de/en/bedingungen.html.)";
$_LANG['directDebitPageTitle'] = "Direct Debit Payment";
$_LANG['directDebitHeader'] = "Direct Debit Payment";
$_LANG['directDebitErrorNoBankName'] = "You must enter your banks name";
$_LANG['directDebitErrorAccountType'] = "You must select your bank account type";
$_LANG['directDebitErrorNoABA'] = "You must enter your banks ABA code";
$_LANG['directDebitErrorAccNumber'] = "You must enter your bank account number";
$_LANG['directDebitErrorConfirmAccNumber'] = "You must confirm your bank account number";
$_LANG['directDebitErrorAccNumberMismatch'] = "Your bank account number & confirmation don't match";
$_LANG['directDebitThanks'] = "Thank you for submitting your details. We will attempt to process your payment using the supplied details within the next few days, and contact you in case of any problems.";
$_LANG['directDebitPleaseSubmit'] = "Please submit your bank account details below to pay by Direct Debit.";
$_LANG['directDebitBankName'] = "ბანკის სახელი";
$_LANG['directDebitAccountType'] = "ანგარიშის ტიპი";
$_LANG['directDebitABA'] = "ბანკის ABA კოდი";
$_LANG['directDebitAccNumber'] = "ანგარიშის ნომერი";
$_LANG['directDebitConfirmAccNumber'] = "ანგარიშის ნომერის დადასტურება";
$_LANG['directDebitSubmit'] = "დადასტურება";
$_LANG['directDebitChecking'] = "მიმდინარე";
$_LANG['directDebitSavings'] = "შემნახველი";

$_LANG['outOfStockProductRemoved'] = "ვადა გასული პროდუქტი ავტომატურად წაიშალა შესყიდვების კალათიდან.";

$_LANG['subaccountpermsquotes'] = "View & Accept Quotes";

$_LANG['chooselanguage'] = "ენის არჩევა";

$_LANG['success'] = "წარმატება";
$_LANG['error'] = "შეცდომა";
$_LANG['print'] = "ამობეჭვდა";
$_LANG['invoicelineitems'] = "მომსახურება";

$_LANG['quotelineitems'] = "Quote Items";

$_LANG['quoteproposal'] = "Proposal";
$_LANG['quoteacceptagreetos'] = "To accept the quote, please confirm your acceptance of our terms of service.";
$_LANG['quoteacceptcontractwarning'] = "Please be aware that accepting a quote is considered entering into a contract and you will not be able to cancel once accepted.";

// Client alerts
$_LANG['clientAlerts']['creditCardExpiring'] = "თქვენი საკრედიტო ბარათს :creditCardType-:creditCardLastFourDigits ვადა გასდის :days დღეში. განაახლეთ ის, რაც შეძლებისდაგვარად მალე";
$_LANG['clientAlerts']['domainsExpiringSoon'] = "თქვენ :numberOfDomains დომეინს ვადა გასდის :days დღეში.";
$_LANG['clientAlerts']['invoicesUnpaid'] = "თქვენ გაქვთ :numberOfInvoices გადაუხდელი ინვოისი. გადაიხადეთ, რაც შეიძლება მალე, რათა იყოთ გათიშვისგან დაზღვეულნი";
$_LANG['clientAlerts']['invoicesOverdue'] = "თქვენ გაქვთ :numberOfInvoices გადასახდელი ინვოისი - გადასახადის შემდეგი ოდენობით :balanceDue. გადაიხადეთ მალევე, რათა თავიდან აიცილოთ მომსახურეობის შეწყვეტა.";
$_LANG['clientAlerts']['creditBalance'] = "თქვენს საკრედიტო ბალანსზეა :creditBalance.";

// Client homepage panels
$_LANG['clientHomePanels']['unpaidInvoices'] = "გადაუხდელი ინვოისები";
$_LANG['clientHomePanels']['unpaidInvoicesMsg'] = "თქვენ გაქვთ :numberOfInvoices გადაუხდელი ინვოისი - გადასახადის შემდეგი ოდენობით :balanceDue. გადაიხადეთ მალევე, რათა იყოთ გათიშვისგან დაზღვეულნი.";
$_LANG['clientHomePanels']['overdueInvoices'] = "გადასახდელი ინვოისები";
$_LANG['clientHomePanels']['overdueInvoicesMsg'] = "თქვენ გაქვთ :numberOfInvoices გადაუხდელი ინვოისი - გადასახადის შემდეგი ოდენობით :balanceDue. გადაიხადეთ მალევე, რათა თავიდან აიცილოთ მომსახურეობის შეწყვეტა.";
$_LANG['clientHomePanels']['domainsExpiringSoon'] = "დომეინების აქტიურობის ვადა იწურება";
$_LANG['clientHomePanels']['domainsExpiringSoonMsg'] = "თქვენ :numberOfDomains დომეინს ვადა გასდის :days დღეში. დღესვე გაახანგრძლივეთ მისი ვადა(ები), რათა იყოთ გამორთვისგან დაზღვეულნი.";
$_LANG['clientHomePanels']['activeProductsServices'] = "თქვენი აქტიური სერვისები";
$_LANG['clientHomePanels']['activeProductsServicesNone'] = "როგორც ჩანს თქვენ არ გაქვთ აქტიური სერვისი. <a href=\"cart.php\">თქვენ შეგიძლია შეუკვეთოთ ის ამ ბმულზე</a>.";
$_LANG['clientHomePanels']['recentNews'] = "ბოლო განახლებები";
$_LANG['clientHomePanels']['affiliateProgram'] = "პარტნ.პროგრამა";
$_LANG['clientHomePanels']['recentSupportTickets'] = "ბოლო დახმარების ბილეთები.";
$_LANG['clientHomePanels']['recentSupportTicketsNone'] = "ბილეთი არ გაქვთ გახსნილი. თუკი დახმარება გჭირდებათ, შეგიძლიათ, <a href=\"submitticket.php\">გახსნათ ბილეთი</a>.";
$_LANG['clientHomePanels']['affiliateSummary'] = "თქვენი ამჟამინდელი ბალანსია :commissionBalance. თანხის გასანაღდებლად გჭირდებათ დამატებით :amountUntilWithdrawalLevel .";
$_LANG['clientHomePanels']['affiliateSummaryWithdrawalReady'] = "ამჟამინდელი ბალანსია :commissionBalance. თქვენ შეგიძლია გაანაღდოთ გამომუშავებული თანხა.";

$_LANG['upgradeNotPossible'] = "Upgrading this product is not possible.  If you believe you've reached this page in error, please notify our support department.  Otherwise, go back and double check your selection.";

$_LANG['hostingInfo'] = "Hosting Information";
$_LANG['additionalInfo'] = "დამატებითი ინფორმაცია";
$_LANG['resourceUsage'] = "რესურსების მოხმარება";
$_LANG['primaryIP'] = "ძირითადი IP";
$_LANG['assignedIPs'] = "Assigned IPs";
$_LANG['diskSpace'] = "დისკური სივრცე";
$_LANG['bandwidth'] = "ტრაფიკი";
$_LANG['registered'] = "რეგისტრირებულია";
$_LANG['upgrade'] = "პაკეტის გაუმჯობესება";

$_LANG['downdoadsdesc'] = "სახელმძღვანელოები, პროგრამები და სხვა ფაილები";

$_LANG['doToday'] = "დღეს, რის გაკეთებას გეგმავთ?";
$_LANG['changeDomainNS'] = "Change the nameservers your domain points to";
$_LANG['updateWhoisContact'] = "Update the WHOIS contact information for your domain";
$_LANG['changeRegLock'] = "Change the registrar lock status for your domain";
$_LANG['renewYourDomain'] = "დომეინის ვადის გახანგრძლივება";

$_LANG['oops'] = "უუპს";
$_LANG['goback'] = "გაბრუნდი უკან";
$_LANG['returnhome'] = "მთავარზე დაბრუნება";
$_LANG['blankCustomField'] = "(მნიშვნელობის გარეშე)";

$_LANG['viewAll'] = "იხილეთ ყველა";
$_LANG['moreDetails'] = "მეტი ინფორმაცია";

$_LANG['clientHomeSearchKb'] = "შეიყვანეთ კითხვა და მოიძიე პასუხი 'დაზმარების გვერდზე'...";

$_LANG['whoisContactWarning'] = "It is important to keep your domain WHOIS contact information up-to-date at all times to avoid losing control of your domain.";

$_LANG['paymentstodate'] = "Payments to Date";
$_LANG['balancedue'] = "გადასახდელი თანხა";
$_LANG['submitpayment'] = "გადახდის დადასტურება";

$_LANG['domaincheckeravailable'] = "ხელმისაწვდომია";
$_LANG['domaincheckertransferable'] = "ტრანსფერისთვის ხელმისაწვდომია";
$_LANG['domaincheckertaken'] = "დაკავებულია";
$_LANG['domaincheckeradding'] = "ემატება";
$_LANG['domaincheckeradded'] = "დაემატა";
$_LANG['domaincheckernomoresuggestions'] = "თქვენთვის სულ ესა გვაქვს! თუკი ჯერაც ვერ იპოვეთ ის, რასაც ეძებდით, შეგიძლია სცადოთ სხვა საძიებო სიტყვები.";
$_LANG['domaincheckerunabletooffertld'] = "სამწუხაროდ, ამ მომენტისათვის ჩვენ არ შეგვიძლია მსგავსი სასრულის მქონე დომეინის დარეგისტირება.";
$_LANG['domaincheckerbulkplaceholder'] = "შეგიძლია შეიყვანოთ 20-მდე დომეინი.\nერთ ველზე მიუთითეთ მხოლოდ ერთი სახელწოდება.\n\nმაგალითები:\nhostnodes.com\nhostnodes.net";

$_LANG['domainchecker']['suggestiontakentitle'] = "დომეინი ხელმიუწვდომელია";
$_LANG['domainchecker']['suggestiontakenmsg'] = "სამწუხაროდ თქვენ მიერ არჩეული დომეინი ხელმიუწვდომელია. ზოგჯერ ასე მაშინ ხდება, როდესაც დომეინი ახლად დარეგისტრირებულია. გთხოვთ, დაბრუნდეთ უკან და მოიძიოთ სხვა დომეინი.";
$_LANG['domainchecker']['suggestiontakenchooseanother'] = "სხვა დომეინის შერჩევა";

$_LANG['domainchecker']['alreadyincarttitle'] = "უკვე კალათშია";
$_LANG['domainchecker']['alreadyincartmsg'] = "ეს დომეინი უკვე კალათშია. გადადით გადახდის გვერდზე და დაასრულეთ შეკვეთა.";
$_LANG['domainchecker']['alreadyincartcheckoutnow'] = "ახლავე გადაიხადე";

$_LANG['genericerror']['title'] = "უუპს, რაღაც შეცდომა დაფიქსირდა!";
$_LANG['genericerror']['msg'] = "სცადეთ კიდევ ერთხელ, თუკი შეცდომა კვლავ დაფიქსირდა მიმართეთ ტექნიკურ განყოფილებას.";

# Licensing Addon

$_LANG['licensingaddon']['mylicenses'] = "ჩემი ლიცენზიები";
$_LANG['licensingaddon']['latestdownload'] = "ბოლო გადმოწერები";
$_LANG['licensingaddon']['downloadnow'] = "გადმოწერე ახლავე";
$_LANG['licensingaddon']['licensekey'] = "License Key";
$_LANG['licensingaddon']['validdomains'] = "Valid Domains";
$_LANG['licensingaddon']['validips'] = "Valid IPs";
$_LANG['licensingaddon']['validdirectory'] = "Valid Directory";
$_LANG['licensingaddon']['status'] = "ლიცენზიის სტატუსი";
$_LANG['licensingaddon']['reissue'] = "Reissue";
$_LANG['licensingaddon']['reissuestatusmsg'] = "The Valid Domain, IP and Directory will be detected & saved the next time the license is accessed.";
$_LANG['licensingaddon']['manageLicense'] = "ლიცენზიების მართვა";

$_LANG['affiliateWithdrawalSummary'] = "თანხის განაღდებას მაშინ შეძლებთ, როდესაც ანგარიშზე მინიმუმ :amountForWithdrawal დაგროვდება.";

$_LANG['projectManagement']['activeProjects'] = "თქვენი აქტიური პროექტები";

# cPanel Module

$_LANG['cPanel']['packageDomain'] = "პაკეტი/დომეინი";
$_LANG['cPanel']['addonsExtras'] = "დამატებითი პროდუქტები";
$_LANG['cPanel']['purchaseActivate'] = "ყიდვა და გააქტიურება";

$_LANG['cPanel']['usageStats'] = "გამოყენებადობის სტატისტიკა";
$_LANG['cPanel']['diskUsage'] = "გამოყენებული სივრცე";
$_LANG['cPanel']['bandwidthUsage'] = "გამოყენებული ტრაფიკი";
$_LANG['cPanel']['usageStatsBwLimitNear'] = "უახლოვდებით ტრაფიკის ლიმიტს.";
$_LANG['cPanel']['usageStatsDiskLimitNear'] = "უახლოვდებით დისკური სივრცის ლიმიტს.";
$_LANG['cPanel']['usageUpgradeNow'] = "Upgrade Now";
$_LANG['cPanel']['usageLastUpdated'] = "ბოლოს განახლდა";

$_LANG['cPanel']['quickShortcuts'] = "მალსახმობი";
$_LANG['cPanel']['emailAccounts'] = "ელ.ფოსტის ანგარიშები";
$_LANG['cPanel']['forwarders'] = "გადამმისამართებელი";
$_LANG['cPanel']['autoresponders'] = "ავტომოპასუხე";
$_LANG['cPanel']['fileManager'] = "ფაილების მენეჯერი";
$_LANG['cPanel']['backup'] = "Backup";
$_LANG['cPanel']['subdomains'] = "ქვედომეინები";
$_LANG['cPanel']['addonDomains'] = "დამატებითი დომეინები";
$_LANG['cPanel']['cronJobs'] = "Cron Jobs";
$_LANG['cPanel']['mysqlDatabases'] = "MySQL Databases";
$_LANG['cPanel']['phpMyAdmin'] = "phpMyAdmin";
$_LANG['cPanel']['awstats'] = "Awstats";

$_LANG['cPanel']['createEmailAccount'] = "ელ.ფოსტის სწრაფადვე შექმნა";
$_LANG['cPanel']['usernamePlaceholder'] = "Yourname";
$_LANG['cPanel']['passwordPlaceholder'] = "სასურველი პაროლი";
$_LANG['cPanel']['create'] = "შექმნა";
$_LANG['cPanel']['emailAccountCreateSuccess'] = "ელ.ფოსტის ანგარიში წარმატებით შეიქმნა!";
$_LANG['cPanel']['emailAccountCreateFailed'] = "ელ.ფოსტის ანგარიში ვერ შეიქმნა: ";

$_LANG['cPanel']['packageNotActive'] = "აღნიშნული სერვისი ამ მომენტისთვის არის";
$_LANG['cPanel']['statusPendingNotice'] = "თქვენ ვერ შეძლებთ ამ სერვისით სარგებლობას, სანამ არ გაააქტიურებთ.";
$_LANG['cPanel']['statusSuspendedNotice'] = "თქვენ ვერ შეძლებთ ამ სერვისით სარგებლობას, სანამ არ გაააქტიურებთ.";

$_LANG['cPanel']['billingOverview'] = "გადახდების მიმოხილვა";

$_LANG['liveHelp']['chatNow'] = "მიწერე ახლავე";

$_LANG['quotes'] = "შეთავაზებები";

$_LANG['productMustBeActiveForModuleCmds'] = "Product must be active to perform this action";
$_LANG['domainCannotBeManagedUnlessActive'] = "This domain is not currently active. Domains cannot be managed unless active.";

$_LANG['actionRequiresAtLeastOneDomainSelected'] = 'Please select at least one domain to perform the selected action on.';

$_LANG['clientAreaProductDownloadsAvailable'] = "The following download items are available for your product/service";
$_LANG['clientAreaProductAddonsAvailable'] = "თქვენი სერვისისთის დანამატები ხელმისაწვდომია. <a href=\"cart.php?gid=addons\">დააჭირეთ, რათა ნახოთ და შეუკვეთოთ &raquo;</a>";
$_LANG['clientAreaSecurityTwoFactorAuthRecommendation'] = "ჩვენ გირჩევთ გააქტიუროთ ორმაგი ავტორიზაცია გაძლიერებული უსაფრთხოებისთვის.";
$_LANG['clientAreaSecurityNoSecurityQuestions'] = "საიდუმლო კითხვა და მისი პასუხი იცავს თქვენს ანგარიშს არა ავტორიზებული პაროლი აღდგენის მოთხოვნებისგან და ჩვენ საშუალებას გვაძლევს თქვენი ვინაობა მარტივად დავადასტუროთ.";
$_LANG['clientAreaSecuritySecurityQuestionOtherError'] = "საიდუმლო კითხვა და მისი პასუხი იცავს თქვენს ანგარიშს არა ავტორიზებული პაროლი აღდგენის მოთხოვნებისგან და ჩვენ საშუალებას გვაძლევს თქვენი ვინაობა მარტივად დავადასტუროთ.";

$_LANG['billingAddress'] = "იურიდული მისამართი";

$_LANG['noPasswordResetWhenLoggedIn'] = "სანამ სისტემაში ხართ შემოსული - ვერ მოითხოვთ პაროლის აღდგენას. გთხოვთ, პირველ რიგში, გამოხვიდეთ სისტემიდან.";

$_LANG['unableToLoadShoppingCart'] = "საყიდლების კალათის ჩატვირთა ვერ ხერხდება. დაუკავშირდით დახმარების ჯგუფს.";

$_LANG['showMenu'] = "მენიუს გახსნა";
$_LANG['hideMenu'] = "მენიუს დამალვა";

$_LANG['from'] = "From";
$_LANG['featuredProduct'] = "ყველაზე პოპულარული";
$_LANG['shoppingCartProductPerMonth'] = "<span>:price</span>/:countmo";
$_LANG['shoppingCartProductPerYear'] = "<span>:price</span>/:countyr";

$_LANG['orderForm']['findNewDomain'] = "იპოვეთ ახალი დომეინი. შეიყვანეთ რაიმე დასახელება ან საძიებო სიტყვა, რათა შეამოწმოთ ხელმისაწვდომობა.";
$_LANG['orderForm']['transferExistingDomain'] = "გააკეთეთ დომეინების ტრანსფერი ჩვენთან.";
$_LANG['orderForm']['www'] = "www.";
$_LANG['orderForm']['check'] = "შემოწმება";
$_LANG['orderForm']['returnToClientArea'] = "მომხმარებლის არეში დაბრუნება";
$_LANG['orderForm']['checkout'] = "გადახდა";
$_LANG['orderForm']['alreadyRegistered'] = "ხართ რეგისტრირებული?";
$_LANG['orderForm']['createAccount'] = "ახალი ანგარიშის შექმნა";
$_LANG['orderForm']['enterPersonalDetails'] = "შეიყვანე პირადი ინფორმაცია, რათა შეძლოთ გადახდა";
$_LANG['orderForm']['correctErrors'] = "სანამ გააგრძელებდეთ, გაასწორეთ შემდეგი შეცდომები.";
$_LANG['orderForm']['existingCustomerLogin'] = "არსებული მომხმარებლის ავტორიზაცია";
$_LANG['orderForm']['emailAddress'] = "ელ.ფოსტის მისამართი";
$_LANG['orderForm']['personalInformation'] = "პირადი ინფორმაცია";
$_LANG['orderForm']['firstName'] = "სახელი";
$_LANG['orderForm']['lastName'] = "გვარი";
$_LANG['orderForm']['phoneNumber'] = "ტელეფონის ნომერი";
$_LANG['orderForm']['billingAddress'] = "იურიდიული მისმართი";
$_LANG['orderForm']['companyName'] = "კომპანიის დასახელება";
$_LANG['orderForm']['optional'] = "არასავალდებულო";
$_LANG['orderForm']['streetAddress'] = "საცხოვრებლის მისამართი";
$_LANG['orderForm']['streetAddress2'] = "საცხოვრებლის მისამართი 2";
$_LANG['orderForm']['city'] = "ქალაქი";
$_LANG['orderForm']['state'] = "რეგიონი/შტატი";
$_LANG['orderForm']['country'] = "ქვეყანა";
$_LANG['orderForm']['postcode'] = "საფოსტო ინდექსი";
$_LANG['orderForm']['domainAlternativeContact'] = "You may specify alternative registered contact details for the domain registration(s) in your order when placing an order on behalf of another person or entity. If you do not require this, you can skip this section.";
$_LANG['orderForm']['accountSecurity'] = "ანგარიშის უსაფრთხოება";
$_LANG['orderForm']['mediumStrength'] = "საშუალო სიძლიერის";
$_LANG['orderForm']['paymentDetails'] = "გადახდის დეტალები";
$_LANG['orderForm']['preferredPaymentMethod'] = "აირჩიეთ გადახდის პრიორიტეტული მეთოდი.";
$_LANG['orderForm']['cardNumber'] = "ბარათის ნომერი";
$_LANG['orderForm']['cvv'] = "CVV Security Number";
$_LANG['orderForm']['additionalNotes'] = "დამატებითი შენიშვნები";
$_LANG['orderForm']['continueToClientArea'] = "გადადი მოხმარებლის არეში";
$_LANG['orderForm']['reviewDomainAndAddons'] = "დააზუსტეთ თქვენი დომეინი და გადახედეთ დამატებით შემოთავაზებებსაც.";
$_LANG['orderForm']['addToCart'] = "კალათაში დამატება";
$_LANG['orderForm']['addedToCartRemove'] = "დაემატა კალათაში (წაშლა)";
$_LANG['orderForm']['configureDesiredOptions'] = "შეარჩიეთ სასურველი პარამეტრები და შემდგომ შეუკვეთეთ.";
$_LANG['orderForm']['haveQuestionsContact'] = "გაქვთ კითხვები? დაუკავშირდით ჩვენს ოპერატორს.";
$_LANG['orderForm']['use'] = "გაგრძელება";
$_LANG['orderForm']['check'] = "შემოწმება";
$_LANG['orderForm']['transfer'] = "ტრანსფერი";
$_LANG['orderForm']['domainAddedToCart'] = "დომეინი თქვენს კალათაში დაემატა.";
$_LANG['orderForm']['registerLongerAndSave'] = "Register for longer and save!";
$_LANG['orderForm']['tryRegisteringInstead'] = "ნაცვლად ამისა, სცადეთ ამ დომეინის რეგისტრირება.";
$_LANG['orderForm']['domainAvailabilityCached'] = "დომეინის ხელმისაწვდომობა იქეშება, ამიტომაც, ზოგჯერ ახლად დარეგისტრირებულ დომეინებს გაჩვენებთ, როგორც ხელმისაწვდომს.";
$_LANG['orderForm']['submitTicket'] = "ბილეთის გახსნა";
$_LANG['orderForm']['promotionAccepted'] = "ფასდაკლების კოდი მიღებულია! შეკვეთაც განახლდა.";
$_LANG['orderForm']['productOptions'] = "პროდუქტი/პარამეტრები";
$_LANG['orderForm']['qty'] = "Qty";
$_LANG['orderForm']['priceCycle'] = "ფასი/ციკლი";
$_LANG['orderForm']['edit'] = "ჩასწორება";
$_LANG['orderForm']['update'] = "განახლება";
$_LANG['orderForm']['remove'] = "წაშლა";
$_LANG['orderForm']['applyPromoCode'] = "გამოიყენე ფასდაკლების კოდი";
$_LANG['orderForm']['estimateTaxes'] = "Estimate Taxes";
$_LANG['orderForm']['removePromotionCode'] = "ფასდაკლების კოდის წაშლა";
$_LANG['orderForm']['updateTotals'] = "Update Totals";
$_LANG['orderForm']['continueShopping'] = "შესყიდვების გაგრძელება";
$_LANG['orderForm']['removeItem'] = "პროდუქტის / სერვისის წაშლა კალათიდან";
$_LANG['orderForm']['yes'] = "დიახ";
$_LANG['orderForm']['cancel'] = "უარყოფა";
$_LANG['orderForm']['close'] = "დახურვა";
$_LANG['orderForm']['totals'] = "ჯამი";
$_LANG['orderForm']['includedWithPlans'] = "მოყვება თითოეულ პაკეტს";
$_LANG['orderForm']['whatIsIncluded'] = "კიდევ რა მოყვება?";
$_LANG['orderForm']['errorNoProductGroup'] = "Could not load any product groups.";
$_LANG['orderForm']['errorNoProducts'] = "Product group does not contain any visible products";

$_LANG['cloudSlider']['feature01Title'] = "Maximum Server Uptime";
$_LANG['cloudSlider']['feature01Description'] = "Server uptime is critical for all businesses - consider it the heart beat of hosting your business. File and databases servers, email and web servers are an indispensable part of most business processes, and downtimes have direct negative effects on productivity, sales, employee and customer satisfaction. That's why ensuring the maximum server uptime is so important to us - we want to make sure your business processes are running healthy so your customers are happy.";
$_LANG['cloudSlider']['feature01DescriptionTwo'] = "By trusting us with your business needs, we promise you a 99.9% uptime on any services we provide, outside of any standard maintenance we may provide.";
$_LANG['cloudSlider']['feature02Title'] = "World Wide Data Delivery";
$_LANG['cloudSlider']['feature02Description'] = "Our services are powered by hundreds of servers and data centers located all over the world, so you can rest easy knowing that customers can access your website from anywhere. In addition, we provide monitoring tools to provide you with expert analytics - website traffic is an important step in improving your site's efficiency and popularity, as well as keeping track of where your visitors are coming from, what time of day they're visiting you, and how long they're staying. Our dedication to a global marketplace extends to domain registrations, as we offer the most popular TLD's for registration.";
$_LANG['cloudSlider']['feature02DescriptionTwo'] = "Our dedication to customer support reaches across the globe as well. We are here to help you with your hosting in any way possible, and you can reach us via phone, email, or live chat.";
$_LANG['cloudSlider']['feature03Title'] = "Top Tier Security";
$_LANG['cloudSlider']['feature03Description'] = "Rest easy knowing that we provide 24/7 security monitoring and DDoS protection. You take protecting your customer's data seriously, and so do we. Our security team and safeguards are on the job all day, every day to provide the level of security needed in today's digital age.";
$_LANG['cloudSlider']['feature03DescriptionTwo'] = "A wide range of security tools are at your disposal, including SSL certificates, firewall configuration, security monitoring services, VPN access, and more.";

$_LANG['domainChecker.additionalPricingOptions'] = 'Additional Pricing Options For :domain';

$_LANG['orderpaymenttermfree'] = "უფასო";

$_LANG['cPanel']['usageStatsBwOverLimit'] = "თქვენ გადააჭარბეთ ტრაფიკის ლიმიტს.";
$_LANG['cPanel']['usageStatsDiskOverLimit'] = "თქვენ გადააჭარბეთ დისკური სივრცის ლიმიტს.";
$_LANG['insufficientstockmessage'] = "We are currently low on stock on certain items. The cart quantities have been adjusted accordingly.";

$_LANG['only'] = "მხოლოდ";
$_LANG['startingat'] = "Starting at";

$_LANG['yourdomainplaceholder'] = 'თქვენი დომეინი';
$_LANG['yourtldplaceholder'] = 'ge';

$_LANG['subaccountpermsproductsso'] = "ერთჯერადად შესვლა";

$_LANG['sso']['title'] = "Single Sign-On";
$_LANG['sso']['summary'] = "Third party applications leverage the Single Sign-On functionality to provide direct access to your billing account without you having to re-authenticate.";
$_LANG['sso']['disablenotice'] = "You may wish to disable this functionality if you provide access to any of your third party applications to users who you do not wish to be able to access your billing account.";
$_LANG['sso']['enabled'] = "Single Sign-On is currently permitted for your account.";
$_LANG['sso']['disabled'] = "Single Sign-On is currently disabled for your account.";
$_LANG['sso']['redirectafterlogin'] = "You will be redirected after login";

$_LANG['oauth']['badTwoFactorAuthModule'] = "Bad Two-Factor Auth Module. Please contact support.";
$_LANG['oauth']['permAccessNameAndEmail'] = "Access your name and email address";

$_LANG['errorButTryAgain'] = "დაფიქსირდა შეცდომა. სცადეთ კიდევ ერთხელ.";
$_LANG['emailSent'] = "წერილი გაგზავნილია";
$_LANG['verificationKeyExpired'] = "მითითებული კოდის მოქმედების ვადა ამოწურულია. გთხოვთ, ხელახლა გაიაროთ ავტორიზაცია, რათა მოითხოვოთ ახალი კოდი.";
$_LANG['emailAddressVerified'] = "გმადლობთ, თქვენი ელ.ფოსტის მისამართის დადასტურებისთვის.";
$_LANG['emailKeyExpired'] = "მითითებული კოდის მოქმედების ვადა ამოწურულია. გთხოვთ, მოითხოვოთ ახალი კოდი.";
$_LANG['verifyEmailAddress'] = "გთხოვთ, შეამოწმოთ თქვენი ელ. ფოსტა და მიჰყვეთ შესაბამის ინსტრუქციას, რათა დაადასტუროთ თქვენი ელ. ფოსტის მისამართი.";
$_LANG['resendEmail'] = "ვერიფიკაციის კოდის გაგზავნა ელ. ფოსტაზე";



$_LANG['ghosting']['nav']['nav'] = 'ნავიგაცია';
$_LANG['ghosting']['nav']['main'] = 'მთავარი';
$_LANG['ghosting']['nav']['webhosting'] = 'ვებ ჰოსტინგი';
$_LANG['ghosting']['nav']['vps'] = 'VPS სერვერები';
$_LANG['ghosting']['nav']['db'] = 'მონაცემთა ბაზები';
$_LANG['ghosting']['nav']['service'] = 'პროფესიონალური ჰოსტინგი';
$_LANG['ghosting']['nav']['about'] = 'კომპანიის შესახებ';
$_LANG['ghosting']['nav']['contact'] = 'კონტაქტი';

$_LANG['ghosting']['slider']['1'] = 'განათავსეთ თქვენი საიტები GOOGLE - ის სერვერებზე.';
$_LANG['ghosting']['slider']['2'] = 'საუკეთესო ჰოსტინგი თქვენი საიტებისთვის.';
$_LANG['ghosting']['slider']['3'] = 'მაღალი სიჩქარე.';
$_LANG['ghosting']['slider']['4'] = 'უსაფრთხოება ყოველთვის მნიშვნელოვანია.';



$_LANG['ghosting']['advantages']['0'] = 'უპირატესობები';
$_LANG['ghosting']['advantages']['1'] = '%99 სამუშაო რეჟიმი';
$_LANG['ghosting']['advantages']['1_desc'] = 'სერვერის გათიშვა შესაზლებელია მოხდეს იშვიათ შემთხვევებში, მაგალითად როცა პროგრამული უზრუნველყოფა საჭიროებს განახლებას.';
$_LANG['ghosting']['advantages']['2'] = 'მარტივი სამართავი სისტემა';
$_LANG['ghosting']['advantages']['2_desc'] = 'თქვენი სერვერის მართვა შესაძლებელია თქვენივე მართვის პანელიდან.';
$_LANG['ghosting']['advantages']['3'] = 'მაღალი სიჩქარე';
$_LANG['ghosting']['advantages']['3_desc'] = 'ჩვენი სერვისი იყენებს GOOGLE - ის სერვერებს, რომელიც უზრუნველყოფს სერვერის მუშაობის მაღალ სიჩქარეს.';
$_LANG['ghosting']['advantages']['4'] = 'უსაფრთხოება';
$_LANG['ghosting']['advantages']['4_desc'] = 'ჩვენი სერვისი იყენებს GOOGLE - ის სერვერებს, რომელიც უზრუნველყოფს უსაფრთხოების მაღალ დონეს.';
$_LANG['ghosting']['advantages']['5'] = '7/24 მხარდაჭერა';
$_LANG['ghosting']['advantages']['5_desc'] = 'ნებისმიერი პრობლემის შესახებ შეგიძლიათ დაგვიკავშირდეთ და ჩვენ უმოკლეს დროში დაგეხმარებით.';

$_LANG['ghosting']['text']['not_registered'] = 'ჯერ არ ხართ დარეგისტრირებული?';
$_LANG['ghosting']['text']['register'] = 'რეგისტრაცია';
$_LANG['ghosting']['text']['registered'] = 'უკვე დარეგისტრირებული ხართ?';
$_LANG['ghosting']['text']['login'] = 'ავტორიზაცია';

$_LANG['ghosting']['text']['company_address'] = ' თბილისი, შ. ნადირაშვილის #42';
$_LANG['ghosting']['text']['terms'] = 'წესები და პირობები';
$_LANG['ghosting']['text']['policy'] = 'კონფიდენციალურობის პოლიტიკა';
$_LANG['ghosting']['text']['contract'] = 'ხელშეკრულება';

$_LANG['ghosting']['text']['about'] = 'კომპანიის შესახებ';
$_LANG['ghosting']['text']['about_desc'] = 'gHosting.ge - გთავაზობთ პროფესიონალურ ჰოსტინგ მომსახურებას თქვენი საიტებისთვის, რისთვისაც იყენებს კომპანია GOOGLE - ის ოფიციალურ სერვერებს მთელი მსოფლიოს მაშტაბით.<br>ასევე ჩვენი კომპანია გთავაზობთ პროფესიონალურ მომსახურებას სერვერის კონფიგურაციის და ადმინისტრირების კუთხით.<br><br>GOOGLE - ის სერვერები გამოირჩევიან სტაბილურობით და მაღალი სიჩქარით, ხოლო ჩვენი სპეციალისტები მუდმივად მზად არიან
ოპერატიულად დაგეხმარონ თქვენი პრობლემების გადაწყვეტაში.<br>
როცა ჩვენ გვანდობთ თქვენს მომსახურებას, თქვენ არასდროს მოგიწევთ ფიქრი საიტის გამართულ მუშაობაზე. 
აქედან გამომდინარე, რატომ უნდა გარისკოთ თქვენი ბიზნესის მინდობით ვინმე სხვისთვის?';

$_LANG['ghosting']['text']['choose_services'] = 'შეარჩიეთ სასურველი სერვისი';
$_LANG['ghosting']['text']['our_services'] = 'ჩვენი სერვისები';

$_LANG['ghosting']['text']['web_hosting'] = 'ვებ ჰოსტინგი';
$_LANG['ghosting']['text']['web_hosting_desc'] = 'თითეული ჰოსტინგ აქაუნთი განთავსებულია იზოლირებულ ვირტუალურ სერვერზე, რომლის მართვაც შესაძლებელია სპეციალური მართვის პანელით. ასევე სერვერის მართვა შესაძლებელია SSH კლიენტის საშუალებითაც (root უფლებებით).';
$_LANG['ghosting']['text']['vps_hosting'] = 'VPS სერვერები';
$_LANG['ghosting']['text']['vps_hosting_desc'] = 'VPS სერვერზე შესაძლებელია ნებისმიერი პროგრამული უზრუნველყოფის დაყენება და სერვერის სრულყოფილად მართვა.';
$_LANG['ghosting']['text']['db'] = 'მონაცემთა ბაზები';
$_LANG['ghosting']['text']['db_desc'] = 'თუ თქვენი საიტი იყენებს MSSQL, MYSQL ან Postgresql დიდი მოცულობის მონაცემთა ბაზას, რამაც შეიძლება გამოიწვიოს საიტის მუშაობის შეფერხება სერვერის რესურსების დიდი ნაწილის მონაცემთა ბაზის მიერ დაკავების გამო. გამოიყენეთ google - ს სპეციალისტების მიერ კონფიგურირებული და ოფტიმიზირებული მონაცემთა ბაზა.';
$_LANG['ghosting']['text']['pro_hosting'] = 'პროფესიონალური ჰოსტინგი';
$_LANG['ghosting']['text']['pro_hosting_desc'] = 'თუ თქვენი საიტი ემსახურება დიდი რაოდენობის ვიზიტორებს და საჭიროებს სპეციალურ სერვერულ მომსახურებას ჩვენი კომპანია გთავაზობთ პროფესიონალურ მომსახურებას სერვერის კონფიგურაციის და ადმინისტრირების კუთხით.';

$_LANG['ghosting']['text']['pro_service']['title'] = 'პროფესიონალური ჰოსტინგი';
$_LANG['ghosting']['text']['pro_service']['desc'] = 'თუ თქვენი საიტი ემსახურება დიდი რაოდენობის ვიზიტორებს და საჭიროებს სპეციალურ სერვერულ მომსახურებას ჩვენი კომპანია გთავაზობთ პროფესიონალურ მომსახურებას სერვერის კონფიგურაციის და ადმინისტრირების კუთხით.

<h2>როგორ მოქმედებს მომხმარებლების ტრაფიკი თქვენი ბიზნესისთვის.</h2>
გსურთ თქვენი კომპანია იყოს პოპულარული და ტრენდული. დღეს ეს ბიზნესის კეთების ნაწილია.
<br><br>
თუმცა, ტრენდულობა ნიშნავს ტრაფიკს და ტრაფიკი ნიშნავს თქვენს საიტზე დიდ დატვირთვას.
<br><br>
შეძლებს თუ არა თქვენი საიტი გაუძლოს თქვენი მარკეტინგული კამპანიის შედეგად გაზრდილ მომხმარებლების რაოდენობას?
<ul>
  <li>შეძლებთ თუ არა მოახდინოთ პირდაპირი ტრანსლაცია?</li>
  <li>შეძლებთ თუ არა მოახდინოთ მედია კონტენტის გავრცელება?</li>
</ul>
ისე რომ არ იდარდოთ საიტის მუშაობის შეფერხებაზე ან (თქვენ საერთოდ არ გიფიქრიათ ამ საკითხზე) სერვერის სრულ დაცემაზე?<br>
ერთერთი მეთოდი იმისთვის, რომ დარწმუნებული იყოთ თქვენი საიტის მზადყოფნაში გაუძლოს ყველა თქვენს მოთხოვნას არის "დატვირთვის ბალანსირება" სხვადასხვა სერვერებს შორის. "დატვირთვის ბალანსირების" დროს გამოიყენება სხვადასხვა ალგორითმები ტრაფიკის სხვადასხვა სერვერზე თანაბრად განაწილებისთვის, რაც უზრუნველყოფს საიტის ოფტიმალურ მუშაობას.<br><br>

მომსახურების საფასური განისაზღვრება შესასრულებელი სამუშაოებიდან გამომდინარე.<br>
დამატებითი ინფორმაციისთვის <a href="/contact.php">დაგვიკავშირდით.</a>';

$_LANG['ghosting']['text']['about_us']['title'] = 'კომპანიის შესახებ';
$_LANG['ghosting']['text']['about_us']['desc'] = 'gHosting.ge - გთავაზობთ პროფესიონალურ ჰოსტინგ მომსახურებას თქვენი საიტებისთვის, რისთვისაც იყენებს კომპანია GOOGLE - ის ოფიციალურ სერვერებს მთელი მსოფლიოს მაშტაბით.<br>ასევე ჩვენი კომპანია გთავაზობთ პროფესიონალურ მომსახურებას სერვერის კონფიგურაციის და ადმინისტრირების კუთხით.<br><br>GOOGLE - ის სერვერები გამოირჩევიან სტაბილურობით და მაღალი სიჩქარით, ხოლო ჩვენი სპეციალისტები მუდმივად მზად არიან
ოპერატიულად დაგეხმარონ თქვენი პრობლემების გადაწყვეტაში.<br>
როცა ჩვენ გვანდობთ თქვენს მომსახურებას, თქვენ არასდროს მოგიწევთ ფიქრი საიტის გამართულ მუშაობაზე. 
აქედან გამომდინარე, რატომ უნდა გარისკოთ თქვენი ბიზნესის მინდობით ვინმე სხვისთვის?
<h2>სერვისები</h2>
<ul>
<li>
    <a href="[seo_web_hosting_url]"><b>ვებ ჰოსტინგი</b></a> - თითეული ჰოსტინგ აქაუნთი განთავსებულია იზოლირებულ ვირტუალურ სერვერზე, რომლის მართვაც შესაძლებელია სპეციალური მართვის პანელით. ასევე სერვერის მართვა შესაძლებელია SSH კლიენტის საშუალებითაც (root უფლებებით), რაც საშუალებას იძლევა სერვერზე დააყენოთ ნებისმიერი პროგრამული უზრუნველყოფა თქვენი მოთხოვნებიდან გამომდინარე.
</li>
<li style="margin-top: 10px;">
    <a href="[seo_vps_servers_url]"><b>VPS სერვერები</b></a> - VPS სერვერზე შესაძლებელია ნებისმიერი პროგრამული უზრუნველყოფის დაყენება და სერვერის სრულყოფილად მართვა.
</li>
<li style="margin-top: 10px;">
    <a href="[seo_databases_url]"><b>მონაცემთა ბაზები</b></a> - თუ თქვენი საიტი იყენებს MSSQL, MYSQL ან Postgresql დიდი მოცულობის მონაცემთა ბაზას, რამაც შეიძლება გამოიწვიოს საიტის მუშაობის შეფერხება სერვერის რესურსების დიდი ნაწილის მონაცემთა ბაზის მიერ დაკავების გამო. გამოიყენეთ GOOGLE - ის სპეციალისტების მიერ კონფიგურირებული და ოფტიმიზირებული მონაცემთა ბაზა.
</li>
<li style="margin-top: 10px;">
    <a href="[seo_profesional_service_url]"><b>პროფესიონალური ჰოსტინგი</b></a> - თუ თქვენი საიტი ემსახურება დიდი რაოდენობის ვიზიტორებს და საჭიროებს სპეციალურ სერვერულ მომსახურებას ჩვენი კომპანია გთავაზობთ პროფესიონალურ მომსახურებას სერვერის კონფიგურაციის და ადმინისტრირების კუთხით.
</li>
</ul>';


$_LANG['ghosting']['options']['CPU'] = 'პროცესორის სიმძლავრე';
$_LANG['ghosting']['options']['RAM'] = 'ოპერატიული მეხსიერება';
$_LANG['ghosting']['options']['HDD'] = 'დისკის მოცულობა';
$_LANG['ghosting']['options']['Personal IP Address'] = 'საკუთარი IP მისამართი';
$_LANG['ghosting']['options']['OS'] = 'ოპერაციული სისტემა';
$_LANG['ghosting']['options']['GPU'] = 'გრაფიკული ბარათი';
$_LANG['ghosting']['options']['GPU Count'] = 'გრაფიკული ბარათების რაოდენობა';
$_LANG['ghosting']['options']['DB'] = 'მონაცემთა ბაზა';

$_LANG['ghosting']['gateways']['Invoice'] = 'ინვოისით გადახდა';
$_LANG['ghosting']['gateways']['Visa'] = 'ბარათით გადახდა (Visa / Mastercard)';

$_LANG['ghosting']['gateways']['pay'] = 'გადახდა';
$_LANG['ghosting']['gateways']['redirecting'] = 'გადამისამართება...';
$_LANG['ghosting']['gateways']['redirecting2'] = 'გთხოვთ დაელოდოთ, მიმდინარეობს ავტომატური გადამისამართება გადახდის გვერდზე...';
$_LANG['ghosting']['gateways']['redirecting3'] = 'თუ ერთი წუთის განმავლობაში არ მოხერხდა ავტომატური გადამისამართება დააჭირეთ შემდეგ ღილაკს';
$_LANG['ghosting']['gateways']['redirecting4'] = 'გადახდის გვერდძე გადასვლა';
$_LANG['ghosting']['control_panel'] = 'სამართავი პანელი';

$_LANG['ghosting']['reboot']['label'] = 'გადატვირთვა';
$_LANG['ghosting']['reboot']['confirm'] = 'ნამდვილად გსურთ სერვერის გადატვირთვა?';
$_LANG['ghosting']['reboot']['yes'] = 'დიახ';
$_LANG['ghosting']['reboot']['no'] = 'არა';


////////// End of english language file.  Do not place any translation strings below this line!